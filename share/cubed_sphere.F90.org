...
!       stop
       call rotation_lonlat_xaxis_to_ll0(lon0,lat0,1.0_r8,lon,lat,rs,lon,lat)
     endif
   endif
!
   return
...
! local variables
   integer(i4) :: ierr
   integer(i4) :: fileid
   integer(i4) :: did_nface, did_np, did_ne, did_nprocs, did_ncols, did_nrows, did_nvrts, did_nnbrs, did_n_nbr
   integer(i4) :: vid_iface, vid_ie, vid_je, vid_global, vid_isfc, vid_rank, vid_local, vid_joiner, vid_nbrs
   integer(i4) :: vid_alpha, vid_beta, vid_lon, vid_lat
   integer(i4) :: np, ne, nface, nprocs, ncols, nrows, iface, ie, je
...
!
   ...
   ierr = nf90_def_dim(fileid,'ncols',ncols,did_ncols)
   ierr = nf90_def_dim(fileid,'nrows',nrows,did_nrows)
   ...
   ! define variables (i4)
   ierr = nf90_def_var(fileid,'iface',nf90_real8,(/did_ncols,did_nrows/),vid_iface)
   ierr = nf90_def_var(fileid,'ie',nf90_int,(/did_ncols,did_nrows/),vid_ie)
   ierr = nf90_def_var(fileid,'je',nf90_int,(/did_ncols,did_nrows/),vid_je)
   ierr = nf90_def_var(fileid,'global',nf90_int,(/did_ncols,did_nrows/),vid_global)
   ierr = nf90_def_var(fileid,'isfc',nf90_int,(/did_ncols,did_nrows/),vid_isfc)
   ierr = nf90_def_var(fileid,'rank',nf90_int,(/did_ncols,did_nrows/),vid_rank)
   ierr = nf90_def_var(fileid,'local',nf90_int,(/did_ncols,did_nrows/),vid_local)
   ierr = nf90_def_var(fileid,'joiner',nf90_int,(/did_ncols,did_nrows/),vid_joiner)
   ierr = nf90_def_var(fileid,'nbrs',nf90_int,(/did_n_nbr,did_nnbrs,did_ncols,did_nrows/),vid_nbrs)
   ! define variables (r8)
   ierr = nf90_def_var(fileid,'alpha',nf90_real8,(/did_nvrts,did_ncols,did_nrows/),vid_alpha)
   ierr = nf90_def_var(fileid,'beta',nf90_real8,(/did_nvrts,did_ncols,did_nrows/),vid_beta)
   ierr = nf90_def_var(fileid,'lon',nf90_real8,(/did_nvrts,did_ncols,did_nrows/),vid_lon)
   ierr = nf90_def_var(fileid,'lat',nf90_real8,(/did_nvrts,did_ncols,did_nrows/),vid_lat)
   ! file close
...
! local variables
   integer(i4) :: np, ne, nface, nprocs, ncols, nrows, iface, ie, je, ic, jc
   integer(i4), dimension(:,:), allocatable :: base
!
   np = cs%np
   ne = cs%ne
   nface = cs%nface
   ...
   allocate(res(ncols,nrows))
   res =-1
!
   do iface = 1,nface
   do je = 1,ne
   do ie = 1,ne
     ! print *,iface,ie,je,base(1,iface),base(2,iface)
     ic = base(1,iface)+ie
     jc = base(2,iface)+je
     !
...
!
   ...
   integer(i4), dimension(:,:, :,:), allocatable, intent(  out) :: res
! local variables
   integer(i4) :: np, ne, nface, nprocs, ncols, nrows, iface, ie, je, ic, jc, inbr
   integer(i4), dimension(:,:), allocatable :: base
!
   np = cs%np
   ne = cs%ne
...
! value
   if (allocated(res)) deallocate(res)
   allocate(res(n_nbr,nnbrs,ncols,nrows))
   res =-1
!
   do iface = 1,nface
   do je = 1,ne
