...
!
!-------------------------------------------------------------------------------
   subroutine nfc_initialize(nc, infilename, oufilename, compress, varnames)
   implicit none
   ...
   character(len=32), dimension(:), optional, intent(in   ) :: varnames
! local variables
   integer(i4)                     :: err
!
...
! variables
   do ii = 1,nc%nvars
     ...
     nc%vars(ii)%selected = selected(nc,trim(nam))
     !
     if (nc%vars(ii)%selected) then
...
! variables
   do ii = 1,nc%nvars
     if (nc%vars(ii)%selected) then
       ...
       call nfc_check(err1,'check get variable 1')
       call nfc_check(err2,'check put variable 2')
     endif
   enddo
!
