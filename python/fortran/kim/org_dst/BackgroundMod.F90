...
!-------------------------------------------------------------------------------
   use kiapsbase, only: int_kind=>kim_int_kind, real_kind=>kim_real8_kind,     &
    iulog=>kim_iu_log, len_path=>kim_string_len_path, len_arg=                 &
                                                            >kim_string_len_arg
   use dimensions, only: np, nlev, ne, nvar3d, nvar2d, nlevar, zwn, zwn2,      &
    zwn_nproc, zwn_iproc, zwn_a, zwn2_a, zwn_nproc_a, zwn_iproc_a, reg_nproc,  &
    reg_iproc, psi_nproc, psi_iproc, chi_nproc, chi_iproc, tps_nproc,          &
    tps_iproc, hum_nproc, hum_iproc, neig, neig_chi, neig_tps, neig_q, nelemd, &
                                        nelem, nets, nete, nk, ntime, nt_itime ! itime of nature
   ...
   use kiapswave, only: zwn_blength, zwn_bstart, psi_blength, psi_bstart,      &
    chi_blength, chi_bstart, tps_blength, tps_bstart, hum_blength, hum_bstart, &
                   map_zwn, map_reg, map_psi, map_chi, map_tps, map_hum, genmap
   ...
   use kiapsparallel, only: par=>kim_par, hybrid=>kim_hybrid,                  &
    par_double_precision, par_character, par_integer, global_shared_buf,       &
             global_shared_sum, barrierpar, par_max, par_min, abortpar, kim_par
...
!REAL(KIND=real_kind), DIMENSION(:,:,:,:,:,:), ALLOCATABLE, & ! extended to 4D
   ...
   real(real_kind), dimension(:,:), allocatable, public :: eigv, eigv_chi,     &
                         eigv_tps, eigv_q, eigv_a, bcov_sp_a, bg_eig, errvar_up
   real(real_kind), dimension(:), allocatable, public :: hyam, hybm, hyai,     &
                                              hybi, znu, znw, b_clim_v, b_ens_v
...
!
   exst_bg = .false.
   if (len_trim(bgfilelist)/= 0) inquire(file = trim(bgfilelist),exist =       &
                                                                       exst_bg)
   exst_ens = .false.
...
!.. Check zwn_nproc and zwn_iproc
   ...
   print*,'proc,zwn_nproc,begin,end:',par%iproc,zwn_nproc,zwn_iproc+1,         &
                                                            zwn_iproc+zwn_nproc
...
! 1. Allocate
   ...
   if (cv_opt_tu==1.or.cv_opt_tu==2.or.cv_opt_tu==3) then
     ...
     allocate(l_eigf_psi(psi_nproc),l_eigf_chi(chi_nproc),                     &
                                   l_eigf_tps(tps_nproc),l_eigf_hum(hum_nproc))
   endif
!=============================================================================
...
!=============================================================================
   exst_nt = .false.
   if (len_trim(naturefile)/= 0) inquire(file = trim(naturefile),exist =       &
                                                                       exst_nt)
   if (exst_nt) then
...
!   Calculate variance between background and nature (From level 11 to bottom)
       global_shared_buf(ie,2) = global_shared_buf(ie,2)+sum((bg_md(:,:,       &
                            11:nlev,ie,nt_itime)-nt_org(:,:,11:nlev,ie))**2.d0)
...
!   Calculate Variance of Background Error covariance (From level 11 to bottom)
       ...
       global_shared_buf(ie,9) = global_shared_buf(ie,9)+sum(errvar_md(:,:,11+ &
                                                             nlev*2:nlev*3,ie))
       global_shared_buf(ie,10) = global_shared_buf(ie,10)+sum(errvar_md(:,:,  &
                                                          11+nlev*3:nlev*4,ie))
...
!   vlenScale= 0.2D0 (default)
   ...
   5.d0/8.d0*(ratio**3.d0)+5.d0/3.d0*(ratio**2.d0)-5.d0*ratio+4.d0-            &
                                                                2.d0/3.d0/ratio
         else
           vloc(k,l) = 0.d0
         endif
...
! 2. Local variables
   ...
   real(real_kind), dimension(:,:,:), allocatable :: eigf_swn_psi,             &
                                       eigf_swn_chi, eigf_swn_tps, eigf_swn_hum
...
!   REAL(KIND=real_kind), DIMENSION(:), ALLOCATABLE &
!       :: zsfc_up, zsfc_ens
   real(real_kind), dimension(:,:), allocatable :: bg_md_up
...
!             pmid(i,j,k,ie,itime),              &
             call vertint(pmid_ens(i,:),pmid_up(i,k,itime),nlev,bg_smpl_tmp(i, &
                         k),bg_smpl_tmp(i,:),bg_smpl(i,k+nlev*(vr-1),itime,ic))
           enddo
         enddo
!END DO
!END DO
...
!                 bg_smpl(i,j,4*nlev+1,ie,itime,ic))
   ...
   tq_smpl_tmp(i,1,itime,ic),tq_smpl_tmp(i,2,itime,ic),zsfc_ens(i),ps_tmp(i),  &
...
!
   ...
   an_up(i,k+nlev*(vr-1)),an_up(i,1+nlev*(vr-1):nlev+nlev*(vr-1)),an_up_adj(i, &
                                                                k+nlev*(vr-1)))
         enddo
       enddo
     enddo
   ...
   an_up(i,3*nlev),an_up(i,4*nlev),zsfc_up(i),an_up(i,4*nlev+1),an_up_adj(i,   &
                                                                     4*nlev+1))
     enddo
...
!.. Check eigenfunctions
   ...
   call par_reduce(tmpmax,maxv,1,par_max)
   call par_reduce(tmpmin,minv,1,par_min)
   if (par%ismasterproc) write(iulog,*) 'Min & max [eigenfunction_psi]:',minv, &
                                                                           maxv
   tmpmax = maxval(l_eigf_chi)
   tmpmin = minval(l_eigf_chi)
   call par_reduce(tmpmax,maxv,1,par_max)
   call par_reduce(tmpmin,minv,1,par_min)
   if (par%ismasterproc) write(iulog,*) 'Min & max [eigenfunction_chi]:',minv, &
                                                                           maxv
   tmpmax = maxval(l_eigf_tps)
   tmpmin = minval(l_eigf_tps)
   call par_reduce(tmpmax,maxv,1,par_max)
   call par_reduce(tmpmin,minv,1,par_min)
   if (par%ismasterproc) write(iulog,*) 'Min & max [eigenfunction_tps]:',minv, &
                                                                           maxv
...
! Tu' = T' - Tb'
   tl_bg_ct(:,:,2*nlev+1:3*nlev,:) = tl_bg_md(:,:,2*nlev+1:3*nlev,:)-tl_var(:, &
                                                                    :,1:nlev,:)
! psu = ps - psb
   tl_bg_ct(:,:,nvar3d*nlev+1,:) = tl_bg_md(:,:,nvar3d*nlev+1,:)-tl_var(:,:,   &
                                                                      nlev+1,:)
     ...
     do j = 1, np
       do i = 1, np
         ...
         tl_phii(i,j,nlev,ie) = rgas*tl_tphi(i,j,nlev,ie)*hkl+rgas*tphi(i,j,   &
                                                                nlev,ie)*tl_hkl
         tl_phi(i,j,nlev,ie) = rgas*tl_tphi(i,j,nlev,ie)*hkk+rgas*tphi(i,j,    &
                                                                nlev,ie)*tl_hkk
       enddo
       do k = nlev-1, 2,-1
         do i = 1, np
           hkk = dp(i,j,k,ie)*0.5d0/p(i,j,k,ie)
           hkl = 2.d0*hkk
           tl_hkk = tl_dp(i,j,k,ie)*0.5d0/p(i,j,k,ie)-dp(i,j,k,                &
                                  ie)*0.5d0*tl_p(i,j,k,ie)/(p(i,j,k,ie)**2.d0) ! tl_hkl = 2.d0*tl_hkk
       ...
       do i = 1, np
         hkk = 0.5d0*dp(i,j,1,ie)/p(i,j,1,ie)
         tl_hkk = tl_dp(i,j,1,ie)*0.5d0/p(i,j,1,ie)-dp(i,j,1,ie)*0.5d0*tl_p(i, &
                                                   j,1,ie)/(p(i,j,1,ie)**2.d0) ! tl_phi(i,j,1,ie) = tl_phii(i,j,2,ie)+rgas*tl_tphi(i,j,1,ie)*hkk+rgas*tphi(i,j,1,ie)*tl_hkk
       enddo
     enddo
   enddo
! M = phi + RTv*ln(p)
