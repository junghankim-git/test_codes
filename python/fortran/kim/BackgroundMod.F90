...
!-------------------------------------------------------------------------------
   ...
   use timing, only:                                                           &
                                                             timingstart,      &
                                                             timingstop
   use ctrltospecmod, only:                                                    &
                                                             ctrltospec_opt,   &
                                                             lat_up,           &
                                                             norm_fac,         &
                                                             d_loc_h
   use kiapsparmpi, only:                                                      &
                                                             par_reduce
   use kiapsparallel, only:                                                    &
                                                             par=>kim_par,     &
                                                             hybrid=>kim_hybrid,&
   ...
   use kio, only:                                                              &
                                                             ...
                                                             getdimension,     &
                                                             getvariable
   use globalnorms, only:                                                      &
                                                             wrap_repro_sum
   use physicalconstants, only:                                                &
                                                             rvap=>kim_rvap,   &
                                                             rgas=>kim_rair,   &
                                                             ps0=>kim_p0,      &
                                                             gravi=>kim_g,     &
...
!             becFile                 ! Background error covariance file name
   character(len=len_path), dimension(:), allocatable, public ::               &
                                                             bgfile ! background file name
   character(len=len_path), public ::                                          &
                                                             naturefile,       &
                                                             becfile ! background error covariance file name
   character(len=len_arg), public ::                                           &
                                                             model_name,       &
                                                             bmethod ! method for background error covariance
   real(real_kind), parameter ::                                               &
                                                             tr = 270.d0
...
!             nt_org,                                   & ! Nature
!             errvar,                                                          &
                                              & ! variance of backgrouond error
!             z_org,                                                           &
                                         & ! Geopotential height for each layer
!             bg_md_q,                                                         &
                                           & ! Specific humidity for background
!             pmid,                                                            &
                                                      & ! Middle level pressure
!                                                                              &
...
!REAL(KIND=real_kind), DIMENSION(:,:,:,:,:,:), ALLOCATABLE, & ! extended to 4D
   real(real_kind), dimension(:,:,:,:), allocatable, public ::                 &
                                                             bg_smpl
   real(real_kind), dimension(:,:,:,:,:,:), allocatable, public ::             &
                                                             bg_uv_rot ! rotational wind in background field
   real(real_kind), dimension(:,:,:,:,:), allocatable, public ::               &
                                                             bg_md,            &
                                                             pmid
   real(real_kind), dimension(:,:,:,:), allocatable, public ::                 &
...
!END DO
       ...
       if (par%ismasterproc) write(iulog,*) "inflation factor for ensemble:",  &
                                                                       infl_ens
!up
!IF (par%ismasterproc) WRITE(iulog,*)  "bg_smpl_mean(1,1,:,1,:):",             &
                                                        bg_smpl_mean(1,1,:,1,:)
...
! Local variables
!
   real(real_kind), dimension(nlev+1) ::                                       &
                                                             pmid_avg,         &
                                                             tmp
   real(real_kind), dimension(nlev+1, nlev+1) ::                               &
                                                             vloc
   real(real_kind), dimension(:), allocatable ::                               &
                                                             work
   real(real_kind) ::                                                          &
...
!-------------------------------------------------------------------------------
   include 'mpif.h'
!
   real(real_kind), dimension(np, np, nlevar, nelemd), intent(inout) ::        &
                                                             errvar
...
! 1. Input and Output
!
   real(real_kind), dimension(psi_nproc), intent(in   ) ::                     &
                                                             l_eigf_psi
   real(real_kind), dimension(chi_nproc), intent(in   ) ::                     &
                                                             l_eigf_chi
   real(real_kind), dimension(tps_nproc), intent(in   ) ::                     &
                                                             l_eigf_tps
   real(real_kind), dimension(hum_nproc), intent(in   ) ::                     &
                                                             l_eigf_hum
! 2. Local variables
   real(real_kind), dimension(:,:,:), allocatable ::                           &
                                                             errcov_sp_psi,    &
...
!
   real(real_kind), dimension(nlev, 2*nlev+1, 180), intent(in   ) ::           &
                                                             reg_avg
   real(real_kind), dimension(reg_nproc), intent(  out) ::                     &
                                                             l_reg_avg
   real(real_kind), dimension(:), allocatable ::                               &
                                                             reg_tmp
   integer(int_kind) ::                                                        &
...
!-------------------------------------------------------------------------------
   include 'mpif.h'
!
   real(real_kind), dimension(reg_nproc), intent(in   ) ::                     &
                                                             l_reg_avg
   real(real_kind), dimension(nlev, nups_l, nk), intent(  out) ::              &
                                                             regress_up
   real(real_kind), dimension(:), allocatable ::                               &
                                                             reg_tmp
   real(real_kind), dimension(:,:,:), allocatable ::                           &
...
! 1-1. Input
!
   real(real_kind), dimension(np, np, nelemd), intent(in   ) ::                &
                                                             x
! 1-2. Output
   real(real_kind), dimension(np, np, nelemd), intent(  out) ::                &
                                                             x_reduced
! 1-3. Local
   integer(int_kind) ::                                                        &
                                                             l_ie,             &
...
!
   integer(int_kind), intent(in   ) ::                                         &
                                                             nsmpl
   character(len=len_path), dimension(nsmpl*ntime), intent(in   ) ::           &
                                                             infilelist
!up
!REAL(KIND=real_kind),DIMENSION(np,np,nlevar,nelemd,ntime,nsmpl), &
   real(real_kind), dimension(nups_l, nlevar, ntime, nsmpl), intent(  out) ::  &
                                                             bg_smpl
! Local variables
!up
!REAL(KIND=real_kind), DIMENSION(:,:,:), ALLOCATABLE &
...
!                            bg_smpl_tmp(i,j,k,ie), bg_smpl(i,j,k+nlev*(vr-1), &
                                                                   ie,itime,ic)
!             END DO
!             END DO
!             END DO
...
!-------------------------------------------------------------------------------
   ...
   use obscorrmod, only:                                                       &
                                                             sfcprscorr
!
   real(real_kind), dimension(np, np, nlevar, nelemd, ntime), intent(inout) :: &
                                                             an_ep
! Local variables
   real(real_kind), dimension(:,:), allocatable ::                             &
                                                             an_up,            &
...
!       IF (par%ismasterproc) WRITE(iulog,*) 'save  = ',bg_md(1,1,nlev*(vr-1)+ &
                                                    1:nlev*(vr-1)+nlev,1,itime)
   enddo
...
!
   ...
   real(real_kind), dimension(reg_nproc), intent(  out) ::                     &
                                                             l_reg_avg
   real(real_kind), dimension(psi_nproc), intent(  out) ::                     &
                                                             l_eigf_psi
   real(real_kind), dimension(chi_nproc), intent(  out) ::                     &
                                                             l_eigf_chi
   real(real_kind), dimension(tps_nproc), intent(  out) ::                     &
                                                             l_eigf_tps
   real(real_kind), dimension(hum_nproc), intent(  out) ::                     &
                                                             l_eigf_hum
   real(real_kind), dimension(np, np, nlevar, nelemd), intent(  out), optional :: &
                                                             errvar_md
! Local variables
   real(real_kind), dimension(:), allocatable ::                               &
                                                             w3d,              &
...
! 2-1. Input variables
!
   real(real_kind), dimension(np, np, nlev, nelemd), intent(in   ) ::          &
                                                             q,                &
                                                             t
! 2-2. Input/output variables
   real(real_kind), dimension(np, np, nlev, nelemd), intent(  out) ::          &
                                                             m
! 2-3. Local variables
   real(real_kind), dimension(np, np, nlev, nelemd) ::                         &
                                                             p,                &
                                                             dp,               &
                                                             phi,              &
                                                             phii,             &
...
! 1-1. Input variables
!
   real(real_kind), dimension(np, np, nlevar, nelemd), intent(in   ) ::        &
                                                             tl_bg_md,         &
                                                             gs_md
   real(real_kind), dimension(np, np, nlev, nelemd), intent(in   ) ::          &
                                                             tl_bg_chi
...
! 1-3. Local variables
   real(real_kind), dimension(np, np, nlevar, nelemd) ::                       &
                                                             md
   real(real_kind), dimension(np, np, nlev, 2, nelemd) ::                      &
                                                             tl_uv,            &
                                                             bg_uv
   real(real_kind), dimension(np, np, nlev, nelemd) ::                         &
                                                             tl_t,             &
                                                             ...
                                                             tl_rh,            &
                                                             bg_rh
   real(real_kind), dimension(np, np, nelemd) ::                               &
                                                             tl_ps,            &
                                                             bg_ps
   real(real_kind), dimension(np, np, nlev, nelemd) ::                         &
                                                             tl_ct
   real(real_kind), dimension(np, np, 2*nlev+1, nelemd) ::                     &
                                                             tl_var
   real(real_kind), dimension(nups_l, nlev) ::                                 &
                                                             tl_ct_up
   real(real_kind), dimension(nups_l, 2*nlev+1) ::                             &
                                                             tl_var_up
   real(real_kind), dimension(2*nlev+1) ::                                     &
                                                             tmp
   integer(int_kind) ::                                                        &
                                                             ie,               &
                                                             i,                &
...
!
   real(real_kind), dimension(np, np, nlevar, nelemd), intent(in   ) ::        &
                                                             tl_bg_md,         &
                                                             gs_md
   real(real_kind), dimension(np, np, nlev, 2, nelemd), intent(in   ) ::       &
                                                             bg_uv
! 1-2. Output variables
   real(real_kind), dimension(np, np, nlevar, nelemd), intent(  out) ::        &
                                                             tl_bg_ct
...
! 2-2. Output variables
   real(real_kind), dimension(np, np, nlev, 2, nelemd), intent(  out) ::       &
                                                             urot
! 2-3. Local variables
   type(edgebuffer_t) ::                                                       &
                                                             edge2
   real(real_kind), dimension(np, np, 2) ::                                    &
                                                             utmp
   integer(int_kind) ::                                                        &
                                                             i,                &
                                                             j,                &
...
! 2-3. Local variables
   ...
   real(real_kind), dimension(np, np, nlev, nelemd) ::                         &
                                                             b
   real(real_kind), dimension(np, np, nlev, 2) ::                              &
                                                             uv_ll
   integer(int_kind) ::                                                        &
                                                             i,                &
                                                             j,                &
...
! 2-2. Input/output variables
   real(real_kind), dimension(np, np, nlev, nelemd), intent(  out) ::          &
                                                             tl_phi
! 2-3. Local variables
   real(real_kind), dimension(np, np, nlev+1, nelemd) ::                       &
                                                             ph,               &
                                                             tl_ph
   real(real_kind), dimension(np, np, nlev, nelemd) ::                         &
                                                             p,                &
...
!
   real(real_kind), dimension(np, np, nlev, nelemd), intent(in   ) ::          &
                                                             b_org
! 2-2. Input/Output
   real(real_kind), dimension(np, np, nlev, nelemd), intent(inout) ::          &
                                                             x
! 2-3. Local
   type(edgebuffer_t) ::                                                       &
                                                             edge2
   real(real_kind), dimension(np, np, nlev, nelemd) ::                         &
                                                             ...
                                                             p,                &
                                                             b
   real(real_kind), dimension(:,:), pointer ::                                 &
                                                             viscosity=>null()
   real(real_kind) ::                                                          &
                                                             rsold,            &
                                                             rsnew,            &
...
!
   integer(int_kind), intent(in   ) ::                                         &
                                                             vert_lev
   real(real_kind), dimension(np, np, vert_lev, nelemd), intent(in   ) ::      &
                                                             b
   real(real_kind), dimension(np, np, vert_lev, nelemd), intent(  out) ::      &
                                                             b_inv
   real(real_kind), dimension(nups_l, vert_lev) ::                             &
                                                             b_up,             &
                                                             b_inv_up
   real(real_kind), dimension(vert_lev, zwn2) ::                               &
                                                             b_sp
   integer(int_kind) ::                                                        &
                                                             i,                &
                                                             j,                &
