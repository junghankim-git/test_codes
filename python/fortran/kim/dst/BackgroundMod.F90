...
!-------------------------------------------------------------------------------
   ...
   use dimensions, only: np, nlev, ne, nvar3d, nvar2d, nlevar, zwn, zwn2,      &
                zwn_nproc, zwn_iproc, zwn_a, zwn2_a, zwn_nproc_a, zwn_iproc_a, &
                reg_nproc, reg_iproc, psi_nproc, psi_iproc, chi_nproc,         &
                chi_iproc, tps_nproc, tps_iproc, hum_nproc, hum_iproc, neig,   &
                neig_chi, neig_tps, neig_q, nelemd, nelem, nets, nete, nk,     &
                                                               ntime, nt_itime ! itime of nature
   ...
   use kiapswave, only: zwn_blength, zwn_bstart, psi_blength, psi_bstart,      &
                chi_blength, chi_bstart, tps_blength, tps_bstart, hum_blength, &
                hum_bstart, map_zwn, map_reg, map_psi, map_chi, map_tps,       &
                                                                map_hum, genmap
   ...
   use kiapsparallel, only: par=>kim_par, hybrid=>kim_hybrid,                  &
                par_double_precision, par_character, par_integer,              &
                global_shared_buf, global_shared_sum, barrierpar, par_max,     &
                                                     par_min, abortpar, kim_par
...
!   Calculate variance between background and nature (From level 11 to bottom)
       ...
       global_shared_buf(ie,3) = global_shared_buf(ie,3)+sum((bg_md(:,:,11+    &
               nlev*1:nlev*2,ie,nt_itime)-nt_org(:,:,11+nlev*1:nlev*2,         &
                                                                    ie))**2.d0)
       global_shared_buf(ie,4) = global_shared_buf(ie,4)+sum((bg_md(:,:,11+    &
               nlev*2:nlev*3,ie,nt_itime)-nt_org(:,:,11+nlev*2:nlev*3,         &
                                                                    ie))**2.d0)
       global_shared_buf(ie,5) = global_shared_buf(ie,5)+sum((bg_md(:,:,11+    &
               nlev*3:nlev*4,ie,nt_itime)-nt_org(:,:,11+nlev*3:nlev*4,         &
                                                                    ie))**2.d0)
...
!                 bg_smpl(i,j,4*nlev+1,ie,itime,ic))
         call sfcprscorr(bg_md_up(i,3*nlev),bg_md_up(i,4*nlev),zsfc_up(i),     &
               tq_smpl_tmp(i,1,itime,ic),tq_smpl_tmp(i,2,itime,ic),zsfc_ens(i),&
                                        ps_tmp(i),bg_smpl(i,4*nlev+1,itime,ic))
!END DO
!END DO
       enddo
   ...
   do itime = 1,ntime
       ...
       do i = 1, nups_l
         do k = 1, nlev
           call vertint(pmid_up(i,:,itime),pmid_ens_mean(i,k,itime),nlev,      &
               an_up(i,k+nlev*(vr-1)),an_up(i,1+nlev*(vr-1):nlev+nlev*(vr-1)), &
     ...
     do i = 1,nups_l
       call sfcprscorr(t_ens_mean(i,itime),q_ens_mean(i,itime),zsfc_ens(i),    &
               an_up(i,3*nlev),an_up(i,4*nlev),zsfc_up(i),an_up(i,4*nlev+1),   &
