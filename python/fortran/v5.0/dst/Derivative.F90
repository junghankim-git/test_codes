...
!> @date 30JAN2015
!>  - JH KIM  : Added to "which_vlaplace", "hyper_scaling", "npdg", "nep", "Mfvm", "Cfvm", "Sfvm" variables and so on. (CAM-SE 5.3)
!-------------------------------------------------------------------------------
#include "KIM.h"
!-------------------------------------------------------------------------------
   module derivative
...
!
!-------------------------------------------------------------------------------
   use kiapsbase, only: real_kind=>kim_real8_kind, longdouble_kind=>kim_longdouble_kind
   use dimensions, only: np, nc, npdg, nep
   use quadrature, only: quadrature_t, gauss, gausslobatto, legendre, jacobi
...
! needed for spherical differential operators:
   ...
   use control, only: which_vlaplace, hypervis_scaling
   implicit none
!
   private
!
   type, public :: derivative_t
     real(real_kind) :: dvv(np, np)
     real(real_kind) :: dvv_diag(np, np)
     real(real_kind) :: dvv_twt(np, np)
     real(real_kind) :: mvv_twt(np, np) ! diagonal matrix of gll weights
     real(real_kind) :: vvtemp(np, np)
     real(real_kind) :: vvtempt(np, np, 2)
     real(real_kind) :: mfvm(np, nc+1)
     real(real_kind) :: cfvm(np, nc)
     real(real_kind) :: sfvm(np, nep)
     real(real_kind) :: legdg(np, np)
   end type derivative_t
!
   type, public :: derivative_stag_t
     real(real_kind) :: d(np, np)
     real(real_kind) :: m(np, np)
     real(real_kind) :: dpv(np, np)
     real(real_kind) :: d_twt(np, np)
     real(real_kind) :: m_twt(np, np)
     real(real_kind) :: m_t(np, np)
     real(real_kind) :: vtemp(np, np, 2)
     real(real_kind) :: vtempt(np, np, 2)
   end type derivative_stag_t
   real(real_kind), allocatable :: integration_matrix(:,:)
...
!    real (kind=longdouble_kind),optional :: phys_points(:)
   real(longdouble_kind), optional :: fvm_corners(nc+1)
   real(longdouble_kind), optional :: fvm_points(nc)
! Local variables
   type(quadrature_t) :: gp ! quadrature points and weights on pressure grid
   real(longdouble_kind) :: dmat(np, np)
   real(longdouble_kind) :: dpv(np, np)
   real(longdouble_kind) :: v2p(np, np)
   real(longdouble_kind) :: p2v(np, np)
   real(longdouble_kind) :: dvv(np, np)
   real(longdouble_kind) :: dvv_diag(np, np)
   real(longdouble_kind) :: v2v(np, np)
   real(longdouble_kind) :: xnorm
   integer i, j
! ============================================
...
! Legendre polynomials of degree npdg-1,on the np GLL grid:
   if (npdg>np) call abortpar('FATAL error:npdg>np')
...
! Local variables
   ...
   integer i, j
   real(longdouble_kind) fact, f1, f2
   real(longdouble_kind) func0, func1
   real(longdouble_kind) dis, c0, c1
   real(longdouble_kind) :: leg(np, np)
   real(longdouble_kind) :: jac(0:np-1)
   real(longdouble_kind) :: djac(0:np-1)
!
...
! Local variables
   ...
   integer i, j
   real(longdouble_kind) dis, c0, c1
   real(longdouble_kind) :: legv(0:np, np)
   real(longdouble_kind) :: dlegv(0:np, np)
   real(longdouble_kind) :: leg(0:np)
   real(longdouble_kind) :: dleg(0:np)
!
   c0 = 0.0_longdouble_kind
   c1 = 1.0_longdouble_kind
   gll = gausslobatto(np)
...
!
   integer :: n1, n2
   real(real_kind) :: v2p(n1, n2)
   real(real_kind) :: v2p_new(n1, n2)
   real(longdouble_kind) :: gll(n1), gs(n2)
! Local variables
   integer i, j, k, m, l
   real(longdouble_kind) fact, f1, sum
   real(longdouble_kind) func0, func1
   real(longdouble_kind) :: leg(n1, n1)
   real(longdouble_kind) :: jac(0:n1-1)
   real(longdouble_kind) :: djac(0:n1-1)
   real(longdouble_kind) :: c0, c1
   type(quadrature_t) :: gll_pts
   real(longdouble_kind) :: leg_out(n1, n2)
   real(longdouble_kind) :: gamma(n1)
!
   c0 = 0.0_longdouble_kind
   c1 = 1.0_longdouble_kind
! ==============================================================
...
!
   real(real_kind), intent(in   ) :: v(np, np, 2)
   type(derivative_stag_t) :: deriv
   real(real_kind) :: div(np, np)
! Local
   integer i
   integer j
   integer l
   logical, parameter :: useunroll = .true.
   real(real_kind) sumx00, sumx01
   real(real_kind) sumy00, sumy01
   real(real_kind) sumx10, sumx11
   real(real_kind) sumy10, sumy11
#ifdef DEBUG
!
...
! Local
   ...
   integer l
   logical, parameter :: useunroll = .true.
   real(real_kind) :: dudx00, dudx01
   real(real_kind) :: dudx10, dudx11
   real(real_kind) :: dvdy00, dvdy01
   real(real_kind) :: dvdy10, dvdy11
!
...
!
   type(derivative_stag_t) :: deriv
   real(real_kind), intent(in   ) :: p(np, np)
   real(real_kind) :: dp(np, np, 2)
! Local
   integer i
   integer j
   integer l
...
!
   type(derivative_t) :: deriv
   real(real_kind), intent(in   ) :: s(np, np)
   real(real_kind) :: ds(np, np, 2)
   integer i
   integer j
   integer l
   logical, parameter :: useunroll = .true.
   real(real_kind) :: dsdx00, dsdx01
   real(real_kind) :: dsdx10, dsdx11
   real(real_kind) :: dsdy00, dsdy01
   real(real_kind) :: dsdy10, dsdy11
#ifdef DEBUG
!
...
!
   type(derivative_t) :: deriv
   real(real_kind), intent(in   ) :: v(np, np, 2)
   real(real_kind) :: vort(np, np)
   integer i
   integer j
   integer l
   logical, parameter :: useunroll = .true.
   real(real_kind) :: dvdx00, dvdx01
   real(real_kind) :: dvdx10, dvdx11
   real(real_kind) :: dudy00, dudy01
   real(real_kind) :: dudy10, dudy11
!
   if (modulo(np, 2)==0.and.useunroll) then
     do j = 1, np, 2
       do l = 1, np, 2
...
!
   real(real_kind), intent(in   ) :: v(np, np)
   type(derivative_t) :: deriv
   real(real_kind) :: p(nc, nc)
! Local
   integer i
   integer j
   integer l
...
!
   real(real_kind), intent(in   ) :: v(np, np)
   type(derivative_t) :: deriv
   real(real_kind) :: p(nep, nep)
! Local
   integer i, j, l
   real(real_kind) sumx00, sumx01
   real(real_kind) sumx10, sumx11
   real(real_kind) vtemp(np, nep)
!
   do j = 1, np
     do l = 1, nep
       sumx00 = 0.0d0
...
!
   integer :: nphys
   real(real_kind), intent(in   ) :: pin(nphys*nphys)
   real(real_kind) :: pout(np, np)
! Local
   integer, save :: nphys_init = 0
   integer, save :: nintersect
   real(real_kind), save, pointer :: acell(:) ! arrivial cell index of i'th intersection
   real(real_kind), save, pointer :: dcell(:) ! departure cell index of i'th intersection
   real(real_kind), save, pointer :: delta(:) ! length of i'th intersection
   real(real_kind), save, pointer :: delta_a(:) ! length of arrival cells
   integer in_i, in_j, ia, ja, id, jd, count, i, j
   logical :: found
   real(real_kind) :: tol = 1e-13
   real(real_kind) :: weight, x1, x2, dx
   real(longdouble_kind) :: gll_edges(np+1), phys_edges(nphys+1)
   type(quadrature_t) :: gll_pts
...
! find point x2 closet to x1 and x2>x1:
       ...
       enddo
       if (x2>1+tol) call abortpar('ERROR:did not find next intersection point')
       count = count+1
       delta(count) = x2-x1
       found = .false.
       do ia = 1,np
       ...
       enddo
       if (.not.found) call abortpar('ERROR:interval search problem')
       found = .false.
       do id = 1,nphys
       ...
       x1 = x2
     enddo
     if (count>nintersect) call abortpar('ERROR:nintersect was too small')
     nintersect = count
#if 0
     ...
     do in_i = 1,nintersect
       ia = acell(in_i)
       id = dcell(in_i)
       write(*,'(i3,a,2f10.6,a,a,2f10.6,a,a,2f10.6,a) ') in_i,'[',x1,x1+delta(in_i),']','[',gll_edges(ia),gll_edges(ia+1),']','[',phys_edges(id),phys_edges(id+1),']'
       x1 = x1+delta(in_i)
     enddo
     pout = 0
     do in_i = 1,nintersect
...
!
   type(derivative_t) :: deriv
   real(real_kind), intent(in   ), dimension(2, 2, np, np) :: dinv
   real(real_kind), intent(in   ) :: s(np, np)
   real(real_kind) :: ds(np, np, 2)
   integer i
   integer j
   integer l
   real(real_kind) :: dsdx00
   real(real_kind) :: dsdy00
   real(real_kind) :: v1(np, np), v2(np, np)
!
   do j = 1, np
     do l = 1, np
       dsdx00 = 0.0d0
...
!
   ...
   real(real_kind) :: ds(np, np, 2)
   integer i, j, l, m, n
   real(real_kind) :: dscontra(np, np, 2)
!
   dscontra = 0
   do n = 1,np
     do m = 1, np
...
!
   ...
   do n = 1,np
     do m = 1, np
       do j = 1, np
         dscontra(m,n,1) = dscontra(m,n,1)-((elem%mp(j,n)*elem%metinv(1,1,m,n)*elem%metdet(m,n)*s(j,n)*deriv%dvv(m,j))+(elem%mp(m,j)*elem%metinv(2,1,m,n)*elem%metdet(m,n)*s(m,j)*deriv%dvv(n,j)))*rrearth
         dscontra(m,n,2) = dscontra(m,n,2)-((elem%mp(j,n)*elem%metinv(1,2,m,n)*elem%metdet(m,n)*s(j,n)*deriv%dvv(m,j))+(elem%mp(m,j)*elem%metinv(2,2,m,n)*elem%metdet(m,n)*s(m,j)*deriv%dvv(n,j)))*rrearth
       enddo
       enddo
   enddo
! convert contra -> latlon
...
!
   ...
   real(real_kind) :: ds(np, np, 2)
   integer i, j, l, m, n
   real(real_kind) :: dscov(np, np, 2)
! debug:
   real(real_kind) :: vcontra(np, np, 2)
   real(real_kind) :: v(np, np, 2)
   real(real_kind) :: div(np, np)
!
   dscov = 0
   do n = 1,np
...
!
   type(derivative_t) :: deriv
   type(element_t) :: elem
   real(real_kind), intent(in   ) :: u(np, np, 2)
   real(real_kind), intent(in   ) :: v(np, np, 2)
   real(real_kind) :: ugradv(np, np, 2)
   real(real_kind) :: dum_cart(np, np, 3)
   integer :: component
! latlon -> cartesian
!
   do component = 1, 3
...
! Dot u with the gradient of each component
     dum_cart(:,:,component) = sum(u(:,:,:)*gradient_sphere(dum_cart(:,:,component),deriv,elem%dinv),3)
   enddo
! cartesian -> latlon
!
   do component = 1, 2
...
!
!
   real(real_kind), intent(in   ) :: v(np, np, 2) ! in lat-lon coordinates
   type(derivative_t) :: deriv
   type(element_t) :: elem
   real(real_kind) :: div(np, np)
! Local
   integer i, j, m, n
   real(real_kind) :: vtemp(np, np, 2)
   real(real_kind) :: ggtemp(np, np, 2)
   real(real_kind) :: gtemp(np, np, 2)
   real(real_kind) :: psi(np, np)
   real(real_kind) :: xtmp
! latlon- > contra
!
   do j = 1, np
...
!
   ...
   type(derivative_t) :: deriv
   type(element_t) :: elem
   real(real_kind) :: result(np, np)
! Local
   real(real_kind) :: ucontra(np, np, 2) ! in lat-lon coordinates
   integer i, j
! latlon->contra
!
   do j = 1, np
...
!
   real(real_kind), intent(in   ) :: v(np, np, 2)
   real(real_kind), intent(in   ) :: p(np, np)
   real(real_kind), intent(in   ) :: pedges(0:np+1, 0:np+1)
   type(derivative_t) :: deriv
   type(element_t) :: elem
   real(real_kind) :: result(np, np)
   logical :: u_is_contra
...
!
   ...
   integer j
   integer l
   real(real_kind) :: dvdx00
   real(real_kind) :: dudy00
   real(real_kind) :: vco(np, np, 2)
   real(real_kind) :: vtemp(np, np)
...
! Local
   ...
   integer j
   integer l
   real(real_kind) :: dudx00
   real(real_kind) :: dvdy00
   real(real_kind) :: gv(np, np, 2), vvtemp(np, np)
...
!
   ...
   type(derivative_t) :: deriv
   type(element_t) :: elem
   real(real_kind) :: laplace(np, np)
   real(real_kind) :: laplace2(np, np)
   integer i, j
! Local
   real(real_kind) :: grads(np, np, 2), oldgrads(np, np, 2)
!
...
!
   ...
   type(derivative_t) :: deriv
   type(element_t) :: elem
   real(real_kind), optional :: nu_ratio
   real(real_kind) :: laplace(np, np, 2)
!
   if (which_vlaplace.eq.2) then
     laplace = cartesian_laplace_sphere_wk(v,deriv,elem,var_coef,nu_ratio)
   elseif (which_vlaplace.eq.1) then
...
! Local
   integer i, j, l, m, n
   real(real_kind) :: vor(np, np), div(np, np)
   real(real_kind) :: v1, v2, div1, div2, vor1, vor2, phi_x, phi_y
!
   div = divergence_sphere(v,deriv,elem)
...
#else
         vor1 = vor1-elem%mp(m,l)*vor(m,l)*elem%met(1,1,m,l)*phi_y+elem%mp(l,n)*vor(l,n)*elem%met(2,1,l,n)*phi_x
         vor2 = vor2-elem%mp(m,l)*vor(m,l)*elem%met(1,2,m,l)*phi_y+elem%mp(l,n)*vor(l,n)*elem%met(2,2,l,n)*phi_x
#endif
       enddo
#ifdef DIVCONTRA_VORCO
       v1 =-div1
...
!
   real(real_kind), intent(in   ) :: p(np, np)
   type(derivative_t) :: deriv
   real(real_kind) :: phat(npdg, npdg)
! Local
   integer i, j, m, n
   real(real_kind) :: a(np, npdg)
!
   a = 0
   phat = 0
...
!
!
   real(real_kind) :: p(np, np)
   type(derivative_t) :: deriv
   real(real_kind) :: phat(npdg, npdg)
! Local
   integer i, j, m, n
   real(real_kind) :: a(npdg, np)
!
   p(:,:) = 0
...
!
   ...
   real(real_kind), intent(in   ) :: sampled_val(np, np)
   real(real_kind), intent(in   ) :: metdet(np, np)
   real(real_kind) :: values(intervals, intervals)
   real(real_kind) :: v(np, np)
   integer i, j
!
   v = sampled_val*metdet
   if (.not.allocated(integration_matrix).or.size(integration_matrix,1).ne.intervals.or.size(integration_matrix,2).ne.np) then
...
!
   ...
   integer, intent(in   ) :: intervals
   real(real_kind) :: values(intervals, intervals)
   real(real_kind), parameter :: zero = 0.0d0, one = 1.0d0, two = 2.0d0
   real(real_kind) :: sub_gll(intervals, np)
   real(real_kind) :: lagrange_interp(intervals, np, np)
   type(quadrature_t) :: gll
   real(real_kind) :: legrange_div(np)
   real(real_kind) :: a, b, x, y, x_j, x_i
   real(real_kind) :: r(1)
   integer i, j, n, m
!
