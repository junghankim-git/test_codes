...
'''
    ...
    def apply_continuation(self, itype, line):
        ...
        if self.var_cont and (itype == 3 or itype == 4):
            ...
            if head[-1] == '&':
                ...
                headofhead, head = self.split_comment(head)
        ...
        # normal continuation
            ...
            comment = ''
            if line.count('\n') > 1:
                comment, line = self.split_comment(line[:-1])
                line = line + '\n'

            if len(line) > self.max_width + 1:
                # processing head
                idx = self.find_idx_near_max(line)
                before = self.add_cont_line(line[:idx+1])
                after  = line[idx+1:]
                after  = (self.max_width-len(after))*' ' + after
                new_line = comment + before + '\n' + after
            else:
    ...
    def add_cont_line(self, line):
        new_line = line+' '*(self.max_width-len(line)-1)+'&'
        return new_line
...
class F90Converter:
    ...
    def __init__(self, infilepath, oufilepath, var_cont=False, extension=[], exclude=[]):
        ...
        self.grammer    = F90Grammer('/home/jhkim/work/python/fortran/v7.0/fortran.dat', var_cont)
