...
!-------------------------------------------------------------------------------
  ...
  iulog=>kim_iu_log, len_path=>kim_string_len_path, len_arg=>kim_string_len_arg
   use dimensions, only: np, nlev, ne, nvar3d, nvar2d, nlevar, zwn, zwn2,      &
 zwn_nproc, zwn_iproc, zwn_a, zwn2_a, zwn_nproc_a, zwn_iproc_a, reg_nproc, reg_iproc, psi_nproc, psi_iproc, chi_nproc, chi_iproc, tps_nproc, tps_iproc, hum_nproc, hum_iproc, neig, neig_chi, neig_tps, neig_q, nelemd, nelem, nets, nete, nk, ntime, nt_itime ! itime of nature
   use kiapsgrid, only: elem, deriv, nups_l, nups_g, neps_l, neps_g, dofs,     &
                dofs_ep, convertep2up_hyb, convertup2ep_hyb, convertup2ep_hyb_l
   use kiapswave, only: zwn_blength, zwn_bstart, psi_blength, psi_bstart,      &
 chi_blength, chi_bstart, tps_blength, tps_bstart, hum_blength, hum_bstart, map_zwn, map_reg, map_psi, map_chi, map_tps, map_hum, genmap
 ...
 par_double_precision, par_character, par_integer, global_shared_buf, global_shared_sum, barrierpar, par_max, par_min, abortpar, kim_par
   use kio, only: kio_t, iniiosystem, finiosystem, openfile, closefile,        &
                                                      getdimension, getvariable
   use globalnorms, only: wrap_repro_sum
   use physicalconstants, only: rvap=>kim_rvap, rgas=>kim_rair, ps0=>kim_p0,   &
                                                gravi=>kim_g, dd_pi, kim_rearth
!  USE BECOutputMod,      ONLY : InitBECOutput,  &
!                                WriteBECOutput, &
!                                FinBECOutput
   use bectuoutputmod, only: initbectuoutput, writebectuoutput, finbectuoutput
   use edge, only: edgebuffer_t, initedgebuffer, freeedgebuffer, edgevpack,    &
                                                                    edgevunpack
...
!
   private
   character(len=len_path), dimension(:),                                      &
                                     allocatable :: listfile ! sample file list
...
!             becFile                 ! Background error covariance file name
   ...
   character(len=len_path), public :: naturefile,                              &
                                becfile ! background error covariance file name
   character(len=len_arg), public :: model_name,                               &
                               bmethod ! method for background error covariance
   real(real_kind), parameter :: tr = 270.d0
   integer(int_kind), public :: cv_opt_wind, cv_opt_hum, cv_opt_tu, cv_opt_psu,&
                                                                 ens_opt_center
...
!REAL(KIND=real_kind), DIMENSION(:,:,:,:,:,:), ALLOCATABLE, & ! extended to 4D
   ...
   real(real_kind), dimension(:,:,:,:), allocatable, public :: nt_org, errvar, &
                                z_org, bg_md_q, pint ! interface level pressure
   real(real_kind), dimension(:,:,:), allocatable, public :: bcov_sp,          &
                        bcov_sp_chi, bcov_sp_tps, bcov_sp_q, phis, zsfc, rl_org
   real(real_kind), dimension(:,:,:,:), allocatable, public :: u10m, v10m, t2m,&
                                                             q2m, tsfc, sfctype
   real(real_kind), dimension(:,:), allocatable, public :: eigv, eigv_chi,     &
                                       eigv_tps, eigv_q, eigv_a, bcov_sp_a, !up
!
   bg_eig,errvar_up
   real(real_kind), dimension(:), allocatable, public :: hyam, hybm, hyai, hybi,&
                                                    znu, znw, b_clim_v, b_ens_v
   real(real_kind), public :: cgtol_il, rescalepsi, rescalechi, rescalet,      &
                                                            rescaleq, rescaleps
   ...
   real(real_kind), dimension(:,:,:), allocatable, public :: pmid_up,          &
                                                                  pmid_ens_mean
   real(real_kind), dimension(:,:), allocatable, public :: t_ens_mean,         &
                                                                     q_ens_mean
...
!
   exst_bg = .false.
   if (len_trim(bgfilelist)/= 0) inquire(file = trim(bgfilelist),              &
                                                               exist = exst_bg)
   exst_ens = .false.
   ...
   if (bmethod=='h4dev'.or.bmethod=='3dvar'.or.bmethod=='4dclv') then !--      &
                                                              for hybrid or nmc
     if (exst_bg) then
!.. Open a netcdf file of background
       call openfile(io_t,trim(bgfile(nt_itime)))
       if (par%ismasterproc) print*,'Open ',trim(bgfile(nt_itime)),            &
                                                 ' to set vertical coordinates'
     else
...
!.. Open a netcdf file of one of the samples
       ...
       if (par%ismasterproc) print*,'Open ',trim(listfile(1)),                 &
                                                 ' to set vertical coordinates'
     endif
   else !-----------for ens
     allocate(listfile(nsmpl))
...
!.. Check zwn_nproc and zwn_iproc
   ...
   print*,'proc,zwn_nproc,begin,end:',par%iproc,zwn_nproc,zwn_iproc+1,zwn_iproc+&
                                                                      zwn_nproc
   if (par%ismasterproc) print*,'Number of wave coefficients for alpha',zwn2_a
   print*,'proc,zwn_nproc_a,begin,end:',par%iproc,zwn_nproc_a,zwn_iproc_a+1,   &
                                                        zwn_iproc_a+zwn_nproc_a
...
! 2. Local variables
!
   real(real_kind), dimension(:,:,:,:,:), allocatable :: bg_smpl_q, bg_smpl_p, &
                                                      bg_smpl_chi, bg_smpl_pint
   real(real_kind), dimension(:,:,:,:,:), allocatable :: !bg_smpl_mean,        &
                              bg_smpl_q_mean, bg_smpl_p_mean, bg_smpl_pint_mean
   real(real_kind), dimension(:,:,:,:), allocatable :: errvar_md, bg_smpl_ep
   real(real_kind), dimension(:,:,:,:), allocatable :: bg_ct
   real(real_kind), dimension(:,:,:), allocatable :: bg_sp_smpl, reg_avg,      &
                                                                   bg_smpl_mean
   real(real_kind), dimension(:,:), allocatable :: bg_sp, bg_md_up
   real(real_kind), dimension(:), allocatable :: l_reg_avg, l_eigf_psi,        &
                                             l_eigf_chi, l_eigf_tps, l_eigf_hum
   real(real_kind) :: psfact, distr
   real(real_kind) :: tmpmax, tmpmin, maxv, minv
   integer(int_kind) :: i, j, k, l, ie, ig, ks, ismpl, wn, vr, iproc, vertnum, &
                                                              icnt, ierr, itime
!=============================================================================
!
...
!.. Allocate  variables
   ...
   elseif (model_name=='kim-sw'.or.model_name=='kim-                           &
                                   sh') then ! read background for kim-sw or-sh
...
! 1. Allocate
   ...
   if (cv_opt_tu==1.or.cv_opt_tu==2.or.cv_opt_tu==3) then
     ...
     allocate(l_eigf_psi(psi_nproc),l_eigf_chi(chi_nproc),l_eigf_tps(tps_nproc),&
...
!   Read background error covariance
     call readbackerrcov(l_reg_avg,l_eigf_psi,l_eigf_chi,l_eigf_tps,l_eigf_hum,&
                                                                     errvar_md)
...
!                             + bg_smpl(i,j,k,ie,itime,ismpl)
                 bg_smpl_mean(i,k,itime) = bg_smpl_mean(i,k,itime)+bg_smpl(i,k,&
                                                                   itime,ismpl)
               enddo
...
!up
         ...
         if (par%ismasterproc) print*,                                         &
                              'Wrong option for the center of ensemble samples'
       endif
...
!END DO
         ...
         enddo
       enddo ! itime
       if (par%ismasterproc) write(iulog,*) "inflation factor for ensemble:",  &
                                                                       infl_ens
!up
!IF (par%ismasterproc) WRITE(iulog,*)  "bg_smpl_mean(1,1,:,1,:):",bg_smpl_mean(1,1,:,1,:)
       if (par%ismasterproc) write(iulog,*) "bg_smpl_mean(1,:,:):",            &
                                                            bg_smpl_mean(1,:,:)
...
!.. Model space (U,V,T,Q) -> controal varible space
       ...
       do ismpl = 1,nsmpl
         do itime = 1, ntime
           if (par%ismasterproc) write(iulog,'(a,i4,                           &
                                a) ') 'To control variable(ismpl = ',ismpl,') '
!CALL ModelToCtrl(bg_smpl(:,:,:,:,itime,ismpl),    &
           call convertup2ep_hyb(nlevar,bg_smpl(:,:,itime,ismpl),bg_smpl_ep)
           call modeltoctrl(bg_smpl_ep(:,:,:,:),bg_md(:,:,:,:,itime),          &
                                              bg_uv_rot(:,:,:,:,:,itime),bg_ct)
!up
!bg_smpl(:,:,:,:,itime,ismpl) = bg_ct(:,:,:,:)
     ...
     if (par%ismasterproc) write(iulog,                                        &
                                    *) "you need to generate climatological b."
   endif !! exst_bec
!=============================================================================
   call mpi_barrier(par%comm,ierr)
...
!.. psi
   if (rescalepsi/= 1.d0) then
     ...
     if (par%ismasterproc) write(iulog,*) 'Psi variance was rescaled,ratio = ',&
                                                                     rescalepsi
   endif
!.. Unbalanced chi
   if (rescalechi/= 1.d0) then
     ...
     if (errvar_md(1, 1, 1, 1)/= 0.d0) then
       do ie = 1, nelemd
         errvar_md(:,:,nlev+1:nlev*2,ie) = errvar_md(:,:,nlev+1:nlev*2,        &
                                                                 ie)*rescalechi
       enddo
     endif
     if (par%ismasterproc) write(iulog,*) 'Chi variance was rescaled,ratio = ',&
                                                                     rescalechi
   endif
!.. Unbalanced T
   if (rescalet/= 1.d0) then
     ...
     if (errvar_md(1, 1, 1, 1)/= 0.d0) then
       do ie = 1, nelemd
         errvar_md(:,:,nlev*2+1:nlev*3,ie) = errvar_md(:,:,nlev*2+1:nlev*3,    &
                                                                   ie)*rescalet
       enddo
     endif
     if (par%ismasterproc) write(iulog,*) 'T variance was rescaled,ratio = ',  &
                                                                       rescalet
   endif
!.. Unbalanced Q
   if (rescaleq/= 1.d0) then
     ...
     if (errvar_md(1, 1, 1, 1)/= 0.d0) then
       do ie = 1, nelemd
         errvar_md(:,:,nlev*3+1:nlev*4,ie) = errvar_md(:,:,nlev*3+1:nlev*4,    &
                                                                   ie)*rescaleq
       enddo
     endif
     if (par%ismasterproc) write(iulog,*) 'Q variance was rescaled,ratio = ',  &
                                                                       rescaleq
   endif
!.. Unbalanced Ps
   if (rescaleps/= 1.d0) then
     ...
     if (par%ismasterproc) write(iulog,*) 'Ps variance was rescaled,ratio = ', &
                                                                      rescaleps
   endif
...
!=============================================================================
   exst_nt = .false.
   if (len_trim(naturefile)/= 0) inquire(file = trim(naturefile),              &
                                                               exist = exst_nt)
   if (exst_nt) then
...
!   Calculate variance between background and nature (From level 11 to bottom)
       global_shared_buf(ie,2) = global_shared_buf(ie,2)+sum((bg_md(:,:,11:nlev,&
                                    ie,nt_itime)-nt_org(:,:,11:nlev,ie))**2.d0)
       global_shared_buf(ie,3) = global_shared_buf(ie,3)+sum((bg_md(:,:,11+    &
             nlev*1:nlev*2,ie,nt_itime)-nt_org(:,:,11+nlev*1:nlev*2,ie))**2.d0)
       global_shared_buf(ie,4) = global_shared_buf(ie,4)+sum((bg_md(:,:,11+    &
             nlev*2:nlev*3,ie,nt_itime)-nt_org(:,:,11+nlev*2:nlev*3,ie))**2.d0)
       global_shared_buf(ie,5) = global_shared_buf(ie,5)+sum((bg_md(:,:,11+    &
             nlev*3:nlev*4,ie,nt_itime)-nt_org(:,:,11+nlev*3:nlev*4,ie))**2.d0)
       global_shared_buf(ie,6) = global_shared_buf(ie,6)+sum((bg_md(:,:,1+     &
                            nlev*4,ie,nt_itime)-nt_org(:,:,1+nlev*4,ie))**2.d0)
!   Calculate Variance of Background Error covariance (From level 11 to bottom)
       global_shared_buf(ie,7) = global_shared_buf(ie,7)+sum(errvar_md(:,:,    &
                                                                   11:nlev,ie))
       global_shared_buf(ie,8) = global_shared_buf(ie,8)+sum(errvar_md(:,:,11+ &
                                                             nlev*1:nlev*2,ie))
       global_shared_buf(ie,9) = global_shared_buf(ie,9)+sum(errvar_md(:,:,11+ &
                                                             nlev*2:nlev*3,ie))
       global_shared_buf(ie,10) = global_shared_buf(ie,10)+sum(errvar_md(:,:,11+&
                                                             nlev*3:nlev*4,ie))
       global_shared_buf(ie,11) = global_shared_buf(ie,11)+sum(errvar_md(:,:,1+&
                                                                    nlev*4,ie))
     enddo
     call mpi_barrier(par%comm,ierr)
     call wrap_repro_sum(nvars = 11,comm = hybrid%par%comm)
     if (par%ismasterproc) write(iulog,                                        &
                                    *) 'Variance between background and nature'
     if (par%ismasterproc) write(iulog,*) 'vari_bg_u = ',                      &
                                    global_shared_sum(2)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_bg_v = ',                      &
                                    global_shared_sum(3)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_bg_T = ',                      &
                                    global_shared_sum(4)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_bg_q = ',                      &
                                    global_shared_sum(5)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_bg_ps = ',                     &
                                       global_shared_sum(6)/dble(np*np*nelem*1)
     if (par%ismasterproc) write(iulog,                                        &
                                   *) 'Variance of background error covariance'
     if (par%ismasterproc) write(iulog,*) 'vari_B_u = ',                       &
                                    global_shared_sum(7)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_B_v = ',                       &
                                    global_shared_sum(8)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_B_T = ',                       &
                                    global_shared_sum(9)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_B_q = ',                       &
                                   global_shared_sum(10)/dble(np*np*nelem*nlev)
     if (par%ismasterproc) write(iulog,*) 'vari_B_ps = ',                      &
                                      global_shared_sum(11)/dble(np*np*nelem*1)
   endif ! exst_nt
!=============================================================================
...
!
   global_shared_buf(:,1) = 0.d0
   do ie = 1,nelemd
     global_shared_buf(ie,1) = global_shared_buf(ie,1)+sum(bg_md(:,:,4*nlev+1, &
                                                                         ie,1))
   enddo
   call mpi_barrier(par%comm,ierr)
   call wrap_repro_sum(nvars = 1,comm = hybrid%par%comm)
   ...
   do l = 1,nlev+1
     do k = 1, nlev+1
       ...
       if (dist.le.dist_zero2) then
         vloc(k,l) =-1.d0/4.d0*(ratio**5.d0)+1.d0/2.d0*(ratio**4.d0)+          &
                           5.d0/8.d0*(ratio**3.d0)-5.d0/3.d0*(ratio**2.d0)+1.d0
       else
         if (dist.le.2.d0*dist_zero2) then
           vloc(k,l) = 1.d0/12.d0*(ratio**5.d0)-1.d0/2.d0*(ratio**4.d0)+       &
5.d0/8.d0*(ratio**3.d0)+5.d0/3.d0*(ratio**2.d0)-5.d0*ratio+4.d0-2.d0/3.d0/ratio
         else
           vloc(k,l) = 0.d0
         endif
       endif
...
!   Eigendecomposition and make eigen-pairs for alpha
   ...
   do k = 1,nlev+1
     ...
     do wn = 1,zwn_nproc_a
       eigv_a(k,wn) = tmp(nlev+1-k+1)*norm_fac*dexp(-dint(dsqrt(dble(wn+       &
                                          zwn_iproc_a-1)))**2.d0/d_loc_h**2.d0)
     enddo
   enddo
   if (par%ismasterproc) then
     do k = 1, nlev+1
...
!
   ...
   real(real_kind), dimension(:,:), allocatable :: errvar_sum, errvar_avg,     &
                                                                      errvar_up
...
! Conduct zonal-averaging of variance
   do iup = 1,nups_l
     do ilat = 1, 180
       if (lat_up(iup, 1).ge.dble(ilat-91)/180.d0*dd_pi.and.lat_up(iup,        &
                                         1).lt.dble(ilat-90)/180.d0*dd_pi) then
   ...
   errvar_avg = 0.d0
   n_avg = 0
   call mpi_allreduce(errvar_sum,errvar_avg,nlevar*180,par_double_precision,   &
...
! 2. Local variables
   real(real_kind), dimension(:,:,:), allocatable :: errcov_sp_psi,            &
                                    errcov_sp_chi, errcov_sp_tps, errcov_sp_hum
   real(real_kind), dimension(:,:,:), allocatable :: eigf_swn_psi, eigf_swn_chi,&
                                                     eigf_swn_tps, eigf_swn_hum
   real(real_kind), dimension(:), allocatable :: eigf_psi1d, eigf_chi1d,       &
                                                         eigf_tps1d, eigf_hum1d
...
!
   ...
   call mpi_allgatherv(l_eigf_psi,psi_nproc,par_double_precision,eigf_psi1d,   &
                     psi_blength,psi_bstart,par_double_precision,par%comm,ierr)
   call mpi_allgatherv(l_eigf_chi,chi_nproc,par_double_precision,eigf_chi1d,   &
                     chi_blength,chi_bstart,par_double_precision,par%comm,ierr)
   call mpi_allgatherv(l_eigf_tps,tps_nproc,par_double_precision,eigf_tps1d,   &
                     tps_blength,tps_bstart,par_double_precision,par%comm,ierr)
   call mpi_allgatherv(l_eigf_hum,hum_nproc,par_double_precision,eigf_hum1d,   &
                     hum_blength,hum_bstart,par_double_precision,par%comm,ierr)
! -> Spherical wavenumber structure
   allocate(eigf_swn_psi(nlev,neig,zwn),eigf_swn_chi(nlev,neig_chi,zwn),       &
               eigf_swn_tps(nlev+1,neig_tps,zwn),eigf_swn_hum(nlev,neig_q,zwn))
   eigf_swn_psi = 0.d0
   eigf_swn_chi = 0.d0
   eigf_swn_tps = 0.d0
   eigf_swn_hum = 0.d0
...
!
   ...
   call mpi_allgatherv(l_reg_avg,reg_nproc,par_double_precision,reg_tmp,       &
                     reg_blength,reg_bstart,par_double_precision,par%comm,ierr)
...
!REAL(KIND=real_kind),DIMENSION(np,np,nlevar,nelemd,ntime,nsmpl), &
   real(real_kind), dimension(nups_l, nlevar, ntime, nsmpl),                   &
                                                       intent(  out) :: bg_smpl
! Local variables
!up
!REAL(KIND=real_kind), DIMENSION(:,:,:), ALLOCATABLE &
...
!IF (tmp .LE. 0 .OR. tmp .EQ. nUPs_g) THEN
       if (tmp.le.0) then
         if (par%ismasterproc) print*,'Check dimension(ncol) or ep file:',     &
                                           trim(infilelist(ic+nsmpl*(itime-1)))
         call abortpar()
       endif
!CALL GetDimension(io_t,'ncol',tmp,DOFs_EP)
       call getdimension(io_t,'ncol',tmp,dofs)
       if (par%ismasterproc) print*,'Read samples from ',trim(infilelist(ic+   &
                                                              nsmpl*(itime-1)))
...
!..    Read Specific humidity (kg/kg) and calculate the mean
           ...
           if (is_moist_mr) then
             do k = 1, nlev
               do i = 1, nups_l
                 ups_2d(i,k) = ups_2d(i,k)/(1.d0+ups_2d(i,                     &
                                                     k)) ! to specific humidity
               enddo
             enddo
           endif
         end select
...
!             pmid(i,j,k,ie,itime),              &
             call vertint(pmid_ens(i,:),! pmid_up(i,k),pmid_up(i,k,itime),nlev,&
                                         !bg_md(i,j,k+nlev*(vr-1),ie,itime),!up
!bg_smpl_tmp(i,j,k,ie),            &
!bg_smpl_tmp(i,j,:,ie),            &
!bg_smpl(i,j,k+nlev*(vr-1),ie,itime,ic))
             bg_smpl_tmp(i,k),bg_smpl_tmp(i,:),bg_smpl(i,k+nlev*(vr-1),itime,  &
                                                                           ic))
           enddo
         enddo
!END DO
...
!                 bg_smpl(i,j,4*nlev+1,ie,itime,ic))
         call sfcprscorr(bg_md_up(i,3*nlev),bg_md_up(i,4*nlev),zsfc_up(i),     &
!bg_smpl(i,3*nlev,itime,ic),!bg_smpl(i,4*nlev,itime,ic),tq_smpl_tmp(i,1,itime,ic),tq_smpl_tmp(i,2,itime,ic),zsfc_ens(i),ps_tmp(i),bg_smpl(i,4*nlev+1,itime,ic))
!END DO
!END DO
       enddo
       t_ens_mean(:,itime) = t_ens_mean(:,itime)+tq_smpl_tmp(:,1,itime,        &
                                                                ic)/dble(nsmpl)
       q_ens_mean(:,itime) = q_ens_mean(:,itime)+tq_smpl_tmp(:,2,itime,        &
                                                                ic)/dble(nsmpl)
! Checking adjusted ps
       if (itime.eq.1.and.ic.eq.1) then
         if (par%ismasterproc) print*,'zsfc','zsfc_ens','bg_md_ps','ps_ens',   &
                                                                   'bg_smpl_ps'
!up
!DO ie = 1,nelemd
!DO j  = 1,np
!DO i  = 1,np
         do i = 1,nups_l
           if (dabs(zsfc_up(i)-zsfc_ens(i)).gt.500.d0) then
             print*,zsfc_up(i),zsfc_ens(i),bg_md_up(i,4*nlev+1),ps_tmp(i),     &
...
!
   ...
   do itime = 1,ntime
     ...
     do vr = 1,nvar3d
       do i = 1, nups_l
         do k = 1, nlev
           call vertint(pmid_up(i,:,itime),pmid_ens_mean(i,k,itime),nlev,      &
an_up(i,k+nlev*(vr-1)),an_up(i,1+nlev*(vr-1):nlev+nlev*(vr-1)),an_up_adj(i,k+nlev*(vr-1)))
         enddo
       enddo
     enddo
     do i = 1,nups_l
       call sfcprscorr(t_ens_mean(i,itime),q_ens_mean(i,itime),zsfc_ens(i),    &
an_up(i,3*nlev),an_up(i,4*nlev),zsfc_up(i),an_up(i,4*nlev+1),an_up_adj(i,4*nlev+1))
     enddo
...
!
   ...
   type(kio_t) :: io_t
   real(real_kind), dimension(:,:), allocatable :: ups_1d, eps_1d
   real(real_kind), dimension(:,:), allocatable :: ups_2d, ups_2d_p1, eps_2d,  &
                                                                      eps_2d_p1
   real(real_kind), dimension(:,:,:,:), allocatable :: bg_ct
   real(real_kind) :: psfact
...
!IF (tmp .LE. 0 .OR. tmp .EQ. nUPs_g) THEN
   if (tmp.le.0) then
     if (par%ismasterproc) print*,'Check dimension or ep file:',               &
                                                            trim(bgfile(itime))
     call abortpar()
   endif
   call getdimension(io_t,'ncol',tmp,dofs)
...
!bg_md(i,j,nlev+1-k+nlev*(vr-1),ie,itime) = x3d(icnt)
   ...
   do vr = 1,nvar2d
     ...
     psfact = 1.d0
     if (eps_1d(1,1)<2000.) psfact = 1.d+2 ! if unit of ps is hpa,             &
                                                                convert into pa
...
!pint(i,j,nlev+2-k,ie) = x3di(icnt)
     ...
     do k = 1, nlev
       do ie = 1, nelemd
         do j = 1, np
           do i = 1, np
             pmid(i,j,k,ie,itime) = hyam(k)*ps0+hybm(k)*bg_md(i,j,nlevar,ie,   &
                                                                         itime)
...
!z_org(i,j,nlev+1-k,ie) = x3d(icnt)
   ...
   phis = zsfc*gravi
   if (par%ismasterproc) write(iulog,*) 'pmid(1,1,:,1,itime):',pmid(1,1,:,1,   &
                                                                         itime)
...
!
!-------------------------------------------------------------------------------
   subroutine readbackerrcov(l_reg_avg, l_eigf_psi, l_eigf_chi, l_eigf_tps,    &
                                                         l_eigf_hum, errvar_md)
!-------------------------------------------------------------------------------
   use netcdf
!
   type(kio_t) :: io_t
   ...
   real(real_kind), dimension(np, np, nlevar, nelemd), intent(  out),          &
                                                          optional :: errvar_md
! Local variables
   real(real_kind), dimension(:), allocatable :: w3d, w3d_chi, w3d_tps, w3d_q, &
                                                              x2d, c3d, reg_tmp
   real(real_kind), dimension(:,:), allocatable :: t3d, w2d, w2d_chi, w2d_tps, &
                                                                     w2d_q, x3d
   real(real_kind) :: psfact
...
!
   ...
   if (status/= 0) then
     if (par%ismasterproc) write(iulog,*) trim(becfile),                       &
                                 "no variance of model variables in backerrcov"
     exst_varmd = .false.
   else
     exst_varmd = .true.
   endif
...
!PRINT *,'map reg 1 = ',map_reg(1)
   if (par%ismasterproc) write(iulog,*) 'File of background error covariance:',&
                                                                        becfile
...
! 1-1. Input variables
!
   real(real_kind), dimension(np, np, nlevar, nelemd),                         &
...
! 1-3. Local variables
   ...
   real(real_kind), dimension(np, np, nlev, nelemd) :: tl_t, tl_q, tl_m, bg_t, &
                                                                   tl_rh, bg_rh
...
! Tu' = T' - Tb'
   tl_bg_ct(:,:,2*nlev+1:3*nlev,:) = tl_bg_md(:,:,2*nlev+1:3*nlev,:)-tl_var(:,:,&
                                                                      1:nlev,:)
! psu = ps - psb
   tl_bg_ct(:,:,nvar3d*nlev+1,:) = tl_bg_md(:,:,nvar3d*nlev+1,:)-!tl_ct(:,:,   &
                                                                        nlev,:)
   tl_var(:,:,nlev+1,:)
...
!    CALL CalChi(tl_uv,&
!                tl_ct)
   tl_bg_ct(:,:,nlev+1:2*nlev,:) = tl_bg_chi(:,:,:,:)-tl_var(:,:,nlev+2:2*nlev+&
                                                                           1,:)
!=============================================================================
   return
!
   ...
   do ie = nets,nete
     do k = 1, nlev
       do j = 1, np
         ...
         enddo
       enddo
       b(:,:,k,ie) = vorticity_sphere_wk(uv_ll(:,:,k,:),deriv(hybrid%ithr),    &
                                                                      elem(ie))
     enddo
   ...
   do ie = nets,nete
     do k = 1, nlev
       do j = 1, np
         ...
         enddo
       enddo
       b(:,:,k,ie) = divergence_sphere_wk(uv_ll(:,:,k,:),deriv(hybrid%ithr),   &
                                                                      elem(ie))
     enddo
     call edgevpack(edge2,b(:,:,:,ie),nlev,0,elem(ie)%desc)
   enddo
   ...
   do ie = nets,nete
     do k = 1, nlev
       ...
       do j = 1, np
         ...
         enddo
       enddo
       b(:,:,k,ie) = divergence_sphere_wk(ucor(:,:,k,:),deriv(hybrid%ithr),    &
                                                                      elem(ie))
     enddo
...
!
   ...
   do ie = nets,nete
     ...
     do j = 1, np
       ...
       enddo
     enddo
     b(:,:,nlev,ie) = divergence_sphere_wk(ucor(:,:,nlev,:),deriv(hybrid%ithr),&
                                                                      elem(ie))
     ...
     do k = nlev,nlev
       do comp = 1, 2
         ...
         gradvb = gradient_sphere(uvb(:,:,k,comp,ie),deriv(hybrid%ithr),       &
                                                                 elem(ie)%dinv)
         ugradv(:,:,comp) = uvb(:,:,k,1,ie)*gradv(:,:,1)+uvb(:,:,k,2,          &
                                                               ie)*gradv(:,:,2)
         ugradvb(:,:,comp) = uv(:,:,k,1,ie)*gradvb(:,:,1)+uv(:,:,k,2,          &
                                                              ie)*gradvb(:,:,2)
       enddo
...
!
   ...
   real(real_kind), dimension(np, np, nlev, nelemd), intent(in   ) :: q, tl_q, &
                                                                        t, tl_t
...
! 2-3. Local variables
   ...
   real(real_kind), dimension(np, np, nlev, nelemd) :: p, dp, phi, phii, tl_p, &
                                                        tl_dp, tl_phii, tl_tphi
   ...
   do ie = nets,nete
     do k = 1, nlev
       do j = 1, np
         do i = 1, np
           tphi(i,j,k,ie) = t(i,j,k,ie)*(1.d0+(rvap/rgas-1.d0)*q(i,j,k,ie))
           tl_tphi(i,j,k,ie) = tl_t(i,j,k,ie)+tl_t(i,j,k,ie)*q(i,j,k,          &
               ie)*(rvap/rgas-1.d0)+t(i,j,k,ie)*tl_q(i,j,k,ie)*(rvap/rgas-1.d0)
         enddo
       enddo
     enddo
   enddo
   ...
   do ie = nets,nete
     do j = 1, np
       do i = 1, np
         hkk = dp(i,j,nlev,ie)*0.5d0/p(i,j,nlev,ie)
         hkl = 2.d0*hkk
         tl_hkk = tl_dp(i,j,nlev,ie)*0.5d0/p(i,j,nlev,ie)-dp(i,j,nlev,         &
                             ie)*0.5d0*tl_p(i,j,nlev,ie)/(p(i,j,nlev,ie)**2.d0)
         tl_hkl = 2.d0*tl_hkk
         tl_phii(i,j,nlev,ie) = rgas*tl_tphi(i,j,nlev,ie)*hkl+rgas*tphi(i,j,   &
                                                                nlev,ie)*tl_hkl
         tl_phi(i,j,nlev,ie) = rgas*tl_tphi(i,j,nlev,ie)*hkk+rgas*tphi(i,j,nlev,&
                                                                     ie)*tl_hkk
       enddo
       do k = nlev-1, 2,-1
         do i = 1, np
           hkk = dp(i,j,k,ie)*0.5d0/p(i,j,k,ie)
           hkl = 2.d0*hkk
           tl_hkk = tl_dp(i,j,k,ie)*0.5d0/p(i,j,k,ie)-dp(i,j,k,ie)*0.5d0*tl_p(i,&
                             j,k,ie)/(p(i,j,k,ie)**2.d0) ! tl_hkl = 2.d0*tl_hkk
           tl_phii(i,j,k,ie) = tl_phii(i,j,k+1,ie)+rgas*tl_tphi(i,j,k,ie)*hkl+ &
                                                     rgas*tphi(i,j,k,ie)*tl_hkl
           tl_phi(i,j,k,ie) = tl_phii(i,j,k+1,ie)+rgas*tl_tphi(i,j,k,ie)*hkk+  &
                                                     rgas*tphi(i,j,k,ie)*tl_hkk
         enddo
       enddo
       do i = 1, np
         hkk = 0.5d0*dp(i,j,1,ie)/p(i,j,1,ie)
         tl_hkk = tl_dp(i,j,1,ie)*0.5d0/p(i,j,1,ie)-dp(i,j,1,ie)*0.5d0*tl_p(i,j,&
1,ie)/(p(i,j,1,ie)**2.d0) ! tl_phi(i,j,1,ie) = tl_phii(i,j,2,ie)+rgas*tl_tphi(i,j,1,ie)*hkk+rgas*tphi(i,j,1,ie)*tl_hkk
       enddo
     enddo
   enddo
! M = phi + RTv*ln(p)
...
!phi(i,j,k,ie) = phi(i,j,k,ie) + rgas*Tr*DLOG(p(i,j,k,ie))
           tl_phi(i,j,k,ie) = tl_phi(i,j,k,ie)+rgas*tr*tl_p(i,j,k,ie)/p(i,j,k, &
                                                                            ie)
         enddo
       enddo
     enddo
