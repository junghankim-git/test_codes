...
!
!-------------------------------------------------------------------------------
   use kiapsbase, only: int_kind=>kim_int_kind, real_kind=>kim_real8_kind,     &
                                                              iulog=>kim_iu_log
   use dimensions, only: np, nlev, nelemd, nets, nete, nvar3d, nvar2d, nlevar, &
 zwn2, zwn2_a, neig, neig_chi, neig_tps, neig_q, zwn_nproc, zwn_iproc, zwn_nproc_a, zwn_iproc_a, ntime, nt_itime ! itime for nature
   use kiapsparallel, only: par=>kim_par, hybrid=>kim_hybrid, global_shared_buf,&
 global_shared_sum, par_double_precision, par_character, par_sum, par_max, par_min
   use kiapsparallel, only: barrierpar
   use kiapsparmpi, only: par_reduce
   use kiapswave, only: zwn_blength, zwn_bstart, zwn_blength_a,                &
...
!errvar,                &
!
   errvar_up,bg_eig,bcov_sp,eigv,bcov_sp_chi,eigv_chi,bcov_sp_tps,eigv_tps,    &
bcov_sp_q,eigv_q,bcov_sp_a,eigv_a,nsmpl,cv_opt_hum,is_moist_mr,exst_nt,bg_smpl,b_clim_v,b_ens_v
   use observationmod, only: obsmet, sdmiss, obcheck, obcheck_outer,           &
 crit_omb_amsua, crit_omb_gpsro, crit_omb_iasi, crit_omb_cris, crit_omb_atms, crit_omb_atmswv, crit_omb_mhs, crit_omb_csr, singleobs, da_sonde, da_surface, da_bogus, da_aircraft, da_amv, da_scatwind, da_amsua, da_gpsro, da_iasi, da_cris, da_atms, da_atmswv, da_mhs, da_csr, rttovused, roppused
   ...
   use makebmod, only: makebsingle, makebsonde, makebsurface, makebaircraft,   &
 makebamv, makebscatwind, makebamsua, makebgpsro, makebiasi, makebcris, makebatms, makebmhs, makebcsr, makebbogus, makebatmswv
   use multiplyrmod, only: multiplyrsingle, multiplyrsonde, multiplyrsurface,  &
 multiplyraircraft, multiplyramv, multiplyrscatwind, multiplyramsua, multiplyrgpsro, multiplyriasi, multiplyrcris, multiplyratms, multiplyrmhs, multiplyrcsr, multiplyrbogus, multiplyratmswv
!  USE ModelToEigMod,  ONLY : TlmModelToEig
   use kiapsgrid, only: nups_l, convertup2ep_hyb, convertep2up_hyb,            &
                                                              convertep2ep0_hyb
! 2. Common variables
   implicit none
!
...
! 2. Public variables
   ...
   type(obsbg_type), dimension(:), allocatable, public :: gs_obs, ad_obs,      &
                                                         tl_obs, bg_obs, an_obs
   type(obs_type), dimension(:), allocatable,                                  &
                     public :: obs_org ! for observation check every outer-loop
   real(real_kind), dimension(:,:), allocatable, public :: x_eig_acc,          &
                                      x_eig_acc_chi, x_eig_acc_tps, x_eig_acc_q
   real(real_kind), dimension(:,:,:), allocatable,                             &
...
!
   ...
   real(real_kind), dimension(nlev+1, zwn_nproc_a,                             &
                                         nsmpl) :: x_eig_a ! for alpha-variable
   integer(int_kind) :: i, j, k, kk, ie, wn, vr, iter, ioutl, iproc, ierr, err,&
                                                                          itime
!=============================================================================
! Extended to 4D
!! ALLOCATE Analysis
!
   ...
   allocate(gs_obs(ntime),tl_obs(ntime),ad_obs(ntime),bg_obs(ntime),           &
                                                  an_obs(ntime),obs_org(ntime))
   do itime = 1,ntime
!! ALLOCATE observation information
     ...
     if (da_amsua) then
       allocate(gs_obs(itime)%amsua(obsmet(itime)%namsua_e_max,                &
                                                obsmet(itime)%namsuach,nelemd))
       allocate(tl_obs(itime)%amsua(obsmet(itime)%namsua_e_max,                &
                                                obsmet(itime)%namsuach,nelemd))
       allocate(ad_obs(itime)%amsua(obsmet(itime)%namsua_e_max,                &
                                                obsmet(itime)%namsuach,nelemd))
       allocate(bg_obs(itime)%amsua(obsmet(itime)%namsua_e_max,                &
                                                obsmet(itime)%namsuach,nelemd))
       allocate(an_obs(itime)%amsua(obsmet(itime)%namsua_e_max,                &
                                                obsmet(itime)%namsuach,nelemd))
       allocate(obs_org(itime)%amsua(obsmet(itime)%namsua_e_max,               &
                                                obsmet(itime)%namsuach,nelemd))
     ...
     if (da_iasi) then
       allocate(gs_obs(itime)%iasi(obsmet(itime)%niasi_e_max,                  &
                                                 obsmet(itime)%niasich,nelemd))
       allocate(tl_obs(itime)%iasi(obsmet(itime)%niasi_e_max,                  &
                                                 obsmet(itime)%niasich,nelemd))
       allocate(ad_obs(itime)%iasi(obsmet(itime)%niasi_e_max,                  &
                                                 obsmet(itime)%niasich,nelemd))
       allocate(bg_obs(itime)%iasi(obsmet(itime)%niasi_e_max,                  &
                                                 obsmet(itime)%niasich,nelemd))
       allocate(an_obs(itime)%iasi(obsmet(itime)%niasi_e_max,                  &
                                                 obsmet(itime)%niasich,nelemd))
       allocate(obs_org(itime)%iasi(obsmet(itime)%niasi_e_max,                 &
                                                 obsmet(itime)%niasich,nelemd))
       obs_org(itime)%iasi(:,:,:) = obs(itime)%iasi(:,:,:)
     endif
     if (da_cris) then
       allocate(gs_obs(itime)%cris(obsmet(itime)%ncris_e_max,                  &
                                                 obsmet(itime)%ncrisch,nelemd))
       allocate(tl_obs(itime)%cris(obsmet(itime)%ncris_e_max,                  &
                                                 obsmet(itime)%ncrisch,nelemd))
       allocate(ad_obs(itime)%cris(obsmet(itime)%ncris_e_max,                  &
                                                 obsmet(itime)%ncrisch,nelemd))
       allocate(bg_obs(itime)%cris(obsmet(itime)%ncris_e_max,                  &
                                                 obsmet(itime)%ncrisch,nelemd))
       allocate(an_obs(itime)%cris(obsmet(itime)%ncris_e_max,                  &
                                                 obsmet(itime)%ncrisch,nelemd))
       allocate(obs_org(itime)%cris(obsmet(itime)%ncris_e_max,                 &
                                                 obsmet(itime)%ncrisch,nelemd))
       obs_org(itime)%cris(:,:,:) = obs(itime)%cris(:,:,:)
     endif
     if (da_atms) then
       allocate(gs_obs(itime)%atms(obsmet(itime)%natms_e_max,                  &
                                                 obsmet(itime)%natmsch,nelemd))
       allocate(tl_obs(itime)%atms(obsmet(itime)%natms_e_max,                  &
                                                 obsmet(itime)%natmsch,nelemd))
       allocate(ad_obs(itime)%atms(obsmet(itime)%natms_e_max,                  &
                                                 obsmet(itime)%natmsch,nelemd))
       allocate(bg_obs(itime)%atms(obsmet(itime)%natms_e_max,                  &
                                                 obsmet(itime)%natmsch,nelemd))
       allocate(an_obs(itime)%atms(obsmet(itime)%natms_e_max,                  &
                                                 obsmet(itime)%natmsch,nelemd))
       allocate(obs_org(itime)%atms(obsmet(itime)%natms_e_max,                 &
                                                 obsmet(itime)%natmsch,nelemd))
       obs_org(itime)%atms(:,:,:) = obs(itime)%atms(:,:,:)
     endif
     if (da_atmswv) then
       allocate(gs_obs(itime)%atmswv(obsmet(itime)%natmswv_e_max,              &
                                               obsmet(itime)%natmswvch,nelemd))
       allocate(tl_obs(itime)%atmswv(obsmet(itime)%natmswv_e_max,              &
                                               obsmet(itime)%natmswvch,nelemd))
       allocate(ad_obs(itime)%atmswv(obsmet(itime)%natmswv_e_max,              &
                                               obsmet(itime)%natmswvch,nelemd))
       allocate(bg_obs(itime)%atmswv(obsmet(itime)%natmswv_e_max,              &
                                               obsmet(itime)%natmswvch,nelemd))
       allocate(an_obs(itime)%atmswv(obsmet(itime)%natmswv_e_max,              &
                                               obsmet(itime)%natmswvch,nelemd))
       allocate(obs_org(itime)%atmswv(obsmet(itime)%natmswv_e_max,             &
                                               obsmet(itime)%natmswvch,nelemd))
       obs_org(itime)%atmswv(:,:,:) = obs(itime)%atmswv(:,:,:)
     endif
     if (da_mhs) then
       allocate(gs_obs(itime)%mhs(obsmet(itime)%nmhs_e_max,obsmet(itime)%nmhsch,&
                                                                       nelemd))
       allocate(tl_obs(itime)%mhs(obsmet(itime)%nmhs_e_max,obsmet(itime)%nmhsch,&
                                                                       nelemd))
       allocate(ad_obs(itime)%mhs(obsmet(itime)%nmhs_e_max,obsmet(itime)%nmhsch,&
                                                                       nelemd))
       allocate(bg_obs(itime)%mhs(obsmet(itime)%nmhs_e_max,obsmet(itime)%nmhsch,&
                                                                       nelemd))
       allocate(an_obs(itime)%mhs(obsmet(itime)%nmhs_e_max,obsmet(itime)%nmhsch,&
                                                                       nelemd))
       allocate(obs_org(itime)%mhs(obsmet(itime)%nmhs_e_max,                   &
                                                  obsmet(itime)%nmhsch,nelemd))
       obs_org(itime)%mhs(:,:,:) = obs(itime)%mhs(:,:,:)
     endif
     if (da_csr) then
       allocate(gs_obs(itime)%csr(obsmet(itime)%ncsr_e_max,obsmet(itime)%ncsrch,&
                                                                       nelemd))
       allocate(tl_obs(itime)%csr(obsmet(itime)%ncsr_e_max,obsmet(itime)%ncsrch,&
                                                                       nelemd))
       allocate(ad_obs(itime)%csr(obsmet(itime)%ncsr_e_max,obsmet(itime)%ncsrch,&
                                                                       nelemd))
       allocate(bg_obs(itime)%csr(obsmet(itime)%ncsr_e_max,obsmet(itime)%ncsrch,&
                                                                       nelemd))
       allocate(an_obs(itime)%csr(obsmet(itime)%ncsr_e_max,obsmet(itime)%ncsrch,&
                                                                       nelemd))
       allocate(obs_org(itime)%csr(obsmet(itime)%ncsr_e_max,                   &
                                                  obsmet(itime)%ncsrch,nelemd))
...
! A. RCG
!==========================================================================
     if (optmethod.eq.1) call rcgmethod(ioutl,gs_md,x_eig,x_eig_chi,x_eig_tps, &
                                                       x_eig_q,x_eig_a) ! alpha
!==========================================================================
!==========================================================================
...
!==========================================================================
     ...
     call tlmeigtomodel(gs_md(:,:,:,:,:),x_eig,x_eig_chi,x_eig_tps,x_eig_q,    &
                                                                  x_eig_a,x_md)
     ...
     do itime = 1, ntime
       do ie = 1, nelemd
         do k = 1, nlev
           do j = 1, np
             do i = 1, np
               bg_md(i,j,k+3*nlev,ie,itime) = bg_md(i,j,k+3*nlev,ie,           &
                   itime)/(1.d0-bg_md(i,j,k+3*nlev,ie,itime)) ! to mixing ratio
               an_md(i,j,k+3*nlev,ie,itime) = an_md(i,j,k+3*nlev,ie,           &
                   itime)/(1.d0-an_md(i,j,k+3*nlev,ie,itime)) ! to mixing ratio
             enddo
           enddo
         enddo
       enddo
...
!
!-------------------------------------------------------------------------------
   subroutine rcgmethod(ioutl, gs_md, x_eig, x_eig_chi, x_eig_tps, x_eig_q,    &
                                                      x_eig_a) ! for alpha(hjs)
! 2. Variables
! 2-1. Output
!
   integer(int_kind), intent(in   ) :: ioutl
   real(real_kind), dimension(np, np, nlevar, nelemd, ntime),                  &
                                                         intent(in   ) :: gs_md
   ...
   real(real_kind), dimension(nlev+1, zwn_nproc_a, nsmpl),                     &
                                                       intent(inout) :: x_eig_a
! 2-2. Local
   real(real_kind), dimension(neig, zwn_nproc) :: b_eig, r_eig, p_eig, ap_eig
   real(real_kind), dimension(neig_chi, zwn_nproc) :: b_eig_chi, r_eig_chi,    &
                                                          p_eig_chi, ap_eig_chi
   real(real_kind), dimension(neig_tps, zwn_nproc) :: b_eig_tps, r_eig_tps,    &
                                                          p_eig_tps, ap_eig_tps
   real(real_kind), dimension(neig_q, zwn_nproc) :: b_eig_q, r_eig_q, p_eig_q, &
                                                                       ap_eig_q
   real(real_kind), dimension(nlev+1, zwn_nproc_a, nsmpl) :: b_eig_a, r_eig_a, &
                                                              p_eig_a, ap_eig_a
   real(real_kind), dimension(neig, zwn_nproc, niter) :: r_eig_history ! for psi
   real(real_kind), dimension(neig_chi, zwn_nproc,                             &
                                          niter) :: r_eig_chi_history ! for chi
   real(real_kind), dimension(neig_tps, zwn_nproc,                             &
                                       niter) :: r_eig_tps_history ! for(t, ps)
   real(real_kind), dimension(neig_q, zwn_nproc,                               &
                                              niter) :: r_eig_q_history ! for q
   real(real_kind), dimension(nlev+1, zwn_nproc_a, nsmpl,                      &
                                           niter) :: r_eig_q_history_a !--alpha
   real(real_kind) :: rsold, rsnew, rs0, pap, alpha, beta, j_b, j_o, tmp
   integer(int_kind) :: i, j, k, kk, ie, wn, vr, iter, iproc, ierr, err, itime,&
                                                                          ismpl
   real(real_kind) :: rjrk
   real(real_kind) :: tmpmax, tmpmin, maxv, minv
   integer(int_kind) :: info, nlanczos, nl, nw
   real(real_kind), dimension(niter) :: lanczos_diag! diagonal part of niter-  &
                                         dimensional lanczos tridiagonal matrix
   real(real_kind), dimension(niter-1) :: lanczos_subd! sub-                   &
                  diagonal part of niter-dimensional lanczos tridiagonal matrix
   real(real_kind) :: alphaold, betaold, gamma, eps, ortho
   real(real_kind), dimension(niter, niter) :: lanczos_orth! sub-              &
                  diagonal part of niter-dimensional lanczos tridiagonal matrix
   real(real_kind), dimension(2*niter-2) :: work
   real(real_kind), allocatable, dimension(:,:,                                &
...
!==========================================================================
   ...
   x_eig_q = 0.d0
   x_eig_a = 0.d0
   call make_rhs_b(gs_md,b_eig,b_eig_chi,b_eig_tps,b_eig_q,b_eig_a)
...
!==========================================================================
   ...
   call make_lhs_ax(gs_md,x_eig,x_eig_chi,x_eig_tps,x_eig_q,x_eig_a,ap_eig,    &
                                       ap_eig_chi,ap_eig_tps,ap_eig_q,ap_eig_a)
...
! 2. Initialize rsold
   ...
   call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,r_eig,&
r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,r_eig,r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,rsold)
...
!       IF (par%ismasterproc) WRITE(iulog,*)  x_eig(:,1)
     ...
     call par_reduce(tmpmax,maxv,1,par_max)
     call par_reduce(tmpmin,minv,1,par_min)
     if (par%ismasterproc) write(iulog,*) 'Min tmpmax = maxval(x_eig_chi)
     tmpmin = minval(x_eig_chi)
     call par_reduce(tmpmax,maxv,1,par_max)
     call par_reduce(tmpmin,minv,1,par_min)
     if (par%ismasterproc) write(iulog,*) 'Min tmpmax = maxval(x_eig_tps)
     tmpmin = minval(x_eig_tps)
     call par_reduce(tmpmax,maxv,1,par_max)
     call par_reduce(tmpmin,minv,1,par_min)
     if (par%ismasterproc) write(iulog,*) 'Min tmpmax = maxval(x_eig_q)
     tmpmin = minval(x_eig_q)
     call par_reduce(tmpmax,maxv,1,par_max)
     call par_reduce(tmpmin,minv,1,par_min)
     if (par%ismasterproc) write(iulog,*) 'Min tmpmax = maxval(x_eig_a)
     tmpmin = minval(x_eig_a)
     call par_reduce(tmpmax,maxv,1,par_max)
     call par_reduce(tmpmin,minv,1,par_min)
     if (par%ismasterproc) write(iulog,*) 'Min ! if (iter==1.or.mod(iter,      &
                                                                 10).eq.0) then
! 1. Calculate Cost Function
       ierr = timingstart('Cal_CostFun') !---profiling
       call cal_cost_func(gs_md,x_eig,x_eig_chi,x_eig_tps,x_eig_q,x_eig_a,j_o, &
                                                                           j_b)
       ierr = timingstop('Cal_CostFun')
!        ENDIF
! 2. Calculate Ap ------------------------------------------------------
       ierr = timingstart('Make_LHS_Ax') !---profiling
       call make_lhs_ax(gs_md,p_eig,p_eig_chi,p_eig_tps,p_eig_q,p_eig_a,ap_eig,&
                                       ap_eig_chi,ap_eig_tps,ap_eig_q,ap_eig_a)
...
! 3-1. Calculate alpha,x,r,and rsnew
       ...
       call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,  &
p_eig,p_eig_chi,p_eig_tps,p_eig_q,p_eig_a,ap_eig,ap_eig_chi,ap_eig_tps,ap_eig_q,ap_eig_a,pap)
...
! r_j : current  residual
         ...
         call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,&
r_eig,r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,r_eig_history(:,:,k),r_eig_chi_history(:,:,k),r_eig_tps_history(:,:,k),r_eig_q_history(:,:,k),r_eig_q_history_a(:,:,:,k),rjrk)
         ...
         do ismpl = 1,nsmpl
           do wn = 1, zwn_nproc_a
             r_eig_a(:,wn,ismpl) = r_eig_a(:,wn,ismpl)-rjrk*r_eig_q_history_a(:,&
                                                                    wn,ismpl,k)
           enddo
         enddo ! ismpl
       enddo
       ...
       call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,  &
r_eig,r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,r_eig,r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,rsnew)
...
! 4. Check rsnew
       ...
       if (par%ismasterproc) write(iulog,*) 'ioutl = ',ioutl,'iter = ',iter,   &
                                                      'rsnew = ',rsnew,criteria
...
! Convergence criterion of criteri as normalized rs
       if (criteria.lt.cgtol) then
         if (par%ismasterproc) write(iulog,*) 'ioutl = ',ioutl,                &
...
! 2-2. Local
   ...
   real(real_kind) :: maxsdu, maxsdv, maxsdt, maxsdq, maxsfu, maxsfv, maxsft,  &
 maxsfq, maxsfp, minsdu, minsdv, minsdt, minsdq, minsfu, minsfv, minsft, minsfq, minsfp, maxaru, maxarv, maxart, minaru, minarv, minart, maxamvu, maxamvv, minamvu, minamvv, maxscatwindu, maxscatwindv, minscatwindu, minscatwindv, maxbgp, minbgp
   ...
   if (singleobs) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
      ie)%u%val.ne.sdmiss).and.(obs(itime)%single(i, ie)%u%val.ne.sdmiss)) then
           tmpmax(1) = max(obs(itime)%single(i,ie)%u%val-gs_obs(itime)%single(i,&
                                                           ie)%u%val,tmpmax(1))
           tmpmin(1) = min(obs(itime)%single(i,ie)%u%val-gs_obs(itime)%single(i,&
                                                           ie)%u%val,tmpmin(1))
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single u:',obs(itime)%single(i,ie)%u%val,                   &
gs_obs(itime)%single(i,ie)%u%val,obs(itime)%single(i,ie)%u%val-gs_obs(itime)%single(i,ie)%u%val,sqrt(obs(itime)%single(i,ie)%u%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
      ie)%v%val.ne.sdmiss).and.(obs(itime)%single(i, ie)%v%val.ne.sdmiss)) then
           tmpmax(2) = max(obs(itime)%single(i,ie)%v%val-gs_obs(itime)%single(i,&
                                                           ie)%v%val,tmpmax(1))
           tmpmin(2) = min(obs(itime)%single(i,ie)%v%val-gs_obs(itime)%single(i,&
                                                           ie)%v%val,tmpmin(1))
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single v:',obs(itime)%single(i,ie)%v%val,                   &
gs_obs(itime)%single(i,ie)%v%val,obs(itime)%single(i,ie)%v%val-gs_obs(itime)%single(i,ie)%v%val,sqrt(obs(itime)%single(i,ie)%v%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
      ie)%t%val.ne.sdmiss).and.(obs(itime)%single(i, ie)%t%val.ne.sdmiss)) then
           tmpmax(3) = max(obs(itime)%single(i,ie)%t%val-gs_obs(itime)%single(i,&
                                                           ie)%t%val,tmpmax(1))
           tmpmin(4) = min(obs(itime)%single(i,ie)%t%val-gs_obs(itime)%single(i,&
                                                           ie)%t%val,tmpmin(1))
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single t:',obs(itime)%single(i,ie)%t%val,                   &
gs_obs(itime)%single(i,ie)%t%val,obs(itime)%single(i,ie)%t%val-gs_obs(itime)%single(i,ie)%t%val,sqrt(obs(itime)%single(i,ie)%t%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
      ie)%q%val.ne.sdmiss).and.(obs(itime)%single(i, ie)%q%val.ne.sdmiss)) then
           tmpmax(4) = max(obs(itime)%single(i,ie)%q%val-gs_obs(itime)%single(i,&
                                                           ie)%q%val,tmpmax(1))
           tmpmin(4) = min(obs(itime)%single(i,ie)%q%val-gs_obs(itime)%single(i,&
                                                           ie)%q%val,tmpmin(1))
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single q:',obs(itime)%single(i,ie)%q%val,                   &
gs_obs(itime)%single(i,ie)%q%val,obs(itime)%single(i,ie)%q%val-gs_obs(itime)%single(i,ie)%q%val,sqrt(obs(itime)%single(i,ie)%q%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
    ie)%ps%val.ne.sdmiss).and.(obs(itime)%single(i, ie)%ps%val.ne.sdmiss)) then
           tmpmax(5) = max(obs(itime)%single(i,ie)%ps%val-                     &
                                   gs_obs(itime)%single(i,ie)%ps%val,tmpmax(1))
           tmpmin(5) = min(obs(itime)%single(i,ie)%ps%val-                     &
                                   gs_obs(itime)%single(i,ie)%ps%val,tmpmin(1))
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single ps:',obs(itime)%single(i,ie)%ps%val,                 &
gs_obs(itime)%single(i,ie)%ps%val,obs(itime)%single(i,ie)%ps%val-gs_obs(itime)%single(i,ie)%ps%val,sqrt(obs(itime)%single(i,ie)%ps%error)
!      END IF
         endif
       enddo
     enddo
   ...
   if (da_sonde) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsd_e(1, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
       ie)%u%val.ne.sdmiss).and.(obs(itime)%sonde(i, ie)%u%val.ne.sdmiss)) then
           tmpmax(1) = max(obs(itime)%sonde(i,ie)%u%val-gs_obs(itime)%sonde(i, &
                                                           ie)%u%val,tmpmax(1))
           tmpmin(1) = min(obs(itime)%sonde(i,ie)%u%val-gs_obs(itime)%sonde(i, &
                                                           ie)%u%val,tmpmin(1))
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde u:',obs(itime)%sonde(i,ie)%u%val,                   &
gs_obs(itime)%sonde(i,ie)%u%val,obs(itime)%sonde(i,ie)%u%val-gs_obs(itime)%sonde(i,ie)%u%val,sqrt(obs(itime)%sonde(i,ie)%u%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nsd_e(2, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
       ie)%v%val.ne.sdmiss).and.(obs(itime)%sonde(i, ie)%v%val.ne.sdmiss)) then
           tmpmax(2) = max(obs(itime)%sonde(i,ie)%v%val-gs_obs(itime)%sonde(i, &
                                                           ie)%v%val,tmpmax(2))
           tmpmin(2) = min(obs(itime)%sonde(i,ie)%v%val-gs_obs(itime)%sonde(i, &
                                                           ie)%v%val,tmpmin(2))
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde v:',obs(itime)%sonde(i,ie)%v%val,                   &
gs_obs(itime)%sonde(i,ie)%v%val,obs(itime)%sonde(i,ie)%v%val-gs_obs(itime)%sonde(i,ie)%v%val,sqrt(obs(itime)%sonde(i,ie)%v%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nsd_e(3, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
       ie)%t%val.ne.sdmiss).and.(obs(itime)%sonde(i, ie)%t%val.ne.sdmiss)) then
           tmpmax(3) = max(obs(itime)%sonde(i,ie)%t%val-gs_obs(itime)%sonde(i, &
                                                           ie)%t%val,tmpmax(3))
           tmpmin(3) = min(obs(itime)%sonde(i,ie)%t%val-gs_obs(itime)%sonde(i, &
                                                           ie)%t%val,tmpmin(3))
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde t:',obs(itime)%sonde(i,ie)%t%val,                   &
gs_obs(itime)%sonde(i,ie)%t%val,obs(itime)%sonde(i,ie)%t%val-gs_obs(itime)%sonde(i,ie)%t%val,sqrt(obs(itime)%sonde(i,ie)%t%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nsd_e(4, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
       ie)%q%val.ne.sdmiss).and.(obs(itime)%sonde(i, ie)%q%val.ne.sdmiss)) then
           tmpmax(4) = max(obs(itime)%sonde(i,ie)%q%val-gs_obs(itime)%sonde(i, &
                                                           ie)%q%val,tmpmax(4))
           tmpmin(4) = min(obs(itime)%sonde(i,ie)%q%val-gs_obs(itime)%sonde(i, &
                                                           ie)%q%val,tmpmin(4))
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde q:',obs(itime)%sonde(i,ie)%q%val,                   &
gs_obs(itime)%sonde(i,ie)%q%val,obs(itime)%sonde(i,ie)%q%val-gs_obs(itime)%sonde(i,ie)%q%val,sqrt(obs(itime)%sonde(i,ie)%q%error)
           endif
         endif
       enddo
     enddo
   ...
   if (da_surface) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((gs_obs(itime)%surface(i,                                         &
     ie)%u%val.ne.sdmiss).and.(obs(itime)%surface(i, ie)%u%val.ne.sdmiss)) then
           tmpmax(5) = max(obs(itime)%surface(i,ie)%u%val-                     &
                                   gs_obs(itime)%surface(i,ie)%u%val,tmpmax(5))
           tmpmin(5) = min(obs(itime)%surface(i,ie)%u%val-                     &
                                   gs_obs(itime)%surface(i,ie)%u%val,tmpmin(5))
           if (par%ismasterproc.and.i==1) then
             print*,'Surface u:',obs(itime)%surface(i,ie)%u%val,               &
gs_obs(itime)%surface(i,ie)%u%val,obs(itime)%surface(i,ie)%u%val-gs_obs(itime)%surface(i,ie)%u%val,sqrt(obs(itime)%surface(i,ie)%u%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
     ie)%v%val.ne.sdmiss).and.(obs(itime)%surface(i, ie)%v%val.ne.sdmiss)) then
           tmpmax(6) = max(obs(itime)%surface(i,ie)%v%val-                     &
                                   gs_obs(itime)%surface(i,ie)%v%val,tmpmax(6))
           tmpmin(6) = min(obs(itime)%surface(i,ie)%v%val-                     &
                                   gs_obs(itime)%surface(i,ie)%v%val,tmpmin(6))
           if (par%ismasterproc.and.i==1) then
             print*,'Surface v:',obs(itime)%surface(i,ie)%v%val,               &
gs_obs(itime)%surface(i,ie)%v%val,obs(itime)%surface(i,ie)%v%val-gs_obs(itime)%surface(i,ie)%v%val,sqrt(obs(itime)%surface(i,ie)%v%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
     ie)%t%val.ne.sdmiss).and.(obs(itime)%surface(i, ie)%t%val.ne.sdmiss)) then
           tmpmax(7) = max(obs(itime)%surface(i,ie)%t%val-                     &
                                   gs_obs(itime)%surface(i,ie)%t%val,tmpmax(7))
           tmpmin(7) = min(obs(itime)%surface(i,ie)%t%val-                     &
                                   gs_obs(itime)%surface(i,ie)%t%val,tmpmin(7))
           if (par%ismasterproc.and.i==1) then
             print*,'Surface t:',obs(itime)%surface(i,ie)%t%val,               &
gs_obs(itime)%surface(i,ie)%t%val,obs(itime)%surface(i,ie)%t%val-gs_obs(itime)%surface(i,ie)%t%val,sqrt(obs(itime)%surface(i,ie)%t%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
     ie)%q%val.ne.sdmiss).and.(obs(itime)%surface(i, ie)%q%val.ne.sdmiss)) then
           tmpmax(8) = max(obs(itime)%surface(i,ie)%q%val-                     &
                                   gs_obs(itime)%surface(i,ie)%q%val,tmpmax(8))
           tmpmin(8) = min(obs(itime)%surface(i,ie)%q%val-                     &
                                   gs_obs(itime)%surface(i,ie)%q%val,tmpmin(8))
           if (par%ismasterproc.and.i==1) then
             print*,'Surface q:',obs(itime)%surface(i,ie)%q%val,               &
gs_obs(itime)%surface(i,ie)%q%val,obs(itime)%surface(i,ie)%q%val-gs_obs(itime)%surface(i,ie)%q%val,sqrt(obs(itime)%surface(i,ie)%q%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
     ie)%p%val.ne.sdmiss).and.(obs(itime)%surface(i, ie)%p%val.ne.sdmiss)) then
           tmpmax(9) = max(obs(itime)%surface(i,ie)%p%val-                     &
                                   gs_obs(itime)%surface(i,ie)%p%val,tmpmax(9))
           tmpmin(9) = min(obs(itime)%surface(i,ie)%p%val-                     &
                                   gs_obs(itime)%surface(i,ie)%p%val,tmpmin(9))
           if (par%ismasterproc.and.i==1) then
             print*,'Surface ps:',obs(itime)%surface(i,ie)%p%val,              &
gs_obs(itime)%surface(i,ie)%p%val,obs(itime)%surface(i,ie)%p%val-gs_obs(itime)%surface(i,ie)%p%val,sqrt(obs(itime)%surface(i,ie)%p%error)
           endif
         endif
       enddo
     enddo
   ...
   if (da_bogus) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%nbg_e(ie)
         if ((gs_obs(itime)%bogus(i,                                           &
       ie)%p%val.ne.sdmiss).and.(obs(itime)%bogus(i, ie)%p%val.ne.sdmiss)) then
           tmpmax(10) = max(obs(itime)%bogus(i,ie)%p%val-gs_obs(itime)%bogus(i,&
                                                          ie)%p%val,tmpmax(10))
           tmpmin(10) = min(obs(itime)%bogus(i,ie)%p%val-gs_obs(itime)%bogus(i,&
                                                          ie)%p%val,tmpmin(10))
           if (par%ismasterproc.and.i==1) then
             print*,'Surface_bg ps:',obs(itime)%bogus(i,ie)%p%val,             &
gs_obs(itime)%bogus(i,ie)%p%val,obs(itime)%bogus(i,ie)%p%val-gs_obs(itime)%bogus(i,ie)%p%val,sqrt(obs(itime)%bogus(i,ie)%p%error)
           endif
         endif
       enddo
     enddo
   ...
   if (da_aircraft) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%nar_e(1, ie)
         if ((gs_obs(itime)%aircraft(i,                                        &
    ie)%u%val.ne.sdmiss).and.(obs(itime)%aircraft(i, ie)%u%val.ne.sdmiss)) then
           tmpmax(11) = max(obs(itime)%aircraft(i,ie)%u%val-                   &
                                 gs_obs(itime)%aircraft(i,ie)%u%val,tmpmax(11))
           tmpmin(11) = min(obs(itime)%aircraft(i,ie)%u%val-                   &
                                 gs_obs(itime)%aircraft(i,ie)%u%val,tmpmin(11))
           if (par%ismasterproc.and.i==1) then
             print*,'Aircraft u:',obs(itime)%aircraft(i,ie)%u%val,             &
gs_obs(itime)%aircraft(i,ie)%u%val,obs(itime)%aircraft(i,ie)%u%val-gs_obs(itime)%aircraft(i,ie)%u%val,sqrt(obs(itime)%aircraft(i,ie)%u%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nar_e(2, ie)
         if ((gs_obs(itime)%aircraft(i,                                        &
    ie)%v%val.ne.sdmiss).and.(obs(itime)%aircraft(i, ie)%v%val.ne.sdmiss)) then
           tmpmax(12) = max(obs(itime)%aircraft(i,ie)%v%val-                   &
                                 gs_obs(itime)%aircraft(i,ie)%v%val,tmpmax(12))
           tmpmin(12) = min(obs(itime)%aircraft(i,ie)%v%val-                   &
                                 gs_obs(itime)%aircraft(i,ie)%v%val,tmpmin(12))
           if (par%ismasterproc.and.i==1) then
             print*,'Aircraft v:',obs(itime)%aircraft(i,ie)%v%val,             &
gs_obs(itime)%aircraft(i,ie)%v%val,obs(itime)%aircraft(i,ie)%v%val-gs_obs(itime)%aircraft(i,ie)%v%val,sqrt(obs(itime)%aircraft(i,ie)%v%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nar_e(3, ie)
         if ((gs_obs(itime)%aircraft(i,                                        &
    ie)%t%val.ne.sdmiss).and.(obs(itime)%aircraft(i, ie)%t%val.ne.sdmiss)) then
           tmpmax(13) = max(obs(itime)%aircraft(i,ie)%t%val-                   &
                                 gs_obs(itime)%aircraft(i,ie)%t%val,tmpmax(13))
           tmpmin(13) = min(obs(itime)%aircraft(i,ie)%t%val-                   &
                                 gs_obs(itime)%aircraft(i,ie)%t%val,tmpmin(13))
           if (par%ismasterproc.and.i==1) then
             print*,'Aircraft t:',obs(itime)%aircraft(i,ie)%t%val,             &
gs_obs(itime)%aircraft(i,ie)%t%val,obs(itime)%aircraft(i,ie)%t%val-gs_obs(itime)%aircraft(i,ie)%t%val,sqrt(obs(itime)%aircraft(i,ie)%t%error)
           endif
         endif
       enddo
     enddo
   ...
   if (da_amv) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%namv_e(ie)
         if ((gs_obs(itime)%amv(i, ie)%u%val.ne.sdmiss).and.(obs(itime)%amv(i, &
                                                     ie)%u%val.ne.sdmiss)) then
           tmpmax(14) = max(obs(itime)%amv(i,ie)%u%val-gs_obs(itime)%amv(i,    &
                                                          ie)%u%val,tmpmax(14))
           tmpmin(14) = min(obs(itime)%amv(i,ie)%u%val-gs_obs(itime)%amv(i,    &
                                                          ie)%u%val,tmpmin(14))
           if (par%ismasterproc.and.i==1) then
             print*,'Amv u:',obs(itime)%amv(i,ie)%u%val,gs_obs(itime)%amv(i,   &
ie)%u%val,obs(itime)%amv(i,ie)%u%val-gs_obs(itime)%amv(i,ie)%u%val,sqrt(obs(itime)%amv(i,ie)%u%error)
           endif
         endif
         if ((gs_obs(itime)%amv(i, ie)%v%val.ne.sdmiss).and.(obs(itime)%amv(i, &
                                                     ie)%v%val.ne.sdmiss)) then
           tmpmax(15) = max(obs(itime)%amv(i,ie)%v%val-gs_obs(itime)%amv(i,    &
                                                          ie)%v%val,tmpmax(15))
           tmpmin(15) = min(obs(itime)%amv(i,ie)%v%val-gs_obs(itime)%amv(i,    &
                                                          ie)%v%val,tmpmin(15))
           if (par%ismasterproc.and.i==1) then
             print*,'Amv v:',obs(itime)%amv(i,ie)%v%val,gs_obs(itime)%amv(i,   &
ie)%v%val,obs(itime)%amv(i,ie)%v%val-gs_obs(itime)%amv(i,ie)%v%val,sqrt(obs(itime)%amv(i,ie)%v%error)
           endif
         endif
       enddo
     enddo
   ...
   if (da_scatwind) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%nscatwind_e(ie)
         if ((gs_obs(itime)%scatwind(i,                                        &
    ie)%u%val.ne.sdmiss).and.(obs(itime)%scatwind(i, ie)%u%val.ne.sdmiss)) then
           tmpmax(16) = max(obs(itime)%scatwind(i,ie)%u%val-                   &
                                 gs_obs(itime)%scatwind(i,ie)%u%val,tmpmax(16))
           tmpmin(16) = min(obs(itime)%scatwind(i,ie)%u%val-                   &
                                 gs_obs(itime)%scatwind(i,ie)%u%val,tmpmin(16))
           if (par%ismasterproc.and.i==1) then
             print*,'Scatwind u:',obs(itime)%scatwind(i,ie)%u%val,             &
gs_obs(itime)%scatwind(i,ie)%u%val,obs(itime)%scatwind(i,ie)%u%val-gs_obs(itime)%scatwind(i,ie)%u%val,sqrt(obs(itime)%scatwind(i,ie)%u%error)
           endif
         endif
         if ((gs_obs(itime)%scatwind(i,                                        &
    ie)%v%val.ne.sdmiss).and.(obs(itime)%scatwind(i, ie)%v%val.ne.sdmiss)) then
           tmpmax(17) = max(obs(itime)%scatwind(i,ie)%v%val-                   &
                                 gs_obs(itime)%scatwind(i,ie)%v%val,tmpmax(17))
           tmpmin(17) = min(obs(itime)%scatwind(i,ie)%v%val-                   &
                                 gs_obs(itime)%scatwind(i,ie)%v%val,tmpmin(17))
           if (par%ismasterproc.and.i==1) then
             print*,'Scatwind v:',obs(itime)%scatwind(i,ie)%v%val,             &
gs_obs(itime)%scatwind(i,ie)%v%val,obs(itime)%scatwind(i,ie)%v%val-gs_obs(itime)%scatwind(i,ie)%v%val,sqrt(obs(itime)%scatwind(i,ie)%v%error)
           endif
         endif
       enddo
     enddo
...
!-------------------------------------------------------------------------------
   use check_omb_mod, only: sondeobcheck, surfaceobcheck, aircraftobcheck,     &
 amvobcheck, scatwindobcheck, amsuaobcheck, gpsroobcheck, iasiobcheck, crisobcheck, atmsobcheck, atmswvobcheck, mhsobcheck, csrobcheck
!
   integer(int_kind), intent(in   ) :: itime
   integer(int_kind) :: i, j, k, ie
!=================================================================
...
! sonde observation
   if (da_sonde) then
     ...
     call sondeobcheck(itime,"u",obs(itime)%sonde%u%val,                       &
                                               gs_obs(itime)%sonde%u%val,20.d0)
     call sondeobcheck(itime,"v",obs(itime)%sonde%v%val,                       &
                                               gs_obs(itime)%sonde%v%val,20.d0)
     call sondeobcheck(itime,"t",obs(itime)%sonde%t%val,                       &
                                               gs_obs(itime)%sonde%t%val,20.d0)
     if (cv_opt_hum==0) then
       call sondeobcheck(itime,"q",obs(itime)%sonde%q%val,                     &
                                               gs_obs(itime)%sonde%q%val,1.d-2)
     elseif (cv_opt_hum==1) then
       call sondeobcheck(itime,"q",obs(itime)%sonde%q%val,                     &
                                                gs_obs(itime)%sonde%q%val,2.d0)
     endif
   endif
! surface observation
   if (da_surface) then
     obs(itime)%surface(:,:) = obs_org(itime)%surface(:,:)
     call surfaceobcheck(itime,obs(itime)%surface%u%val,                       &
                                             gs_obs(itime)%surface%u%val,20.d0)
     call surfaceobcheck(itime,obs(itime)%surface%v%val,                       &
                                             gs_obs(itime)%surface%v%val,20.d0)
     call surfaceobcheck(itime,obs(itime)%surface%t%val,                       &
                                             gs_obs(itime)%surface%t%val,20.d0)
     call surfaceobcheck(itime,obs(itime)%surface%q%val,                       &
                                             gs_obs(itime)%surface%q%val,1.d-2)
     call surfaceobcheck(itime,obs(itime)%surface%p%val,                       &
                                           gs_obs(itime)%surface%p%val,1000.d0)
   endif
! aircraft observation
   if (da_aircraft) then
     obs(itime)%aircraft(:,:) = obs_org(itime)%aircraft(:,:)
     call aircraftobcheck(itime,"u",obs(itime)%aircraft%u%val,                 &
                                            gs_obs(itime)%aircraft%u%val,20.d0)
     call aircraftobcheck(itime,"v",obs(itime)%aircraft%v%val,                 &
                                            gs_obs(itime)%aircraft%v%val,20.d0)
     call aircraftobcheck(itime,"t",obs(itime)%aircraft%t%val,                 &
                                            gs_obs(itime)%aircraft%t%val,20.d0)
   endif
! amv observation
   if (da_amv) then
...
! scatwind observation
   if (da_scatwind) then
     ...
     call scatwindobcheck(itime,obs(itime)%scatwind%u%val,                     &
                                             gs_obs(itime)%scatwind%u%val,5.d0)
     call scatwindobcheck(itime,obs(itime)%scatwind%v%val,                     &
                                             gs_obs(itime)%scatwind%v%val,5.d0)
   endif
! amsua
   if (da_amsua) then
     obs(itime)%amsua(:,:,:) = obs_org(itime)%amsua(:,:,:)
     call amsuaobcheck(itime,obs(itime)%amsua%tb%val,                          &
dsqrt(obs(itime)%amsua%tb%error),obsmet(itime)%amsuabg%amsuahdx%tb%val,crit_omb_amsua)
   endif
! gpsroOBCheck
   if (da_gpsro) then
     obs(itime)%gpsro(:,:) = obs_org(itime)%gpsro(:,:)
     call gpsroobcheck(itime,obs(itime)%gpsro%ba%val,                          &
dsqrt(obs(itime)%gpsro%ba%error),obsmet(itime)%gpsrobg%gpsrohdx%ba%val,crit_omb_gpsro)
   endif
! iasiOBCheck
   if (da_iasi) then
     obs(itime)%iasi(:,:,:) = obs_org(itime)%iasi(:,:,:)
     call iasiobcheck(itime,obs(itime)%iasi%tb%val,                            &
dsqrt(obs(itime)%iasi%tb%error),obsmet(itime)%iasibg%iasihdx%tb%val,crit_omb_iasi)
   endif
! crisOBCheck
   if (da_cris) then
     obs(itime)%cris(:,:,:) = obs_org(itime)%cris(:,:,:)
     call crisobcheck(itime,obs(itime)%cris%tb%val,                            &
dsqrt(obs(itime)%cris%tb%error),obsmet(itime)%crisbg%crishdx%tb%val,crit_omb_cris)
   endif
! atmsOBCheck
   if (da_atms) then
     obs(itime)%atms(:,:,:) = obs_org(itime)%atms(:,:,:)
     call atmsobcheck(itime,obs(itime)%atms%tb%val,                            &
dsqrt(obs(itime)%atms%tb%error),obsmet(itime)%atmsbg%atmshdx%tb%val,crit_omb_atms)
   endif
! atmswvOBCheck
   if (da_atmswv) then
     obs(itime)%atmswv(:,:,:) = obs_org(itime)%atmswv(:,:,:)
     call atmswvobcheck(itime,obs(itime)%atmswv%tb%val,                        &
dsqrt(obs(itime)%atmswv%tb%error),obsmet(itime)%atmswvbg%atmswvhdx%tb%val,crit_omb_atmswv)
   endif
! mhsOBCheck
   if (da_mhs) then
     obs(itime)%mhs(:,:,:) = obs_org(itime)%mhs(:,:,:)
     call mhsobcheck(itime,obs(itime)%mhs%tb%val,                              &
                                obsmet(itime)%mhsbg%mhshdx%tb%val,crit_omb_mhs)
   endif
! csrOBCheck
   if (da_csr) then
     obs(itime)%csr(:,:,:) = obs_org(itime)%csr(:,:,:)
     call csrobcheck(itime,obs(itime)%csr%tb%val,dsqrt(obs(itime)%csr%tb%error),&
                                obsmet(itime)%csrbg%csrhdx%tb%val,crit_omb_csr)
   endif
   return
...
!
   ...
   real(real_kind) :: rmse_ua, rmse_va, rmse_ta, rmse_qa, rmse_psa, rmse_ub,   &
 rmse_vb, rmse_tb, rmse_qb, rmse_psb, rmse_sduo, rmse_sdvo, rmse_sdto, rmse_sdqo
!
                                                                               &
...
!.. Calculate Background error (U,V,T,and Q)
           global_shared_buf(ie,8) = global_shared_buf(ie,8)+(bg_md(i,j,k,ie,  &
                                              nt_itime)-nt_org(i,j,k,ie))**2.d0
           global_shared_buf(ie,9) = global_shared_buf(ie,9)+(bg_md(i,j,nlev+k,&
                                      ie,nt_itime)-nt_org(i,j,nlev+k,ie))**2.d0
           global_shared_buf(ie,10) = global_shared_buf(ie,10)+(bg_md(i,j,     &
                           2*nlev+k,ie,nt_itime)-nt_org(i,j,2*nlev+k,ie))**2.d0
           global_shared_buf(ie,11) = global_shared_buf(ie,11)+(bg_md(i,j,     &
                           3*nlev+k,ie,nt_itime)-nt_org(i,j,3*nlev+k,ie))**2.d0
!.. Calculate Analysis error (U,V,T,and Q)
           global_shared_buf(ie,12) = global_shared_buf(ie,12)+(an_md(i,j,k,ie,&
                                              nt_itime)-nt_org(i,j,k,ie))**2.d0
           global_shared_buf(ie,13) = global_shared_buf(ie,13)+(an_md(i,j,nlev+&
                                    k,ie,nt_itime)-nt_org(i,j,nlev+k,ie))**2.d0
           global_shared_buf(ie,14) = global_shared_buf(ie,14)+(an_md(i,j,     &
                           2*nlev+k,ie,nt_itime)-nt_org(i,j,2*nlev+k,ie))**2.d0
           global_shared_buf(ie,15) = global_shared_buf(ie,15)+(an_md(i,j,     &
                           3*nlev+k,ie,nt_itime)-nt_org(i,j,3*nlev+k,ie))**2.d0
         enddo
       enddo
     enddo
...
!.. Calculate Background error (PS)
         global_shared_buf(ie,17) = global_shared_buf(ie,17)+(bg_md(i,j,4*nlev+&
                                  1,ie,nt_itime)-nt_org(i,j,4*nlev+1,ie))**2.d0
!.. Calculate Analysis error (PS)
         global_shared_buf(ie,18) = global_shared_buf(ie,18)+(an_md(i,j,4*nlev+&
                                  1,ie,nt_itime)-nt_org(i,j,4*nlev+1,ie))**2.d0
       enddo
     enddo
   enddo
...
!  Local
   ...
   real(real_kind) :: obmsfu, obmsfv, obmsft, obmsfq, obmsfps ! o-b, average,  &
                                                                        surface
   real(real_kind) :: obdsdu, obdsdv, obdsdt, obdsdq ! o-b, sdv, sonde
   real(real_kind) :: obdsfu, obdsfv, obdsft, obdsfq, obdsfps ! o-b, sdv,      &
                                                                        surface
   ...
   real(real_kind) :: oamsfu, oamsfv, oamsft, oamsfq, oamsfps ! o-a, average,  &
                                                                        surface
   real(real_kind) :: oadsdu, oadsdv, oadsdt, oadsdq ! o-a, sdv, sonde
   real(real_kind) :: oadsfu, oadsfv, oadsft, oadsfq, oadsfps ! o-a, sdv,      &
                                                                        surface
   ...
   real(real_kind) :: nobsfu, nobsfv, nobsft, nobsfq, nobsfps ! number of o-b, &
                                                                        surface
   real(real_kind) :: noasdu, noasdv, noasdt, noasdq ! number of o-a, sonde
   real(real_kind) :: noasfu, noasfv, noasft, noasfq, noasfps ! number of o-a, &
                                                                        surface
...
!.. Mean of O-B and O-A for Single Observatino
   if (singleobs) then
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsg_e(ie)
         print*,'i,ie:',i,ie
         print*,'bg_obs(itime)%single(i,ie)%u%val,an_obs(itime)%single(i,      &
   ie)%u%val',bg_obs(itime)%single(i,ie)%u%val,an_obs(itime)%single(i,ie)%u%val
         print*,'bg_obs(itime)%single(i,ie)%v%val,an_obs(itime)%single(i,      &
   ie)%v%val',bg_obs(itime)%single(i,ie)%v%val,an_obs(itime)%single(i,ie)%v%val
         print*,'bg_obs(itime)%single(i,ie)%t%val,an_obs(itime)%single(i,      &
   ie)%t%val',bg_obs(itime)%single(i,ie)%t%val,an_obs(itime)%single(i,ie)%t%val
         print*,'bg_obs(itime)%single(i,ie)%q%val,an_obs(itime)%single(i,      &
   ie)%q%val',bg_obs(itime)%single(i,ie)%q%val,an_obs(itime)%single(i,ie)%q%val
         print*,'bg_obs(itime)%single(i,ie)%ps%val,an_obs(itime)%single(i,     &
ie)%ps%val',bg_obs(itime)%single(i,ie)%ps%val,an_obs(itime)%single(i,ie)%ps%val
       enddo
     enddo
   endif
...
! background
       do i = 1, obsmet(itime)%nsd_e(1, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%u%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i, ie)%u%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsd_e(2, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%v%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i, ie)%v%val.ne.sdmiss)) then
           global_shared_buf(ie,2) = global_shared_buf(ie,2)+                  &
       ...
       do i = 1, obsmet(itime)%nsd_e(3, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%t%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i, ie)%t%val.ne.sdmiss)) then
           global_shared_buf(ie,3) = global_shared_buf(ie,3)+                  &
       ...
       do i = 1, obsmet(itime)%nsd_e(4, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%q%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i, ie)%q%val.ne.sdmiss)) then
           global_shared_buf(ie,4) = global_shared_buf(ie,4)+                  &
...
! analysis
       do i = 1, obsmet(itime)%nsd_e(1, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%u%val.ne.sdmiss).and.(an_obs(itime)%sonde(i, ie)%u%val.ne.sdmiss)) then
           global_shared_buf(ie,5) = global_shared_buf(ie,5)+                  &
       ...
       do i = 1, obsmet(itime)%nsd_e(2, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%v%val.ne.sdmiss).and.(an_obs(itime)%sonde(i, ie)%v%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsd_e(3, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%t%val.ne.sdmiss).and.(an_obs(itime)%sonde(i, ie)%t%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsd_e(4, ie)
         if ((obs(itime)%sonde(i,                                              &
    ie)%q%val.ne.sdmiss).and.(an_obs(itime)%sonde(i, ie)%q%val.ne.sdmiss)) then
...
! background
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%u%val.ne.sdmiss).and.(bg_obs(itime)%surface(i, ie)%u%val.ne.sdmiss)) then
           global_shared_buf(ie,1) = global_shared_buf(ie,1)+                  &
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%v%val.ne.sdmiss).and.(bg_obs(itime)%surface(i, ie)%v%val.ne.sdmiss)) then
           global_shared_buf(ie,2) = global_shared_buf(ie,2)+                  &
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%t%val.ne.sdmiss).and.(bg_obs(itime)%surface(i, ie)%t%val.ne.sdmiss)) then
           global_shared_buf(ie,3) = global_shared_buf(ie,3)+                  &
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%q%val.ne.sdmiss).and.(bg_obs(itime)%surface(i, ie)%q%val.ne.sdmiss)) then
           global_shared_buf(ie,4) = global_shared_buf(ie,4)+                  &
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%p%val.ne.sdmiss).and.(bg_obs(itime)%surface(i, ie)%p%val.ne.sdmiss)) then
           global_shared_buf(ie,5) = global_shared_buf(ie,5)+                  &
...
! analysis
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%u%val.ne.sdmiss).and.(an_obs(itime)%surface(i, ie)%u%val.ne.sdmiss)) then
           global_shared_buf(ie,6) = global_shared_buf(ie,6)+                  &
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%v%val.ne.sdmiss).and.(an_obs(itime)%surface(i, ie)%v%val.ne.sdmiss)) then
           global_shared_buf(ie,7) = global_shared_buf(ie,7)+                  &
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%t%val.ne.sdmiss).and.(an_obs(itime)%surface(i, ie)%t%val.ne.sdmiss)) then
           global_shared_buf(ie,8) = global_shared_buf(ie,8)+                  &
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%q%val.ne.sdmiss).and.(an_obs(itime)%surface(i, ie)%q%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
  ie)%p%val.ne.sdmiss).and.(an_obs(itime)%surface(i, ie)%p%val.ne.sdmiss)) then
...
! background
       do i = 1, obsmet(itime)%nbg_e(ie)
         if ((obs(itime)%bogus(i,                                              &
    ie)%p%val.ne.sdmiss).and.(bg_obs(itime)%bogus(i, ie)%p%val.ne.sdmiss)) then
           global_shared_buf(ie,1) = global_shared_buf(ie,1)+                  &
...
! analysis
       do i = 1, obsmet(itime)%nbg_e(ie)
         if ((obs(itime)%bogus(i,                                              &
    ie)%p%val.ne.sdmiss).and.(an_obs(itime)%bogus(i, ie)%p%val.ne.sdmiss)) then
           global_shared_buf(ie,2) = global_shared_buf(ie,2)+                  &
...
!
   ...
   call tlmeigtoobs(gs_md(:,:,:,:,:),b_eig,b_eig_chi,b_eig_tps,b_eig_q,b_eig_a,&
                                                                     tl_obs(:))
   ierr = timingstop('TlmEigToObs_Check_Adj') !---profiling
   do itime = 1,ntime
! single observation
     if (singleobs) then
       call multiplyrsingle(itime,tl_obs(itime)%single%u%val,                  &
tl_obs(itime)%single%v%val,tl_obs(itime)%single%t%val,tl_obs(itime)%single%q%val,tl_obs(itime)%single%ps%val)
     endif
! sonde observation
     if (da_sonde) then
       call multiplyrsonde(itime,tl_obs(itime)%sonde%u%val,                    &
 tl_obs(itime)%sonde%v%val,tl_obs(itime)%sonde%t%val,tl_obs(itime)%sonde%q%val)
     endif
! surface observation
     if (da_surface) then
       call multiplyrsurface(itime,tl_obs(itime)%surface%u%val,                &
tl_obs(itime)%surface%v%val,tl_obs(itime)%surface%t%val,tl_obs(itime)%surface%q%val,tl_obs(itime)%surface%p%val)
     endif
! bogus observation
     if (da_bogus) then
...
! aircraft observation
     if (da_aircraft) then
       call multiplyraircraft(itime,tl_obs(itime)%aircraft%u%val,              &
...
! scatwind observation
     if (da_scatwind) then
       call multiplyrscatwind(itime,tl_obs(itime)%scatwind%u%val,              &
                                                  tl_obs(itime)%scatwind%v%val)
     endif
! amsua observation
     if (da_amsua) then
...
! gpsro observation
     if (da_gpsro) then
       call multiplyrgpsro(itime,tl_obs(itime)%gpsro%t%val,                    &
                                                     tl_obs(itime)%gpsro%q%val)
     endif
! iasi observation
     if (da_iasi) then
       call multiplyriasi(itime,tl_obs(itime)%iasi%t%val,                      &
                                                      tl_obs(itime)%iasi%q%val)
     endif
! cris observation
     if (da_cris) then
       call multiplyrcris(itime,tl_obs(itime)%cris%t%val,                      &
                                                      tl_obs(itime)%cris%q%val)
     endif
! atms observation
     if (da_atms) then
...
! atmswv observation
     if (da_atmswv) then
       call multiplyratmswv(itime,tl_obs(itime)%atmswv%t%val,                  &
                                                    tl_obs(itime)%atmswv%q%val)
     endif
! mhs observation
     if (da_mhs) then
...
! Single observation
     if (singleobs) then
       ...
       call multiplyrsingle(itime,ad_obs(itime)%single%u%val,                  &
ad_obs(itime)%single%v%val,ad_obs(itime)%single%t%val,ad_obs(itime)%single%q%val,ad_obs(itime)%single%ps%val)
     endif
! sonde observation
     if (da_sonde) then
       ad_obs(itime)%sonde = tl_obs(itime)%sonde
       call multiplyrsonde(itime,ad_obs(itime)%sonde%u%val,                    &
 ad_obs(itime)%sonde%v%val,ad_obs(itime)%sonde%t%val,ad_obs(itime)%sonde%q%val)
     endif
! surface observation
     if (da_surface) then
       ad_obs(itime)%surface = tl_obs(itime)%surface
       call multiplyrsurface(itime,ad_obs(itime)%surface%u%val,                &
ad_obs(itime)%surface%v%val,ad_obs(itime)%surface%t%val,ad_obs(itime)%surface%q%val,ad_obs(itime)%surface%p%val)
     endif
! bogus observation
     if (da_bogus) then
...
! aircraft observation
     if (da_aircraft) then
       ...
       call multiplyraircraft(itime,ad_obs(itime)%aircraft%u%val,              &
                     ad_obs(itime)%aircraft%v%val,ad_obs(itime)%aircraft%t%val)
     endif
! amv observation
     if (da_amv) then
...
! scatwind observation
     if (da_scatwind) then
       ...
       call multiplyrscatwind(itime,ad_obs(itime)%scatwind%u%val,              &
                                                  ad_obs(itime)%scatwind%v%val)
     endif
! amsua observation
     if (da_amsua) then
...
! gpsro observation
     if (da_gpsro) then
       ...
       call multiplyrgpsro(itime,ad_obs(itime)%gpsro%t%val,                    &
                                                     ad_obs(itime)%gpsro%q%val)
     endif
! iasi observation
     if (da_iasi) then
       ad_obs(itime)%iasi = tl_obs(itime)%iasi
       call multiplyriasi(itime,ad_obs(itime)%iasi%t%val,                      &
                                                      ad_obs(itime)%iasi%q%val)
     endif
! cris observation
     if (da_cris) then
       ad_obs(itime)%cris = tl_obs(itime)%cris
       call multiplyrcris(itime,ad_obs(itime)%cris%t%val,                      &
                                                      ad_obs(itime)%cris%q%val)
     endif
! atms observation
     if (da_atms) then
...
! atmswv observation
     if (da_atmswv) then
       ...
       call multiplyratmswv(itime,ad_obs(itime)%atmswv%t%val,                  &
                                                    ad_obs(itime)%atmswv%q%val)
     endif
! mhs observation
     if (da_mhs) then
...
! csr observation
   ...
   ad_eig_q = 0.d0
   ad_eig_a = 0.d0
   call adjeigtoobs(gs_md(:,:,:,:,:),ad_obs(:),ad_eig,ad_eig_chi,ad_eig_tps,   &
                                                             ad_eig_q,ad_eig_a)
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%nbg_e(ie)
           if (tl_obs(itime)%bogus(i, ie)%p%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                          tl_obs(itime)%bogus(i,ie)%p%val**2.d0
           endif
         enddo
       enddo
     endif
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%nar_e(1, ie)
           if (tl_obs(itime)%aircraft(i, ie)%u%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                       tl_obs(itime)%aircraft(i,ie)%u%val**2.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nar_e(2, ie)
           if (tl_obs(itime)%aircraft(i, ie)%v%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                       tl_obs(itime)%aircraft(i,ie)%v%val**2.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nar_e(3, ie)
           if (tl_obs(itime)%aircraft(i, ie)%t%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                       tl_obs(itime)%aircraft(i,ie)%t%val**2.d0
           endif
         enddo
       enddo
     endif
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%namv_e(ie)
           if (tl_obs(itime)%amv(i, ie)%u%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                            tl_obs(itime)%amv(i,ie)%u%val**2.d0
           endif
           if (tl_obs(itime)%amv(i, ie)%v%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                            tl_obs(itime)%amv(i,ie)%v%val**2.d0
           endif
         enddo
       enddo
     endif
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%nscatwind_e(ie)
           if (tl_obs(itime)%scatwind(i, ie)%u%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                       tl_obs(itime)%scatwind(i,ie)%u%val**2.d0
           endif
           if (tl_obs(itime)%scatwind(i, ie)%v%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                       tl_obs(itime)%scatwind(i,ie)%v%val**2.d0
           endif
         enddo
       enddo
     endif
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%namsua_e(ie)
             if (tl_obs(itime)%amsua(i, j, ie)%t%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                        tl_obs(itime)%amsua(i,j,ie)%t%val**2.d0
             endif
           enddo
         enddo
       enddo
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%ngpsro_e(ie)
           if (tl_obs(itime)%gpsro(i, ie)%t%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                          tl_obs(itime)%gpsro(i,ie)%t%val**2.d0
           endif
           if (tl_obs(itime)%gpsro(i, ie)%q%val.ne.sdmiss) then
             global_shared_buf(ie,1) = global_shared_buf(ie,1)+                &
                                          tl_obs(itime)%gpsro(i,ie)%q%val**2.d0
           endif
         enddo
       enddo
     endif
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%niasi_e(ie)
             if (tl_obs(itime)%iasi(i, j, ie)%t%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                         tl_obs(itime)%iasi(i,j,ie)%t%val**2.d0
             endif
             if (tl_obs(itime)%iasi(i, j, ie)%q%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                         tl_obs(itime)%iasi(i,j,ie)%q%val**2.d0
             endif
           enddo
         enddo
       enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%ncris_e(ie)
             if (tl_obs(itime)%cris(i, j, ie)%t%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                         tl_obs(itime)%cris(i,j,ie)%t%val**2.d0
             endif
             if (tl_obs(itime)%cris(i, j, ie)%q%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                         tl_obs(itime)%cris(i,j,ie)%q%val**2.d0
             endif
           enddo
         enddo
       enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%natms_e(ie)
             if (tl_obs(itime)%atms(i, j, ie)%t%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                         tl_obs(itime)%atms(i,j,ie)%t%val**2.d0
             endif
           enddo
         enddo
       enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%natmswv_e(ie)
             if (tl_obs(itime)%atmswv(i, j, ie)%t%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                       tl_obs(itime)%atmswv(i,j,ie)%t%val**2.d0
             endif
             if (tl_obs(itime)%atmswv(i, j, ie)%q%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                       tl_obs(itime)%atmswv(i,j,ie)%q%val**2.d0
             endif
           enddo
         enddo
       enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%nmhs_e(ie)
             if (tl_obs(itime)%mhs(i, j, ie)%t%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                          tl_obs(itime)%mhs(i,j,ie)%t%val**2.d0
             endif
             if (tl_obs(itime)%mhs(i, j, ie)%q%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                          tl_obs(itime)%mhs(i,j,ie)%q%val**2.d0
             endif
           enddo
         enddo
       enddo
...
! csr observation
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%ncsr_e(ie)
             if (tl_obs(itime)%csr(i, j, ie)%t%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                          tl_obs(itime)%csr(i,j,ie)%t%val**2.d0
             endif
             if (tl_obs(itime)%csr(i, j, ie)%q%val.ne.sdmiss) then
               global_shared_buf(ie,1) = global_shared_buf(ie,1)+              &
                                          tl_obs(itime)%csr(i,j,ie)%q%val**2.d0
             endif
           enddo
         enddo
       enddo
   ...
   call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,b_eig,&
b_eig_chi,b_eig_tps,b_eig_q,b_eig_a,ad_eig,ad_eig_chi,ad_eig_tps,ad_eig_q,ad_eig_a,tmp)
   if (par%ismasterproc) then
...
!===============================================================================
   subroutine innerproduct(neig, neig_chi, neig_tps, neig_q, zwn_nproc,        &
 zwn_nproc_a, vect1, vect1_chi, vect1_tps, vect1_q, vect1_a, vect2, vect2_chi, vect2_tps, vect2_q, vect2_a, ddot_product)
!=============================================================================
! A. Declaration
!=============================================================================
! 1. Local modules
...
! 2-1. Input
!
   integer(int_kind), intent(in   ) :: neig, neig_chi, neig_tps, neig_q,       &
                                                         zwn_nproc, zwn_nproc_a
   real(real_kind), dimension(neig, zwn_nproc), intent(in   ) :: vect1, vect2
   real(real_kind), dimension(neig_chi, zwn_nproc), intent(in   ) :: vect1_chi,&
                                                                      vect2_chi
   real(real_kind), dimension(neig_tps, zwn_nproc), intent(in   ) :: vect1_tps,&
                                                                      vect2_tps
   real(real_kind), dimension(neig_q, zwn_nproc), intent(in   ) :: vect1_q,    &
                                                                        vect2_q
   real(real_kind), dimension(nlev+1, zwn_nproc_a, nsmpl),                     &
...
!IF (par%ismasterproc) WRITE(iulog,*) 'tmp 1= ', tmp1,k,zwn_nproc,jump
     ...
     do wn = j, zwn_nproc_a, jump
       tmp1 = tmp1+dot_product(vect1_a(:,wn,ismpl),vect2_a(:,wn,ismpl))+       &
dot_product(vect1_a(:,wn+1,ismpl),vect2_a(:,wn+1,ismpl))+dot_product(vect1_a(:,wn+2,ismpl),vect2_a(:,wn+2,ismpl))+dot_product(vect1_a(:,wn+3,ismpl),vect2_a(:,wn+3,ismpl))+dot_product(vect1_a(:,wn+4,ismpl),vect2_a(:,wn+4,ismpl))
     enddo
   enddo ! ismpl
   global_shared_buf(:,1) = 0.d0
...
!===============================================================================
!===============================================================================
   subroutine make_lhs_ax(gs_md, x_eig, x_eig_chi, x_eig_tps, x_eig_q, x_eig_a,&
                            ap_eig, ap_eig_chi, ap_eig_tps, ap_eig_q, ap_eig_a)
! In/Output
!
   real(real_kind), dimension(np, np, nlevar, nelemd, ntime),                  &
                                                         intent(in   ) :: gs_md
...
! 3. Adjoint of spectral space -> observation space
   ...
   ad_eig_q = 0.d0
   ad_eig_a = 0.d0
   call adjeigtoobs(gs_md,tl_obs(:),ad_eig,ad_eig_chi,ad_eig_tps,ad_eig_q,     &
                                                                      ad_eig_a)
...
! single observation test
     if (singleobs) then
       call makebsingle(itime,gs_obs(itime)%single%u%val,                      &
gs_obs(itime)%single%v%val,gs_obs(itime)%single%t%val,gs_obs(itime)%single%q%val,gs_obs(itime)%single%ps%val,tl_obs(itime)%single%u%val,tl_obs(itime)%single%v%val,tl_obs(itime)%single%t%val,tl_obs(itime)%single%q%val,tl_obs(itime)%single%ps%val)
     endif
! sonde observation
     if (da_sonde) then
       call makebsonde(itime,gs_obs(itime)%sonde%u%val,                        &
gs_obs(itime)%sonde%v%val,gs_obs(itime)%sonde%t%val,gs_obs(itime)%sonde%q%val,tl_obs(itime)%sonde%u%val,tl_obs(itime)%sonde%v%val,tl_obs(itime)%sonde%t%val,tl_obs(itime)%sonde%q%val)
     endif
! surface observation
     if (da_surface) then
       call makebsurface(itime,gs_obs(itime)%surface%u%val,                    &
gs_obs(itime)%surface%v%val,gs_obs(itime)%surface%t%val,gs_obs(itime)%surface%q%val,gs_obs(itime)%surface%p%val,tl_obs(itime)%surface%u%val,tl_obs(itime)%surface%v%val,tl_obs(itime)%surface%t%val,tl_obs(itime)%surface%q%val,tl_obs(itime)%surface%p%val)
     endif
! bogus observation
     if (da_bogus) then
       call makebbogus(itime,gs_obs(itime)%bogus%p%val,                        &
                                                     tl_obs(itime)%bogus%p%val)
     endif
! aircraft observation
     if (da_aircraft) then
       call makebaircraft(itime,gs_obs(itime)%aircraft%u%val,                  &
gs_obs(itime)%aircraft%v%val,gs_obs(itime)%aircraft%t%val,tl_obs(itime)%aircraft%u%val,tl_obs(itime)%aircraft%v%val,tl_obs(itime)%aircraft%t%val)
     endif
! amv observation
     if (da_amv) then
...
! scatwind observation
     if (da_scatwind) then
       call makebscatwind(itime,gs_obs(itime)%scatwind%u%val,                  &
gs_obs(itime)%scatwind%v%val,tl_obs(itime)%scatwind%u%val,tl_obs(itime)%scatwind%v%val)
     endif
! amsua observation
     if (da_amsua) then
...
! gpsro observation
     if (da_gpsro) then
       call makebgpsro(itime,gs_obs(itime)%gpsro%t%val,                        &
 gs_obs(itime)%gpsro%q%val,tl_obs(itime)%gpsro%t%val,tl_obs(itime)%gpsro%q%val)
     endif
! iasi observation
     if (da_iasi) then
...
! atmswv observation
     if (da_atmswv) then
       call makebatmswv(itime,gs_obs(itime)%atmswv%t%val,                      &
gs_obs(itime)%atmswv%q%val,tl_obs(itime)%atmswv%t%val,tl_obs(itime)%atmswv%q%val)
     endif
! mhs observation
     if (da_mhs) then
...
!
   ...
   b_eig_q = 0.d0
   b_eig_a = 0.d0
   call adjeigtoobs(gs_md(:,:,:,:,:),tl_obs(:),b_eig,b_eig_chi,b_eig_tps,      &
                                                               b_eig_q,b_eig_a)
...
!===============================================================================
!===============================================================================
   subroutine cal_cost_func(gs_md, x_eig, x_eig_chi, x_eig_tps, x_eig_q,       &
                                                             x_eig_a, j_o, j_b)
! 2. Variables
! 2-1. In/Output
!
...
! 2-2. Local
   ...
   real(real_kind) :: tmpmax, tmpmin, maxv, minv
   real(real_kind) :: rmse_an_u, rmse_an_v, rmse_an_t, rmse_an_q, rmse_an_ps,  &
 tmp, jo_single, jo_sonde, jo_surface, jo_bogus, jo_aircraft, jo_amv, jo_scatwind, jo_amsua, jo_gpsro, jo_iasi, jo_cris, jo_atms, jo_atmswv, jo_mhs, jo_csr
...
!
   ...
   call tlmeigtoobs(gs_md(:,:,:,:,:),x_eig,x_eig_chi,x_eig_tps,x_eig_q,x_eig_a,&
                                                                     tl_obs(:))
...
! Single observation test
     if (singleobs) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%nsg_e(ie)
           if ((obs(itime)%single(i,                                           &
 ie)%u%val.ne.sdmiss).and.(gs_obs(itime)%single(i, ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%single(i, ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,2) = global_shared_buf(ie,2)+                &
(gs_obs(itime)%single(i,ie)%u%val+tl_obs(itime)%single(i,ie)%u%val-obs(itime)%single(i,ie)%u%val)**2.d0/(obs(itime)%single(i,ie)%u%error)
           endif
           if ((obs(itime)%single(i,                                           &
 ie)%v%val.ne.sdmiss).and.(gs_obs(itime)%single(i, ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%single(i, ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,3) = global_shared_buf(ie,3)+                &
(gs_obs(itime)%single(i,ie)%v%val+tl_obs(itime)%single(i,ie)%v%val-obs(itime)%single(i,ie)%v%val)**2.d0/(obs(itime)%single(i,ie)%v%error)
           endif
           if ((obs(itime)%single(i,                                           &
 ie)%t%val.ne.sdmiss).and.(gs_obs(itime)%single(i, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%single(i, ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,4) = global_shared_buf(ie,4)+                &
(gs_obs(itime)%single(i,ie)%t%val+tl_obs(itime)%single(i,ie)%t%val-obs(itime)%single(i,ie)%t%val)**2.d0/(obs(itime)%single(i,ie)%t%error)
           endif
           if ((obs(itime)%single(i,                                           &
 ie)%q%val.ne.sdmiss).and.(gs_obs(itime)%single(i, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%single(i, ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,5) = global_shared_buf(ie,5)+                &
(gs_obs(itime)%single(i,ie)%q%val+tl_obs(itime)%single(i,ie)%q%val-obs(itime)%single(i,ie)%q%val)**2.d0/(obs(itime)%single(i,ie)%q%error)
           endif
           if ((obs(itime)%single(i,                                           &
 ie)%ps%val.ne.sdmiss).and.(gs_obs(itime)%single(i, ie)%ps%val.ne.sdmiss).and.(tl_obs(itime)%single(i, ie)%ps%val.ne.sdmiss)) then
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+                &
(gs_obs(itime)%single(i,ie)%ps%val+tl_obs(itime)%single(i,ie)%ps%val-obs(itime)%single(i,ie)%ps%val)**2.d0/(obs(itime)%single(i,ie)%ps%error)
           endif
         enddo
       enddo
     endif
! sonde observation
     if (da_sonde) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%nsd_e(1, ie)
           if ((obs(itime)%sonde(i,                                            &
 ie)%u%val.ne.sdmiss).and.(gs_obs(itime)%sonde(i, ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i, ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,2) = global_shared_buf(ie,2)+                &
(gs_obs(itime)%sonde(i,ie)%u%val+tl_obs(itime)%sonde(i,ie)%u%val-obs(itime)%sonde(i,ie)%u%val)**2.d0/(obs(itime)%sonde(i,ie)%u%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nsd_e(2, ie)
           if ((obs(itime)%sonde(i,                                            &
 ie)%v%val.ne.sdmiss).and.(gs_obs(itime)%sonde(i, ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i, ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,3) = global_shared_buf(ie,3)+                &
(gs_obs(itime)%sonde(i,ie)%v%val+tl_obs(itime)%sonde(i,ie)%v%val-obs(itime)%sonde(i,ie)%v%val)**2.d0/(obs(itime)%sonde(i,ie)%v%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nsd_e(3, ie)
           if ((obs(itime)%sonde(i,                                            &
 ie)%t%val.ne.sdmiss).and.(gs_obs(itime)%sonde(i, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i, ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,4) = global_shared_buf(ie,4)+                &
(gs_obs(itime)%sonde(i,ie)%t%val+tl_obs(itime)%sonde(i,ie)%t%val-obs(itime)%sonde(i,ie)%t%val)**2.d0/(obs(itime)%sonde(i,ie)%t%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nsd_e(4, ie)
           if ((obs(itime)%sonde(i,                                            &
 ie)%q%val.ne.sdmiss).and.(gs_obs(itime)%sonde(i, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i, ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,5) = global_shared_buf(ie,5)+                &
(gs_obs(itime)%sonde(i,ie)%q%val+tl_obs(itime)%sonde(i,ie)%q%val-obs(itime)%sonde(i,ie)%q%val)**2.d0/(obs(itime)%sonde(i,ie)%q%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
       enddo
...
! surface observation
     if (da_surface) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%nsf_e(ie)
           if ((obs(itime)%surface(i,                                          &
 ie)%u%val.ne.sdmiss).and.(gs_obs(itime)%surface(i, ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%surface(i, ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,7) = global_shared_buf(ie,7)+                &
(gs_obs(itime)%surface(i,ie)%u%val+tl_obs(itime)%surface(i,ie)%u%val-obs(itime)%surface(i,ie)%u%val)**2.d0/(obs(itime)%surface(i,ie)%u%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
 ie)%v%val.ne.sdmiss).and.(gs_obs(itime)%surface(i, ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%surface(i, ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,8) = global_shared_buf(ie,8)+                &
(gs_obs(itime)%surface(i,ie)%v%val+tl_obs(itime)%surface(i,ie)%v%val-obs(itime)%surface(i,ie)%v%val)**2.d0/(obs(itime)%surface(i,ie)%v%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
 ie)%t%val.ne.sdmiss).and.(gs_obs(itime)%surface(i, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%surface(i, ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,9) = global_shared_buf(ie,9)+                &
(gs_obs(itime)%surface(i,ie)%t%val+tl_obs(itime)%surface(i,ie)%t%val-obs(itime)%surface(i,ie)%t%val)**2.d0/(obs(itime)%surface(i,ie)%t%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
 ie)%q%val.ne.sdmiss).and.(gs_obs(itime)%surface(i, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%surface(i, ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,10) = global_shared_buf(ie,10)+              &
(gs_obs(itime)%surface(i,ie)%q%val+tl_obs(itime)%surface(i,ie)%q%val-obs(itime)%surface(i,ie)%q%val)**2.d0/(obs(itime)%surface(i,ie)%q%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
 ie)%p%val.ne.sdmiss).and.(gs_obs(itime)%surface(i, ie)%p%val.ne.sdmiss).and.(tl_obs(itime)%surface(i, ie)%p%val.ne.sdmiss)) then
             global_shared_buf(ie,11) = global_shared_buf(ie,11)+              &
(gs_obs(itime)%surface(i,ie)%p%val+tl_obs(itime)%surface(i,ie)%p%val-obs(itime)%surface(i,ie)%p%val)**2.d0/(obs(itime)%surface(i,ie)%p%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
         enddo
       enddo
...
! bogus observation
     if (da_bogus) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%nbg_e(ie)
           if ((obs(itime)%bogus(i,                                            &
 ie)%p%val.ne.sdmiss).and.(gs_obs(itime)%bogus(i, ie)%p%val.ne.sdmiss).and.(tl_obs(itime)%bogus(i, ie)%p%val.ne.sdmiss)) then
             global_shared_buf(ie,13) = global_shared_buf(ie,13)+              &
(gs_obs(itime)%bogus(i,ie)%p%val+tl_obs(itime)%bogus(i,ie)%p%val-obs(itime)%bogus(i,ie)%p%val)**2.d0/(obs(itime)%bogus(i,ie)%p%error)
             global_shared_buf(ie,14) = global_shared_buf(ie,14)+1.d0
           endif
         enddo
       enddo
...
! aircraft observation
     if (da_aircraft) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%nar_e(1, ie)
           if ((obs(itime)%aircraft(i,                                         &
 ie)%u%val.ne.sdmiss).and.(gs_obs(itime)%aircraft(i, ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%aircraft(i, ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,15) = global_shared_buf(ie,15)+              &
(gs_obs(itime)%aircraft(i,ie)%u%val+tl_obs(itime)%aircraft(i,ie)%u%val-obs(itime)%aircraft(i,ie)%u%val)**2.d0/(obs(itime)%aircraft(i,ie)%u%error)
             global_shared_buf(ie,18) = global_shared_buf(ie,18)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nar_e(2, ie)
           if ((obs(itime)%aircraft(i,                                         &
 ie)%v%val.ne.sdmiss).and.(gs_obs(itime)%aircraft(i, ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%aircraft(i, ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,16) = global_shared_buf(ie,16)+              &
(gs_obs(itime)%aircraft(i,ie)%v%val+tl_obs(itime)%aircraft(i,ie)%v%val-obs(itime)%aircraft(i,ie)%v%val)**2.d0/(obs(itime)%aircraft(i,ie)%v%error)
             global_shared_buf(ie,18) = global_shared_buf(ie,18)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nar_e(3, ie)
           if ((obs(itime)%aircraft(i,                                         &
 ie)%t%val.ne.sdmiss).and.(gs_obs(itime)%aircraft(i, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%aircraft(i, ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,17) = global_shared_buf(ie,17)+              &
(gs_obs(itime)%aircraft(i,ie)%t%val+tl_obs(itime)%aircraft(i,ie)%t%val-obs(itime)%aircraft(i,ie)%t%val)**2.d0/(obs(itime)%aircraft(i,ie)%t%error)
             global_shared_buf(ie,18) = global_shared_buf(ie,18)+1.d0
           endif
         enddo
       enddo
...
! amv observation
     if (da_amv) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%namv_e(ie)
           if ((obs(itime)%amv(i, ie)%u%val.ne.sdmiss).and.(gs_obs(itime)%amv(i,&
      ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%amv(i, ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,19) = global_shared_buf(ie,19)+              &
(gs_obs(itime)%amv(i,ie)%u%val+tl_obs(itime)%amv(i,ie)%u%val-obs(itime)%amv(i,ie)%u%val)**2.d0/(obs(itime)%amv(i,ie)%u%error)
             global_shared_buf(ie,21) = global_shared_buf(ie,21)+1.d0
           endif
           if ((obs(itime)%amv(i, ie)%v%val.ne.sdmiss).and.(gs_obs(itime)%amv(i,&
      ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%amv(i, ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,20) = global_shared_buf(ie,20)+              &
(gs_obs(itime)%amv(i,ie)%v%val+tl_obs(itime)%amv(i,ie)%v%val-obs(itime)%amv(i,ie)%v%val)**2.d0/(obs(itime)%amv(i,ie)%v%error)
             global_shared_buf(ie,21) = global_shared_buf(ie,21)+1.d0
           endif
         enddo
       enddo
...
! scatwind observation
     if (da_scatwind) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%nscatwind_e(ie)
           if ((obs(itime)%scatwind(i,                                         &
 ie)%u%val.ne.sdmiss).and.(gs_obs(itime)%scatwind(i, ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%scatwind(i, ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,22) = global_shared_buf(ie,22)+              &
(gs_obs(itime)%scatwind(i,ie)%u%val+tl_obs(itime)%scatwind(i,ie)%u%val-obs(itime)%scatwind(i,ie)%u%val)**2.d0/(obs(itime)%scatwind(i,ie)%u%error)
             global_shared_buf(ie,24) = global_shared_buf(ie,24)+1.d0
           endif
           if ((obs(itime)%scatwind(i,                                         &
 ie)%v%val.ne.sdmiss).and.(gs_obs(itime)%scatwind(i, ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%scatwind(i, ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,23) = global_shared_buf(ie,23)+              &
(gs_obs(itime)%scatwind(i,ie)%v%val+tl_obs(itime)%scatwind(i,ie)%v%val-obs(itime)%scatwind(i,ie)%v%val)**2.d0/(obs(itime)%scatwind(i,ie)%v%error)
             global_shared_buf(ie,24) = global_shared_buf(ie,24)+1.d0
           endif
         enddo
       enddo
...
! amsua observation
     if (da_amsua) then
       do j = 1, obsmet(itime)%namsuach
         do ie = nets, nete
           do i = 1, obsmet(itime)%namsua_e(ie)
             if ((obs(itime)%amsua(i, j,                                       &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%amsua(i, j, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%amsua(i, j, ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,25) = global_shared_buf(ie,25)+            &
(gs_obs(itime)%amsua(i,j,ie)%t%val+tl_obs(itime)%amsua(i,j,ie)%t%val-obs(itime)%amsua(i,j,ie)%tb%val)**2.d0/(obs(itime)%amsua(i,j,ie)%tb%error)
...
! gpsro observation
     if (da_gpsro) then
       do ie = nets, nete
         do i = 1, obsmet(itime)%ngpsro_e(ie)
           if ((obs(itime)%gpsro(i,                                            &
 ie)%ba%val.ne.sdmiss).and.(gs_obs(itime)%gpsro(i, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%gpsro(i, ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,27) = global_shared_buf(ie,27)+              &
(gs_obs(itime)%gpsro(i,ie)%t%val+tl_obs(itime)%gpsro(i,ie)%t%val-obs(itime)%gpsro(i,ie)%ba%val)**2.d0/(obs(itime)%gpsro(i,ie)%ba%error)
             global_shared_buf(ie,29) = global_shared_buf(ie,29)+1.d0
           endif
           if ((obs(itime)%gpsro(i,                                            &
 ie)%ba%val.ne.sdmiss).and.(gs_obs(itime)%gpsro(i, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%gpsro(i, ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,28) = global_shared_buf(ie,28)+              &
(gs_obs(itime)%gpsro(i,ie)%q%val+tl_obs(itime)%gpsro(i,ie)%q%val-obs(itime)%gpsro(i,ie)%ba%val)**2.d0/(obs(itime)%gpsro(i,ie)%ba%error)
             global_shared_buf(ie,29) = global_shared_buf(ie,29)+1.d0
           endif
         enddo
       enddo
...
! iasi observation
     if (da_iasi) then
       do j = 1, obsmet(itime)%niasich
         do ie = nets, nete
           do i = 1, obsmet(itime)%niasi_e(ie)
             if ((obs(itime)%iasi(i, j,                                        &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%iasi(i, j, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%iasi(i, j, ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,30) = global_shared_buf(ie,30)+            &
(gs_obs(itime)%iasi(i,j,ie)%t%val+tl_obs(itime)%iasi(i,j,ie)%t%val-obs(itime)%iasi(i,j,ie)%tb%val)**2.d0/(obs(itime)%iasi(i,j,ie)%tb%error)
               global_shared_buf(ie,32) = global_shared_buf(ie,32)+1.d0
             endif
             if ((obs(itime)%iasi(i, j,                                        &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%iasi(i, j, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%iasi(i, j, ie)%q%val.ne.sdmiss)) then
               global_shared_buf(ie,31) = global_shared_buf(ie,31)+            &
(gs_obs(itime)%iasi(i,j,ie)%q%val+tl_obs(itime)%iasi(i,j,ie)%q%val-obs(itime)%iasi(i,j,ie)%tb%val)**2.d0/(obs(itime)%iasi(i,j,ie)%tb%error)
               global_shared_buf(ie,32) = global_shared_buf(ie,32)+1.d0
             endif
           enddo
         enddo
...
! cris observation
     if (da_cris) then
       do j = 1, obsmet(itime)%ncrisch
         do ie = nets, nete
           do i = 1, obsmet(itime)%ncris_e(ie)
             if ((obs(itime)%cris(i, j,                                        &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%cris(i, j, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%cris(i, j, ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,33) = global_shared_buf(ie,33)+            &
(gs_obs(itime)%cris(i,j,ie)%t%val+tl_obs(itime)%cris(i,j,ie)%t%val-obs(itime)%cris(i,j,ie)%tb%val)**2.d0/(obs(itime)%cris(i,j,ie)%tb%error)
               global_shared_buf(ie,35) = global_shared_buf(ie,35)+1.d0
             endif
             if ((obs(itime)%cris(i, j,                                        &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%cris(i, j, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%cris(i, j, ie)%q%val.ne.sdmiss)) then
               global_shared_buf(ie,34) = global_shared_buf(ie,34)+            &
(gs_obs(itime)%cris(i,j,ie)%q%val+tl_obs(itime)%cris(i,j,ie)%q%val-obs(itime)%cris(i,j,ie)%tb%val)**2.d0/(obs(itime)%cris(i,j,ie)%tb%error)
               global_shared_buf(ie,35) = global_shared_buf(ie,35)+1.d0
             endif
           enddo
         enddo
...
! atms observation
     if (da_atms) then
       do j = 1, obsmet(itime)%natmsch
         do ie = nets, nete
           do i = 1, obsmet(itime)%natms_e(ie)
             if ((obs(itime)%atms(i, j,                                        &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%atms(i, j, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%atms(i, j, ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,36) = global_shared_buf(ie,36)+            &
(gs_obs(itime)%atms(i,j,ie)%t%val+tl_obs(itime)%atms(i,j,ie)%t%val-obs(itime)%atms(i,j,ie)%tb%val)**2.d0/(obs(itime)%atms(i,j,ie)%tb%error)
...
! atmswv observation
     if (da_atmswv) then
       do j = 1, obsmet(itime)%natmswvch
         do ie = nets, nete
           do i = 1, obsmet(itime)%natmswv_e(ie)
             if ((obs(itime)%atmswv(i, j,                                      &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%atmswv(i, j, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%atmswv(i, j, ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,38) = global_shared_buf(ie,38)+            &
(gs_obs(itime)%atmswv(i,j,ie)%t%val+tl_obs(itime)%atmswv(i,j,ie)%t%val-obs(itime)%atmswv(i,j,ie)%tb%val)**2.d0/(obs(itime)%atmswv(i,j,ie)%tb%error)
               global_shared_buf(ie,40) = global_shared_buf(ie,40)+1.d0
             endif
             if ((obs(itime)%atmswv(i, j,                                      &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%atmswv(i, j, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%atmswv(i, j, ie)%q%val.ne.sdmiss)) then
               global_shared_buf(ie,39) = global_shared_buf(ie,39)+            &
(gs_obs(itime)%atmswv(i,j,ie)%q%val+tl_obs(itime)%atmswv(i,j,ie)%q%val-obs(itime)%atmswv(i,j,ie)%tb%val)**2.d0/(obs(itime)%atmswv(i,j,ie)%tb%error)
               global_shared_buf(ie,40) = global_shared_buf(ie,40)+1.d0
             endif
           enddo
         enddo
...
! mhs observation
     if (da_mhs) then
       do j = 1, obsmet(itime)%nmhsch
         do ie = nets, nete
           do i = 1, obsmet(itime)%nmhs_e(ie)
             if ((obs(itime)%mhs(i, j,                                         &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%mhs(i, j, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%mhs(i, j, ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,41) = global_shared_buf(ie,41)+            &
(gs_obs(itime)%mhs(i,j,ie)%t%val+tl_obs(itime)%mhs(i,j,ie)%t%val-obs(itime)%mhs(i,j,ie)%tb%val)**2.d0/(obs(itime)%mhs(i,j,ie)%tb%error)
               global_shared_buf(ie,43) = global_shared_buf(ie,43)+1.d0
             endif
             if ((obs(itime)%mhs(i, j,                                         &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%mhs(i, j, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%mhs(i, j, ie)%q%val.ne.sdmiss)) then
               global_shared_buf(ie,42) = global_shared_buf(ie,42)+            &
(gs_obs(itime)%mhs(i,j,ie)%q%val+tl_obs(itime)%mhs(i,j,ie)%q%val-obs(itime)%mhs(i,j,ie)%tb%val)**2.d0/(obs(itime)%mhs(i,j,ie)%tb%error)
               global_shared_buf(ie,43) = global_shared_buf(ie,43)+1.d0
             endif
           enddo
         enddo
...
! csr observation
     if (da_csr) then
       do j = 1, obsmet(itime)%ncsrch
         do ie = nets, nete
           do i = 1, obsmet(itime)%ncsr_e(ie)
             if ((obs(itime)%csr(i, j,                                         &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%csr(i, j, ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%csr(i, j, ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,44) = global_shared_buf(ie,44)+            &
(gs_obs(itime)%csr(i,j,ie)%t%val+tl_obs(itime)%csr(i,j,ie)%t%val-obs(itime)%csr(i,j,ie)%tb%val)**2.d0/(obs(itime)%csr(i,j,ie)%tb%error)
               global_shared_buf(ie,46) = global_shared_buf(ie,46)+1.d0
             endif
             if ((obs(itime)%csr(i, j,                                         &
 ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%csr(i, j, ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%csr(i, j, ie)%q%val.ne.sdmiss)) then
               global_shared_buf(ie,45) = global_shared_buf(ie,45)+            &
(gs_obs(itime)%csr(i,j,ie)%q%val+tl_obs(itime)%csr(i,j,ie)%q%val-obs(itime)%csr(i,j,ie)%tb%val)**2.d0/(obs(itime)%csr(i,j,ie)%tb%error)
               global_shared_buf(ie,46) = global_shared_buf(ie,46)+1.d0
             endif
           enddo
         enddo
...
! 1-2. Jb
   tmp = 0.d0
   do wn = 1,zwn_nproc
     tmp = tmp+sum((x_eig(:,wn)-x_eig_acc(:,wn))**2.d0)+sum((x_eig_chi(:,wn)-  &
x_eig_acc_chi(:,wn))**2.d0)+sum((x_eig_tps(:,wn)-x_eig_acc_tps(:,wn))**2.d0)+sum((x_eig_q(:,wn)-x_eig_acc_q(:,wn))**2.d0)
   enddo
   do ismpl = 1,nsmpl
     do wn = 1, zwn_nproc_a
...
! 1-3. Communication and evaluation
   ...
   j_o = 0.5d0*(global_shared_sum(2)+global_shared_sum(3)+global_shared_sum(4)+&
global_shared_sum(5)+global_shared_sum(7)+global_shared_sum(8)+global_shared_sum(9)+global_shared_sum(10)+global_shared_sum(11)+global_shared_sum(13)+global_shared_sum(15)+global_shared_sum(16)+global_shared_sum(17)+global_shared_sum(19)+global_shared_sum(20)+global_shared_sum(22)+global_shared_sum(23)+global_shared_sum(25)+global_shared_sum(27)+global_shared_sum(28)+global_shared_sum(30)+global_shared_sum(31)+global_shared_sum(33)+global_shared_sum(34)+global_shared_sum(36)+global_shared_sum(38)+global_shared_sum(39)+global_shared_sum(41)+global_shared_sum(42)+global_shared_sum(44)+global_shared_sum(45)) ! csr
   if (singleobs) then
     jo_single = 0.5d0*(global_shared_sum(2)+global_shared_sum(3)+             &
                global_shared_sum(4)+global_shared_sum(5)+global_shared_sum(6))
   endif
   if (da_sonde) then
   ...
   if (da_sonde) then
     if (par%ismasterproc) write(iulog,*) '# of sonde observations = ',        &
                                                     dint(global_shared_sum(6))
   endif
   if (da_surface) then
     if (par%ismasterproc) write(iulog,*) '# of surface observations = ',      &
                                                    dint(global_shared_sum(12))
   endif
   if (da_bogus) then
     if (par%ismasterproc) write(iulog,*) '# of bogus observations = ',        &
                                                    dint(global_shared_sum(14))
   endif
   if (da_aircraft) then
     if (par%ismasterproc) write(iulog,*) '# of aircraft observations = ',     &
                                                    dint(global_shared_sum(18))
   endif
   if (da_amv) then
     if (par%ismasterproc) write(iulog,*) '# of amv observations = ',          &
                                                    dint(global_shared_sum(21))
   endif
   if (da_scatwind) then
     if (par%ismasterproc) write(iulog,*) '# of scatwind observations = ',     &
                                                    dint(global_shared_sum(24))
   endif
   if (da_amsua) then
     if (par%ismasterproc) write(iulog,*) '# of amsua observations = ',        &
                                                    dint(global_shared_sum(26))
   endif
   if (da_gpsro) then
     if (par%ismasterproc) write(iulog,*) '# of gpsro observations = ',        &
                                                    dint(global_shared_sum(29))
   endif
   if (da_iasi) then
     if (par%ismasterproc) write(iulog,*) '# of iasi observations = ',         &
                                                    dint(global_shared_sum(32))
   endif
   if (da_cris) then
     if (par%ismasterproc) write(iulog,*) '# of cris observations = ',         &
                                                    dint(global_shared_sum(35))
   endif
   if (da_atms) then
     if (par%ismasterproc) write(iulog,*) '# of atms observations = ',         &
                                                    dint(global_shared_sum(37))
   endif
   if (da_atmswv) then
     if (par%ismasterproc) write(iulog,*) '# of atmswv observations = ',       &
                                                    dint(global_shared_sum(40))
   endif
   if (da_mhs) then
     if (par%ismasterproc) write(iulog,*) '# of mhs observations = ',          &
                                                    dint(global_shared_sum(43))
   endif
   if (da_csr) then
     if (par%ismasterproc) write(iulog,*) '# of csr observations = ',          &
                                                    dint(global_shared_sum(46))
   endif
   return
   end subroutine cal_cost_func
!===============================================================================
!--------- modified for alpha (HJS)
   subroutine eigtospec(l_zwn_nproc, l_zwn2, l_zwn_blength, l_zwn_bstart, eigen,&
                                             eigen_chi, eigen_tps, eigen_q, sp)
!=============================================================================
! A. Declaration
!=============================================================================
! 2. Variables
...
!
   ...
   integer(int_kind), dimension(:), allocatable, intent(in   ) :: l_zwn_blength,&
                                                                   l_zwn_bstart
...
!
   ...
   do wn = 1,l_zwn_nproc
     do k = 1, neig
       sp_tmp(1:nlev,wn) = sp_tmp(1:nlev,wn)+bcov_sp(1:nlev,k,wn)*dsqrt(eigv(k,&
                                                               wn))*eigen(k,wn)
     enddo
     do k = 1, neig_chi
       sp_tmp(nlev+1:2*nlev,wn) = sp_tmp(nlev+1:2*nlev,wn)+bcov_sp_chi(1:nlev,k,&
                                      wn)*dsqrt(eigv_chi(k,wn))*eigen_chi(k,wn)
     enddo
     do k = 1, neig_tps
       sp_tmp(2*nlev+1:3*nlev,wn) = sp_tmp(2*nlev+1:3*nlev,wn)+                &
                 bcov_sp_tps(1:nlev,k,wn)*dsqrt(eigv_tps(k,wn))*eigen_tps(k,wn)
       sp_tmp(4*nlev+1,wn) = sp_tmp(4*nlev+1,wn)+bcov_sp_tps(nlev+1,k,         &
                                      wn)*dsqrt(eigv_tps(k,wn))*eigen_tps(k,wn)
     enddo
     do k = 1, neig_q
       sp_tmp(3*nlev+1:4*nlev,wn) = sp_tmp(3*nlev+1:4*nlev,wn)+bcov_sp_q(:,k,  &
                                          wn)*dsqrt(eigv_q(k,wn))*eigen_q(k,wn)
     enddo
   enddo
   ierr = timingstop('Run beforebcast_fromeigtospec')
   ierr = timingstart('Run bcast4spectoctrl_fromeigtospec')
   call mpi_allgatherv(sp_tmp,nlevar*l_zwn_nproc,par_double_precision,sp,      &
   nlevar*l_zwn_blength,nlevar*l_zwn_bstart,par_double_precision,par%comm,ierr)
   ierr = timingstop('Run bcast4spectoctrl_fromeigtospec')
   return
   end subroutine eigtospec
! Desinged for alpha-variable
   subroutine eigtospecens(l_zwn_nproc, l_zwn2, l_zwn_blength, l_zwn_bstart,   &
                                                                   eigen_a, sp)
!=============================================================================
! A. Declaration
!=============================================================================
...
!       END DO
     do k = 1, nlev+1
       sp_tmp(2*nlev+1:3*nlev,wn) = sp_tmp(2*nlev+1:3*nlev,wn)+bcov_sp_a(1:nlev,&
                                           k)*dsqrt(eigv_a(k,wn))*eigen_a(k,wn)
       sp_tmp(4*nlev+1,wn) = sp_tmp(4*nlev+1,wn)+bcov_sp_a(nlev+1,             &
                                           k)*dsqrt(eigv_a(k,wn))*eigen_a(k,wn)
     enddo
...
!===============================================================================
   subroutine tlmeigtoobs(l_gs_md, tl_eigen, tl_eigen_chi, tl_eigen_tps,       &
                                              tl_eigen_q, tl_eigen_a, l_tl_obs)
!=============================================================================
! A. Declaration
!=============================================================================
! 1. Local modules
...
!=============================================================================
!
   call tlmeigtomodel(l_gs_md(:,:,:,:,:),tl_eigen,tl_eigen_chi,tl_eigen_tps,   &
                                        tl_eigen_q,tl_eigen_a,tl_md(:,:,:,:,:))
   do itime = 1,ntime
     ierr = timingstart('Run tlmmodeltoobs_fromtlmeigtoobs')
     call tlmmodeltoobs(itime,l_gs_md(:,:,:,:,itime),tl_md(:,:,:,:,itime),     &
                                                         l_tl_obs(itime)) ! out
...
!> @param[out] tl_sd,tl_sf
!===============================================================================
   subroutine tlmeigtomodel(l_gs_md, tl_eigen, tl_eigen_chi, tl_eigen_tps,     &
                                                 tl_eigen_q, tl_eigen_a, tl_md)
!=============================================================================
! A. Declaration
!=============================================================================
...
!
   ...
   call eigtospec(zwn_nproc,zwn2,zwn_blength,zwn_bstart,tl_eigen,tl_eigen_chi, &
                                                 tl_eigen_tps,tl_eigen_q,tl_sp)
...
! Add Ensemble Part
   do ismpl = 1,nsmpl
     ...
     call eigtospecens(zwn_nproc_a,zwn2_a,zwn_blength_a,zwn_bstart_a,          &
                                            tl_eigen_a(:,:,ismpl),tl_sp_a(:,:))
...
!---------------Parameter Transform-------------------
     ...
     call tlmctrltomodel(l_gs_md(:,:,:,:,itime),bg_uv_rot(:,:,:,:,:,itime),    &
                              tl_ct_hyb_ep(:,:,:,:),tl_md(:,:,:,:,itime)) ! out
...
!===============================================================================
   subroutine adjeigtoobs(l_gs_md, l_ad_obs, ad_eigen, ad_eigen_chi,           &
                                          ad_eigen_tps, ad_eigen_q, ad_eigen_a)
!=============================================================================
! A. Declaration
!=============================================================================
! 1. Local modules
...
! 2-2. Input/Output
   ...
   real(real_kind), dimension(neig, zwn_nproc),                                &
...
!=============================================================================
     ...
     call adjmodeltoobs(itime,l_gs_md(:,:,:,:,itime),l_ad_obs(itime),ad_md(:,:,&
                                                            :,:,itime)) ! inout
     ierr = timingstop('Run adjmodeltoobs_fromadjeigtoobs')
   enddo ! itime
   call adjeigtomodel(l_gs_md(:,:,:,:,:),ad_md(:,:,:,:,:),ad_eigen,ad_eigen_chi,&
                                            ad_eigen_tps,ad_eigen_q,ad_eigen_a)
   return
...
!> @param[inout] ad_sd,ad_eigen3d,ad_eigen2d
!===============================================================================
   subroutine adjeigtomodel(l_gs_md, ad_md, ad_eigen, ad_eigen_chi,            &
                                          ad_eigen_tps, ad_eigen_q, ad_eigen_a)
!=============================================================================
! A. Declaration
!=============================================================================
...
!ad_ct_hyb(:,:,:,:,itime)  = 0.D0
     ...
     call adjctrltomodel(l_gs_md(:,:,:,:,itime),bg_uv_rot(:,:,:,:,:,itime),    &
                            ad_md(:,:,:,:,itime),ad_ct_hyb_ep(:,:,:,:)) ! inout
!up
...
!
   do ismpl = nsmpl, 1,-1
     ...
     do itime = ntime,1,-1
       do k = nlevar, 1,-1
         ad_ct_a(:,k) = ad_ct_a(:,k)+dsqrt(b_ens_v(k))*bg_smpl(:,k,itime,      &
                                                    ismpl)*ad_ct_hyb(:,k,itime)
       enddo
     enddo
     ad_sp_a = 0.d0
     ierr = timingstart('Run adjspectoctrl_fromadjeigtomodel')
     call adjspectoctrl_opt(zwn2_a,zwn_nproc_a,zwn_iproc_a,zwn_bstart_a,       &
                                                 zwn_blength_a,ad_ct_a,ad_sp_a)
...
!
   ...
   ad_sp = 0.d0
   ierr = timingstart('Run adjspectoctrl_fromadjeigtomodel')
   call adjspectoctrl_opt(zwn2,zwn_nproc,zwn_iproc,zwn_bstart,zwn_blength,ad_ct,&
                                                                         ad_sp)
   ierr = timingstop('Run adjspectoctrl_fromadjeigtomodel')
   ierr = timingstart('Run afteradjspectoctrl_fromadjeigtomodel')
   call adjeigtospec(zwn_nproc,ad_sp,ad_eigen,ad_eigen_chi,ad_eigen_tps,       &
                                                                    ad_eigen_q)
   ierr = timingstop('Run afteradjspectoctrl_fromadjeigtomodel')
!------------------------------------------------------------------------
   return
...
!===============================================================================
   subroutine adjeigtospec(l_zwn_nproc, ad_sp, ad_eigen, ad_eigen_chi,         &
                                                      ad_eigen_tps, ad_eigen_q)
!=============================================================================
! A. Declaration
!=============================================================================
! 1. Local modules
...
! 2-2. Input/Output
   ...
   real(real_kind), dimension(neig_chi, l_zwn_nproc),                          &
                                                  intent(inout) :: ad_eigen_chi
   real(real_kind), dimension(neig_tps, l_zwn_nproc),                          &
...
!
   do wn = 1, l_zwn_nproc
     do k = 1, neig
       ad_eigen(k,wn) = dsqrt(eigv(k,wn))*sum(bcov_sp(1:nlev,k,wn)*ad_sp(1:nlev,&
                                                                           wn))
! Check
!IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     enddo
     do k = 1, neig_chi
       ad_eigen_chi(k,wn) = dsqrt(eigv_chi(k,wn))*sum(bcov_sp_chi(:,k,         &
                                                   wn)*ad_sp(nlev+1:2*nlev,wn))
! Check
!IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     enddo
     do k = 1, neig_tps
       ad_eigen_tps(k,wn) = dsqrt(eigv_tps(k,wn))*(sum(bcov_sp_tps(1:nlev,k,   &
    wn)*ad_sp(2*nlev+1:3*nlev,wn))+bcov_sp_tps(nlev+1,k,wn)*ad_sp(4*nlev+1,wn))
! Check
!IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     enddo
     do k = 1, neig_q
       ad_eigen_q(k,wn) = dsqrt(eigv_q(k,wn))*sum(bcov_sp_q(:,k,               &
                                                 wn)*ad_sp(3*nlev+1:4*nlev,wn))
! Check
!IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     enddo
   enddo
...
!
   do wn = l_zwn_nproc, 1,-1
     ...
     ad_sp(2*nlev+1:3*nlev,wn) = ad_sp(2*nlev+1:3*nlev,wn)+ad_sp(nlev+1:2*nlev,&
                                                                            wn)
     ad_sp(nlev+1:2*nlev,wn) = 0.d0
     ad_sp(2*nlev+1:3*nlev,wn) = ad_sp(2*nlev+1:3*nlev,wn)+ad_sp(3*nlev+       &
                                                                   1:4*nlev,wn)
...
!       END DO
     do k = 1,nlev+1
       ad_eigen_a(k,wn) = dsqrt(eigv_a(k,wn))*(sum(bcov_sp_a(1:nlev,           &
          k)*ad_sp(2*nlev+1:3*nlev,wn))+bcov_sp_a(nlev+1,k)*ad_sp(4*nlev+1,wn))
!          ! Check
!          !IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     enddo
   enddo
