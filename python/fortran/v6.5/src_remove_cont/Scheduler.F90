...
!#endif
  ...
  subroutine MessageStats(nlyr)
    ...
    real(kind=real_kind) :: Time_comm1,Time_commf,          Time_comm2,Time_comm3
    ...
    integer(kind=8) :: imin_volume,          imax_volume
    ...
    if(npart .gt. 1) then
	  ...
	  !-----------------------------------------------------------
	     ...
	     !---------------------------------------
             if(filter_freq > 0) then 
                Time_commf   = (bndryf(indx_comm(1))%latency                       + bndryf(indx_comm(1))%bandwidth)/real(filter_freq,kind=real_kind)
