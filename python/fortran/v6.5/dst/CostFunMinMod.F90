...
!-------------------------------------------------------------------------------
   ...
   use dimensions, only: np, nlev, nelemd, nets, nete, nvar3d, nvar2d, nlevar, zwn2, zwn2_a, neig, neig_chi, neig_tps, neig_q, zwn_nproc, zwn_iproc, zwn_nproc_a, zwn_iproc_a, ntime, nt_itime ! itime for nature
...
!errvar,                &
!
   errvar_up,bg_eig,bcov_sp,eigv,bcov_sp_chi,eigv_chi,bcov_sp_tps,eigv_tps,bcov_sp_q,eigv_q,bcov_sp_a,eigv_a,nsmpl,cv_opt_hum,is_moist_mr,exst_nt,bg_smpl,b_clim_v,b_ens_v
...
! 2. Public variables
   ...
   type(obsbg_type), dimension(:), allocatable, public :: gs_obs, ad_obs, tl_obs, bg_obs, an_obs
...
! 2-2. Local
   ...
   real(real_kind), dimension(neig_chi, zwn_nproc) :: b_eig_chi, r_eig_chi, p_eig_chi, ap_eig_chi
   real(real_kind), dimension(neig_tps, zwn_nproc) :: b_eig_tps, r_eig_tps, p_eig_tps, ap_eig_tps
   real(real_kind), dimension(neig_q, zwn_nproc) :: b_eig_q, r_eig_q, p_eig_q, ap_eig_q
   real(real_kind), dimension(nlev+1, zwn_nproc_a, nsmpl) :: b_eig_a, r_eig_a, p_eig_a, ap_eig_a
...
#define Check_HxmObs_on
   ...
   if (par%ismasterproc) write(iulog,*) 'Min !==========================================================================
...
! 2. Initialize rsold
   ...
   call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,r_eig,r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,r_eig,r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,rsold)
...
!       IF (par%ismasterproc) WRITE(iulog,*)  x_eig(:,1)
     ...
     call par_reduce(tmpmax,maxv,1,par_max)
     call par_reduce(tmpmin,minv,1,par_min)
     if (par%ismasterproc) write(iulog,*) 'Min ! if (iter==1.or.mod(iter,10).eq.0) then
...
!
   ...
   do itime = 1,ntime
     ...
     call tlmmodeltoobs(itime,l_gs_md(:,:,:,:,itime),tl_md(:,:,:,:,itime),l_tl_obs(itime)) ! out
...
!---------------Parameter Transform-------------------
     ...
     call tlmctrltomodel(l_gs_md(:,:,:,:,itime),bg_uv_rot(:,:,:,:,:,itime),tl_ct_hyb_ep(:,:,:,:),tl_md(:,:,:,:,itime)) ! out
...
!===============================================================================
   subroutine adjeigtoobs(l_gs_md, l_ad_obs, ad_eigen, ad_eigen_chi, ad_eigen_tps, ad_eigen_q, ad_eigen_a)
!=============================================================================
! A. Declaration
!=============================================================================
! 1. Local modules
...
!=============================================================================
     ...
     call adjmodeltoobs(itime,l_gs_md(:,:,:,:,itime),l_ad_obs(itime),ad_md(:,:,:,:,itime)) ! inout
...
!> @param[inout] ad_sd,ad_eigen3d,ad_eigen2d
!===============================================================================
   subroutine adjeigtomodel(l_gs_md, ad_md, ad_eigen, ad_eigen_chi, ad_eigen_tps, ad_eigen_q, ad_eigen_a)
!=============================================================================
! A. Declaration
!=============================================================================
! 1. Local modules
...
!ad_ct_hyb(:,:,:,:,itime)  = 0.D0
     ...
     call adjctrltomodel(l_gs_md(:,:,:,:,itime),bg_uv_rot(:,:,:,:,:,itime),ad_md(:,:,:,:,itime),ad_ct_hyb_ep(:,:,:,:)) ! inout
!up
