...
!-------------------------------------------------------------------------------
   ...
   use dimensions, only: np, nlev, ne, nvar3d, nvar2d, nlevar, zwn, zwn2, zwn_nproc, zwn_iproc, zwn_a, zwn2_a, ! wavenumber of alpha
!
   zwn_nproc_a,zwn_iproc_a,! for alpha
   reg_nproc,reg_iproc,psi_nproc,psi_iproc,chi_nproc,chi_iproc,tps_nproc,tps_iproc,hum_nproc,hum_iproc,neig,! # of eigenmodes for psi
   neig_chi,! # of eigenmodes for chi
   neig_tps,! # of eigenmodes for t,ps
   neig_q,! # of eigenmodes for q
   nelemd,nelem,nets,nete,nk,ntime,! # of time bins in 4d
...
!             becFile                 ! Background error covariance file name
   ...
   character(len=len_path), public :: naturefile, ! nature file name
!
   becfile ! background error covariance file name
   character(len=len_arg), public :: model_name, ! model which generates the background
!
...
!REAL(KIND=real_kind), DIMENSION(:,:,:,:,:,:), ALLOCATABLE, & ! extended to 4D
   real(real_kind), dimension(:,:,:,:), allocatable, ! extended to 4d
   public :: bg_smpl
   real(real_kind), dimension(:,:,:,:,:,:), allocatable, public :: bg_uv_rot ! rotational wind in background field
   real(real_kind), dimension(:,:,:,:,:), allocatable, public :: bg_md, ! background
!
   pmid
   real(real_kind), dimension(:,:,:,:), allocatable, public :: nt_org, ! nature
!
   errvar,! variance of backgrouond error
   z_org,! geopotential height for each layer
   bg_md_q,! specific humidity for background
   pint ! interface level pressure
   real(real_kind), dimension(:,:,:), allocatable, public :: bcov_sp, ! eigenfunction of backerrcor
!
   bcov_sp_chi,bcov_sp_tps,bcov_sp_q,phis,zsfc,rl_org
   real(real_kind), dimension(:,:,:,:), allocatable, public :: u10m, ! 10m zonal wind
!
   ...
   sfctype
   real(real_kind), dimension(:,:), allocatable, public :: eigv, ! eigenvalue of backerrcor
!
   eigv_chi,eigv_tps,eigv_q,eigv_a,! for alpha-variable
