...
!-------------------------------------------------------------------------------
   ...
   use dimensions, only: np, nlev, nelemd, nets, nete, nvar3d, nvar2d, nlevar, &
                zwn2, zwn2_a, neig, neig_chi, neig_tps, neig_q, zwn_nproc,     &
                          zwn_iproc, zwn_nproc_a, zwn_iproc_a, ntime, nt_itime ! itime for nature
   use kiapsparallel, only: par=>kim_par, hybrid=>kim_hybrid,                  &
                global_shared_buf, global_shared_sum, par_double_precision,    &
                                       par_character, par_sum, par_max, par_min
   ...
   use backgroundmod, only: temptom, bg_md, nt_org, bg_uv_rot, errvar_up,      &
                bg_eig, bcov_sp, eigv, bcov_sp_chi, eigv_chi, bcov_sp_tps,     &
                eigv_tps, bcov_sp_q, eigv_q, bcov_sp_a, eigv_a, nsmpl,         &
                   cv_opt_hum, is_moist_mr, exst_nt, bg_smpl, b_clim_v, b_ens_v
   use observationmod, only: obsmet, sdmiss, obcheck, obcheck_outer,           &
                crit_omb_amsua, crit_omb_gpsro, crit_omb_iasi, crit_omb_cris,  &
                crit_omb_atms, crit_omb_atmswv, crit_omb_mhs, crit_omb_csr,    &
                singleobs, da_sonde, da_surface, da_bogus, da_aircraft, da_amv,&
                da_scatwind, da_amsua, da_gpsro, da_iasi, da_cris, da_atms,    &
                                 da_atmswv, da_mhs, da_csr, rttovused, roppused
   ...
   use makebmod, only: makebsingle, makebsonde, makebsurface, makebaircraft,   &
                makebamv, makebscatwind, makebamsua, makebgpsro, makebiasi,    &
                makebcris, makebatms, makebmhs, makebcsr, makebbogus,          &
                                                                    makebatmswv
   use multiplyrmod, only: multiplyrsingle, multiplyrsonde, multiplyrsurface,  &
                multiplyraircraft, multiplyramv, multiplyrscatwind,            &
                multiplyramsua, multiplyrgpsro, multiplyriasi, multiplyrcris,  &
                multiplyratms, multiplyrmhs, multiplyrcsr, multiplyrbogus,     &
                                                                multiplyratmswv
...
! 2. Initialize rsold
   ...
   call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,r_eig,&
               r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,r_eig,r_eig_chi,r_eig_tps,  &
                                                         r_eig_q,r_eig_a,rsold)
...
! 3-1. Calculate alpha,x,r,and rsnew
     ...
     call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,    &
               p_eig,p_eig_chi,p_eig_tps,p_eig_q,p_eig_a,ap_eig,ap_eig_chi,    &
                                              ap_eig_tps,ap_eig_q,ap_eig_a,pap)
...
! r_j : current  residual
       ...
       call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,  &
               ...
               r_eig_chi_history(:,:,k),r_eig_tps_history(:,:,k),              &
                        r_eig_q_history(:,:,k),r_eig_q_history_a(:,:,:,k),rjrk)
     ...
     call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,    &
               r_eig,r_eig_chi,r_eig_tps,r_eig_q,r_eig_a,r_eig,r_eig_chi,      &
                                               r_eig_tps,r_eig_q,r_eig_a,rsnew)
...
! 2-2. Local
   ...
   real(real_kind) :: maxsdu, maxsdv, maxsdt, maxsdq, maxsfu, maxsfv, maxsft,  &
                maxsfq, maxsfp, minsdu, minsdv, minsdt, minsdq, minsfu, minsfv,&
                minsft, minsfq, minsfp, maxaru, maxarv, maxart, minaru, minarv,&
                minart, maxamvu, maxamvv, minamvu, minamvv, maxscatwindu,      &
                       maxscatwindv, minscatwindu, minscatwindv, maxbgp, minbgp
     ...
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
                ie)%u%val.ne.sdmiss).and.(obs(itime)%single(i,                 &
...
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single u:',obs(itime)%single(i,ie)%u%val,                   &
               ...
               gs_obs(itime)%single(i,ie)%u%val,sqrt(obs(itime)%single(i,      &
                                                                   ie)%u%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
                ie)%v%val.ne.sdmiss).and.(obs(itime)%single(i,                 &
...
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single v:',obs(itime)%single(i,ie)%v%val,                   &
               ...
               gs_obs(itime)%single(i,ie)%v%val,sqrt(obs(itime)%single(i,      &
                                                                   ie)%v%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
                ie)%t%val.ne.sdmiss).and.(obs(itime)%single(i,                 &
...
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single t:',obs(itime)%single(i,ie)%t%val,                   &
               ...
               gs_obs(itime)%single(i,ie)%t%val,sqrt(obs(itime)%single(i,      &
                                                                   ie)%t%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
                ie)%q%val.ne.sdmiss).and.(obs(itime)%single(i,                 &
...
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single q:',obs(itime)%single(i,ie)%q%val,                   &
               ...
               gs_obs(itime)%single(i,ie)%q%val,sqrt(obs(itime)%single(i,      &
                                                                   ie)%q%error)
!      END IF
         endif
       enddo
       do i = 1, obsmet(itime)%nsg_e(ie)
         if ((gs_obs(itime)%single(i,                                          &
                ie)%ps%val.ne.sdmiss).and.(obs(itime)%single(i,                &
...
!      IF ( par%ismasterproc .and. i == 1 ) THEN
           print*,'Single ps:',obs(itime)%single(i,ie)%ps%val,                 &
               gs_obs(itime)%single(i,ie)%ps%val,obs(itime)%single(i,          &
               ie)%ps%val-gs_obs(itime)%single(i,ie)%ps%val,                   &
     ...
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsd_e(1, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
                ie)%u%val.ne.sdmiss).and.(obs(itime)%sonde(i,                  &
                                                     ie)%u%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde u:',obs(itime)%sonde(i,ie)%u%val,                   &
               ...
               gs_obs(itime)%sonde(i,ie)%u%val,sqrt(obs(itime)%sonde(i,        &
                                                                   ie)%u%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nsd_e(2, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
                ie)%v%val.ne.sdmiss).and.(obs(itime)%sonde(i,                  &
                                                     ie)%v%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde v:',obs(itime)%sonde(i,ie)%v%val,                   &
               ...
               gs_obs(itime)%sonde(i,ie)%v%val,sqrt(obs(itime)%sonde(i,        &
                                                                   ie)%v%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nsd_e(3, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
                ie)%t%val.ne.sdmiss).and.(obs(itime)%sonde(i,                  &
                                                     ie)%t%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde t:',obs(itime)%sonde(i,ie)%t%val,                   &
               ...
               gs_obs(itime)%sonde(i,ie)%t%val,sqrt(obs(itime)%sonde(i,        &
                                                                   ie)%t%error)
           endif
         endif
       enddo
       do i = 1, obsmet(itime)%nsd_e(4, ie)
         if ((gs_obs(itime)%sonde(i,                                           &
                ie)%q%val.ne.sdmiss).and.(obs(itime)%sonde(i,                  &
                                                     ie)%q%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Sonde q:',obs(itime)%sonde(i,ie)%q%val,                   &
               ...
               gs_obs(itime)%sonde(i,ie)%q%val,sqrt(obs(itime)%sonde(i,        &
                                                                   ie)%q%error)
           endif
         endif
       enddo
     ...
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((gs_obs(itime)%surface(i,                                         &
                ie)%u%val.ne.sdmiss).and.(obs(itime)%surface(i,                &
                                                     ie)%u%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Surface u:',obs(itime)%surface(i,ie)%u%val,               &
               gs_obs(itime)%surface(i,ie)%u%val,obs(itime)%surface(i,         &
               ie)%u%val-gs_obs(itime)%surface(i,ie)%u%val,                    &
                                         sqrt(obs(itime)%surface(i,ie)%u%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
                ie)%v%val.ne.sdmiss).and.(obs(itime)%surface(i,                &
                                                     ie)%v%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Surface v:',obs(itime)%surface(i,ie)%v%val,               &
               gs_obs(itime)%surface(i,ie)%v%val,obs(itime)%surface(i,         &
               ie)%v%val-gs_obs(itime)%surface(i,ie)%v%val,                    &
                                         sqrt(obs(itime)%surface(i,ie)%v%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
                ie)%t%val.ne.sdmiss).and.(obs(itime)%surface(i,                &
                                                     ie)%t%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Surface t:',obs(itime)%surface(i,ie)%t%val,               &
               gs_obs(itime)%surface(i,ie)%t%val,obs(itime)%surface(i,         &
               ie)%t%val-gs_obs(itime)%surface(i,ie)%t%val,                    &
                                         sqrt(obs(itime)%surface(i,ie)%t%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
                ie)%q%val.ne.sdmiss).and.(obs(itime)%surface(i,                &
                                                     ie)%q%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Surface q:',obs(itime)%surface(i,ie)%q%val,               &
               gs_obs(itime)%surface(i,ie)%q%val,obs(itime)%surface(i,         &
               ie)%q%val-gs_obs(itime)%surface(i,ie)%q%val,                    &
                                         sqrt(obs(itime)%surface(i,ie)%q%error)
           endif
         endif
         if ((gs_obs(itime)%surface(i,                                         &
                ie)%p%val.ne.sdmiss).and.(obs(itime)%surface(i,                &
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Surface ps:',obs(itime)%surface(i,ie)%p%val,              &
               gs_obs(itime)%surface(i,ie)%p%val,obs(itime)%surface(i,         &
               ie)%p%val-gs_obs(itime)%surface(i,ie)%p%val,                    &
     ...
     do ie = nets, nete
       do i = 1, obsmet(itime)%nbg_e(ie)
         if ((gs_obs(itime)%bogus(i,                                           &
                ie)%p%val.ne.sdmiss).and.(obs(itime)%bogus(i,                  &
                                                     ie)%p%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Surface_bg ps:',obs(itime)%bogus(i,ie)%p%val,             &
               ...
               gs_obs(itime)%bogus(i,ie)%p%val,sqrt(obs(itime)%bogus(i,        &
                                                                   ie)%p%error)
           endif
         endif
       enddo
     ...
     do ie = nets, nete
       do i = 1, obsmet(itime)%nar_e(1, ie)
         if ((gs_obs(itime)%aircraft(i,                                        &
                ie)%u%val.ne.sdmiss).and.(obs(itime)%aircraft(i,               &
                                                     ie)%u%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Aircraft u:',obs(itime)%aircraft(i,ie)%u%val,             &
               gs_obs(itime)%aircraft(i,ie)%u%val,obs(itime)%aircraft(i,       &
               ie)%u%val-gs_obs(itime)%aircraft(i,ie)%u%val,                   &
       ...
       do i = 1, obsmet(itime)%nar_e(2, ie)
         if ((gs_obs(itime)%aircraft(i,                                        &
                ie)%v%val.ne.sdmiss).and.(obs(itime)%aircraft(i,               &
                                                     ie)%v%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Aircraft v:',obs(itime)%aircraft(i,ie)%v%val,             &
               gs_obs(itime)%aircraft(i,ie)%v%val,obs(itime)%aircraft(i,       &
               ie)%v%val-gs_obs(itime)%aircraft(i,ie)%v%val,                   &
       ...
       do i = 1, obsmet(itime)%nar_e(3, ie)
         if ((gs_obs(itime)%aircraft(i,                                        &
                ie)%t%val.ne.sdmiss).and.(obs(itime)%aircraft(i,               &
                                                     ie)%t%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Aircraft t:',obs(itime)%aircraft(i,ie)%t%val,             &
               gs_obs(itime)%aircraft(i,ie)%t%val,obs(itime)%aircraft(i,       &
               ie)%t%val-gs_obs(itime)%aircraft(i,ie)%t%val,                   &
         ...
         if ((gs_obs(itime)%amv(i, ie)%u%val.ne.sdmiss).and.(obs(itime)%amv(i, &
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Amv u:',obs(itime)%amv(i,ie)%u%val,gs_obs(itime)%amv(i,   &
               ie)%u%val,obs(itime)%amv(i,ie)%u%val-gs_obs(itime)%amv(i,       &
                                   ie)%u%val,sqrt(obs(itime)%amv(i,ie)%u%error)
           endif
         endif
         if ((gs_obs(itime)%amv(i, ie)%v%val.ne.sdmiss).and.(obs(itime)%amv(i, &
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Amv v:',obs(itime)%amv(i,ie)%v%val,gs_obs(itime)%amv(i,   &
               ie)%v%val,obs(itime)%amv(i,ie)%v%val-gs_obs(itime)%amv(i,       &
                                   ie)%v%val,sqrt(obs(itime)%amv(i,ie)%v%error)
           endif
         endif
       enddo
     enddo
     ...
     do ie = nets, nete
       do i = 1, obsmet(itime)%nscatwind_e(ie)
         if ((gs_obs(itime)%scatwind(i,                                        &
                ie)%u%val.ne.sdmiss).and.(obs(itime)%scatwind(i,               &
                                                     ie)%u%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Scatwind u:',obs(itime)%scatwind(i,ie)%u%val,             &
               gs_obs(itime)%scatwind(i,ie)%u%val,obs(itime)%scatwind(i,       &
               ie)%u%val-gs_obs(itime)%scatwind(i,ie)%u%val,                   &
                                        sqrt(obs(itime)%scatwind(i,ie)%u%error)
           endif
         endif
         if ((gs_obs(itime)%scatwind(i,                                        &
                ie)%v%val.ne.sdmiss).and.(obs(itime)%scatwind(i,               &
                                                     ie)%v%val.ne.sdmiss)) then
           ...
           if (par%ismasterproc.and.i==1) then
             print*,'Scatwind v:',obs(itime)%scatwind(i,ie)%v%val,             &
               gs_obs(itime)%scatwind(i,ie)%v%val,obs(itime)%scatwind(i,       &
               ie)%v%val-gs_obs(itime)%scatwind(i,ie)%v%val,                   &
...
!-------------------------------------------------------------------------------
   use check_omb_mod, only: sondeobcheck, surfaceobcheck, aircraftobcheck,     &
                amvobcheck, scatwindobcheck, amsuaobcheck, gpsroobcheck,       &
                iasiobcheck, crisobcheck, atmsobcheck, atmswvobcheck,          &
                                                         mhsobcheck, csrobcheck
!
   integer(int_kind), intent(in   ) :: itime
   integer(int_kind) :: i, j, k, ie
!=================================================================
   ...
   if (da_amsua) then
     ...
     call amsuaobcheck(itime,obs(itime)%amsua%tb%val,                          &
               dsqrt(obs(itime)%amsua%tb%error),                               &
                          obsmet(itime)%amsuabg%amsuahdx%tb%val,crit_omb_amsua)
   endif
! gpsroOBCheck
   if (da_gpsro) then
     ...
     call gpsroobcheck(itime,obs(itime)%gpsro%ba%val,                          &
               dsqrt(obs(itime)%gpsro%ba%error),                               &
                          obsmet(itime)%gpsrobg%gpsrohdx%ba%val,crit_omb_gpsro)
   endif
! iasiOBCheck
   if (da_iasi) then
     ...
     call iasiobcheck(itime,obs(itime)%iasi%tb%val,                            &
               dsqrt(obs(itime)%iasi%tb%error),                                &
                             obsmet(itime)%iasibg%iasihdx%tb%val,crit_omb_iasi)
   endif
! crisOBCheck
   if (da_cris) then
     ...
     call crisobcheck(itime,obs(itime)%cris%tb%val,                            &
               dsqrt(obs(itime)%cris%tb%error),                                &
                             obsmet(itime)%crisbg%crishdx%tb%val,crit_omb_cris)
   endif
! atmsOBCheck
   if (da_atms) then
     ...
     call atmsobcheck(itime,obs(itime)%atms%tb%val,                            &
               dsqrt(obs(itime)%atms%tb%error),                                &
                             obsmet(itime)%atmsbg%atmshdx%tb%val,crit_omb_atms)
   endif
! atmswvOBCheck
   if (da_atmswv) then
     ...
     call atmswvobcheck(itime,obs(itime)%atmswv%tb%val,                        &
               dsqrt(obs(itime)%atmswv%tb%error),                              &
                       obsmet(itime)%atmswvbg%atmswvhdx%tb%val,crit_omb_atmswv)
   endif
! mhsOBCheck
   if (da_mhs) then
...
!
   ...
   real(real_kind) :: rmse_ua, rmse_va, rmse_ta, rmse_qa, rmse_psa, rmse_ub,   &
                rmse_vb, rmse_tb, rmse_qb, rmse_psb, rmse_sduo, rmse_sdvo,     &
                                                           rmse_sdto, rmse_sdqo
!
     ...
     do ie = nets, nete
       do i = 1, obsmet(itime)%nsg_e(ie)
         ...
         print*,'bg_obs(itime)%single(i,ie)%u%val,an_obs(itime)%single(i,      &
               ie)%u%val',bg_obs(itime)%single(i,ie)%u%val,                    &
                                               an_obs(itime)%single(i,ie)%u%val
         print*,'bg_obs(itime)%single(i,ie)%v%val,an_obs(itime)%single(i,      &
               ie)%v%val',bg_obs(itime)%single(i,ie)%v%val,                    &
                                               an_obs(itime)%single(i,ie)%v%val
         print*,'bg_obs(itime)%single(i,ie)%t%val,an_obs(itime)%single(i,      &
               ie)%t%val',bg_obs(itime)%single(i,ie)%t%val,                    &
                                               an_obs(itime)%single(i,ie)%t%val
         print*,'bg_obs(itime)%single(i,ie)%q%val,an_obs(itime)%single(i,      &
               ie)%q%val',bg_obs(itime)%single(i,ie)%q%val,                    &
                                               an_obs(itime)%single(i,ie)%q%val
         print*,'bg_obs(itime)%single(i,ie)%ps%val,an_obs(itime)%single(i,     &
               ie)%ps%val',bg_obs(itime)%single(i,ie)%ps%val,                  &
                                              an_obs(itime)%single(i,ie)%ps%val
       enddo
     enddo
   endif
...
! background
       do i = 1, obsmet(itime)%nsd_e(1, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%u%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i,               &
                                                     ie)%u%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsd_e(2, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%v%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i,               &
                                                     ie)%v%val.ne.sdmiss)) then
           global_shared_buf(ie,2) = global_shared_buf(ie,2)+                  &
       ...
       do i = 1, obsmet(itime)%nsd_e(3, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%t%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i,               &
                                                     ie)%t%val.ne.sdmiss)) then
           global_shared_buf(ie,3) = global_shared_buf(ie,3)+                  &
       ...
       do i = 1, obsmet(itime)%nsd_e(4, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%q%val.ne.sdmiss).and.(bg_obs(itime)%sonde(i,               &
                                                     ie)%q%val.ne.sdmiss)) then
           global_shared_buf(ie,4) = global_shared_buf(ie,4)+                  &
...
! analysis
       do i = 1, obsmet(itime)%nsd_e(1, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%u%val.ne.sdmiss).and.(an_obs(itime)%sonde(i,               &
                                                     ie)%u%val.ne.sdmiss)) then
           global_shared_buf(ie,5) = global_shared_buf(ie,5)+                  &
       ...
       do i = 1, obsmet(itime)%nsd_e(2, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%v%val.ne.sdmiss).and.(an_obs(itime)%sonde(i,               &
                                                     ie)%v%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsd_e(3, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%t%val.ne.sdmiss).and.(an_obs(itime)%sonde(i,               &
                                                     ie)%t%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsd_e(4, ie)
         if ((obs(itime)%sonde(i,                                              &
                ie)%q%val.ne.sdmiss).and.(an_obs(itime)%sonde(i,               &
                                                     ie)%q%val.ne.sdmiss)) then
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%u%val.ne.sdmiss).and.(bg_obs(itime)%surface(i,             &
                                                     ie)%u%val.ne.sdmiss)) then
           global_shared_buf(ie,1) = global_shared_buf(ie,1)+                  &
               (obs(itime)%surface(i,ie)%u%val-bg_obs(itime)%surface(i,        &
                                                                     ie)%u%val)
           global_shared_buf(ie,11) = global_shared_buf(ie,11)+1.d0
         endif
       enddo
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%v%val.ne.sdmiss).and.(bg_obs(itime)%surface(i,             &
                                                     ie)%v%val.ne.sdmiss)) then
           global_shared_buf(ie,2) = global_shared_buf(ie,2)+                  &
               (obs(itime)%surface(i,ie)%v%val-bg_obs(itime)%surface(i,        &
                                                                     ie)%v%val)
           global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
         endif
       enddo
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%t%val.ne.sdmiss).and.(bg_obs(itime)%surface(i,             &
                                                     ie)%t%val.ne.sdmiss)) then
           global_shared_buf(ie,3) = global_shared_buf(ie,3)+                  &
               (obs(itime)%surface(i,ie)%t%val-bg_obs(itime)%surface(i,        &
                                                                     ie)%t%val)
           global_shared_buf(ie,13) = global_shared_buf(ie,13)+1.d0
         endif
       enddo
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%q%val.ne.sdmiss).and.(bg_obs(itime)%surface(i,             &
                                                     ie)%q%val.ne.sdmiss)) then
           global_shared_buf(ie,4) = global_shared_buf(ie,4)+                  &
               (obs(itime)%surface(i,ie)%q%val-bg_obs(itime)%surface(i,        &
                                                                     ie)%q%val)
           global_shared_buf(ie,14) = global_shared_buf(ie,14)+1.d0
         endif
       enddo
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%p%val.ne.sdmiss).and.(bg_obs(itime)%surface(i,             &
                                                     ie)%p%val.ne.sdmiss)) then
           global_shared_buf(ie,5) = global_shared_buf(ie,5)+                  &
               (obs(itime)%surface(i,ie)%p%val-bg_obs(itime)%surface(i,        &
                                                                     ie)%p%val)
           global_shared_buf(ie,15) = global_shared_buf(ie,15)+1.d0
         endif
       enddo
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%u%val.ne.sdmiss).and.(an_obs(itime)%surface(i,             &
                                                     ie)%u%val.ne.sdmiss)) then
           global_shared_buf(ie,6) = global_shared_buf(ie,6)+                  &
               (obs(itime)%surface(i,ie)%u%val-an_obs(itime)%surface(i,        &
                                                                     ie)%u%val)
           global_shared_buf(ie,16) = global_shared_buf(ie,16)+1.d0
         endif
       enddo
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%v%val.ne.sdmiss).and.(an_obs(itime)%surface(i,             &
                                                     ie)%v%val.ne.sdmiss)) then
           global_shared_buf(ie,7) = global_shared_buf(ie,7)+                  &
               (obs(itime)%surface(i,ie)%v%val-an_obs(itime)%surface(i,        &
                                                                     ie)%v%val)
           global_shared_buf(ie,17) = global_shared_buf(ie,17)+1.d0
         endif
       enddo
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
                ie)%t%val.ne.sdmiss).and.(an_obs(itime)%surface(i,             &
                                                     ie)%t%val.ne.sdmiss)) then
           global_shared_buf(ie,8) = global_shared_buf(ie,8)+                  &
               (obs(itime)%surface(i,ie)%t%val-an_obs(itime)%surface(i,        &
                                                                     ie)%t%val)
           global_shared_buf(ie,18) = global_shared_buf(ie,18)+1.d0
         endif
       enddo
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
           ...
           global_shared_buf(ie,9) = global_shared_buf(ie,9)+                  &
               (obs(itime)%surface(i,ie)%q%val-an_obs(itime)%surface(i,        &
                                                                     ie)%q%val)
       ...
       do i = 1, obsmet(itime)%nsf_e(ie)
         if ((obs(itime)%surface(i,                                            &
           ...
           global_shared_buf(ie,10) = global_shared_buf(ie,10)+                &
               (obs(itime)%surface(i,ie)%p%val-an_obs(itime)%surface(i,        &
                                                                     ie)%p%val)
...
! background
       do i = 1, obsmet(itime)%nbg_e(ie)
         if ((obs(itime)%bogus(i,                                              &
                ie)%p%val.ne.sdmiss).and.(bg_obs(itime)%bogus(i,               &
                                                     ie)%p%val.ne.sdmiss)) then
           global_shared_buf(ie,1) = global_shared_buf(ie,1)+                  &
...
! analysis
       do i = 1, obsmet(itime)%nbg_e(ie)
         if ((obs(itime)%bogus(i,                                              &
                ie)%p%val.ne.sdmiss).and.(an_obs(itime)%bogus(i,               &
                                                     ie)%p%val.ne.sdmiss)) then
           global_shared_buf(ie,2) = global_shared_buf(ie,2)+                  &
...
! csr observation
   ...
   call innerproduct(neig,neig_chi,neig_tps,neig_q,zwn_nproc,zwn_nproc_a,b_eig,&
               b_eig_chi,b_eig_tps,b_eig_q,b_eig_a,ad_eig,ad_eig_chi,          &
                                              ad_eig_tps,ad_eig_q,ad_eig_a,tmp)
   if (par%ismasterproc) then
...
!-------------------------------------------------------------------------------
   subroutine innerproduct(neig, neig_chi, neig_tps, neig_q, zwn_nproc,        &
                zwn_nproc_a, vect1, vect1_chi, vect1_tps, vect1_q, vect1_a,    &
                   vect2, vect2_chi, vect2_tps, vect2_q, vect2_a, ddot_product)
!=============================================================================
! A. Declaration
!=============================================================================
! 1. Local modules
...
!IF (par%ismasterproc) WRITE(iulog,*) 'tmp 1= ', tmp1,k,zwn_nproc,jump
   ...
   do wn = j,zwn_nproc,jump
     tmp1 = tmp1+dot_product(vect1(:,wn),vect2(:,wn))+dot_product(vect1(:,wn+  &
               ...
               dot_product(vect1(:,wn+3),vect2(:,wn+3))+dot_product(vect1(:,wn+&
               4),vect2(:,wn+4))+dot_product(vect1_chi(:,wn),vect2_chi(:,wn))+ &
               ...
               dot_product(vect1_q(:,wn),vect2_q(:,wn))+dot_product(vect1_q(:, &
               wn+1),vect2_q(:,wn+1))+dot_product(vect1_q(:,wn+2),vect2_q(:,wn+&
               2))+dot_product(vect1_q(:,wn+3),vect2_q(:,wn+3))+               &
...
! 2-2. Local
   ...
   real(real_kind) :: rmse_an_u, rmse_an_v, rmse_an_t, rmse_an_q, rmse_an_ps,  &
                tmp, jo_single, jo_sonde, jo_surface, jo_bogus, jo_aircraft,   &
                jo_amv, jo_scatwind, jo_amsua, jo_gpsro, jo_iasi, jo_cris,     &
                                             jo_atms, jo_atmswv, jo_mhs, jo_csr
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%nsg_e(ie)
           if ((obs(itime)%single(i,                                           &
                ...
                ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%single(i,              &
                                                     ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,2) = global_shared_buf(ie,2)+                &
               (gs_obs(itime)%single(i,ie)%u%val+tl_obs(itime)%single(i,       &
               ie)%u%val-obs(itime)%single(i,                                  &
                             ie)%u%val)**2.d0/(obs(itime)%single(i,ie)%u%error)
           endif
           if ((obs(itime)%single(i,                                           &
                ie)%v%val.ne.sdmiss).and.(gs_obs(itime)%single(i,              &
                ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%single(i,              &
                                                     ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,3) = global_shared_buf(ie,3)+                &
               (gs_obs(itime)%single(i,ie)%v%val+tl_obs(itime)%single(i,       &
               ie)%v%val-obs(itime)%single(i,                                  &
                             ie)%v%val)**2.d0/(obs(itime)%single(i,ie)%v%error)
           endif
           if ((obs(itime)%single(i,                                           &
                ie)%t%val.ne.sdmiss).and.(gs_obs(itime)%single(i,              &
                ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%single(i,              &
                                                     ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,4) = global_shared_buf(ie,4)+                &
               (gs_obs(itime)%single(i,ie)%t%val+tl_obs(itime)%single(i,       &
               ie)%t%val-obs(itime)%single(i,                                  &
                             ie)%t%val)**2.d0/(obs(itime)%single(i,ie)%t%error)
           endif
           if ((obs(itime)%single(i,                                           &
                ie)%q%val.ne.sdmiss).and.(gs_obs(itime)%single(i,              &
                ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%single(i,              &
                                                     ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,5) = global_shared_buf(ie,5)+                &
               (gs_obs(itime)%single(i,ie)%q%val+tl_obs(itime)%single(i,       &
               ie)%q%val-obs(itime)%single(i,                                  &
                             ie)%q%val)**2.d0/(obs(itime)%single(i,ie)%q%error)
           endif
           if ((obs(itime)%single(i,                                           &
                ...
                ie)%ps%val.ne.sdmiss).and.(tl_obs(itime)%single(i,             &
                                                    ie)%ps%val.ne.sdmiss)) then
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+                &
               (gs_obs(itime)%single(i,ie)%ps%val+tl_obs(itime)%single(i,      &
               ie)%ps%val-obs(itime)%single(i,                                 &
                           ie)%ps%val)**2.d0/(obs(itime)%single(i,ie)%ps%error)
           endif
         enddo
       enddo
     endif
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%nsd_e(1, ie)
           if ((obs(itime)%sonde(i,                                            &
                ...
                ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i,               &
                                                     ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,2) = global_shared_buf(ie,2)+                &
               (gs_obs(itime)%sonde(i,ie)%u%val+tl_obs(itime)%sonde(i,         &
               ie)%u%val-obs(itime)%sonde(i,                                   &
                              ie)%u%val)**2.d0/(obs(itime)%sonde(i,ie)%u%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nsd_e(2, ie)
           if ((obs(itime)%sonde(i,                                            &
                ...
                ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i,               &
                                                     ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,3) = global_shared_buf(ie,3)+                &
               (gs_obs(itime)%sonde(i,ie)%v%val+tl_obs(itime)%sonde(i,         &
               ie)%v%val-obs(itime)%sonde(i,                                   &
                              ie)%v%val)**2.d0/(obs(itime)%sonde(i,ie)%v%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nsd_e(3, ie)
           if ((obs(itime)%sonde(i,                                            &
                ...
                ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i,               &
                                                     ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,4) = global_shared_buf(ie,4)+                &
               (gs_obs(itime)%sonde(i,ie)%t%val+tl_obs(itime)%sonde(i,         &
               ie)%t%val-obs(itime)%sonde(i,                                   &
                              ie)%t%val)**2.d0/(obs(itime)%sonde(i,ie)%t%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nsd_e(4, ie)
           if ((obs(itime)%sonde(i,                                            &
                ...
                ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%sonde(i,               &
                                                     ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,5) = global_shared_buf(ie,5)+                &
               (gs_obs(itime)%sonde(i,ie)%q%val+tl_obs(itime)%sonde(i,         &
               ie)%q%val-obs(itime)%sonde(i,                                   &
                              ie)%q%val)**2.d0/(obs(itime)%sonde(i,ie)%q%error)
             global_shared_buf(ie,6) = global_shared_buf(ie,6)+1.d0
           endif
         enddo
       enddo
         ...
         do i = 1, obsmet(itime)%nsf_e(ie)
           if ((obs(itime)%surface(i,                                          &
                ...
                ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%surface(i,             &
                                                     ie)%u%val.ne.sdmiss)) then
             global_shared_buf(ie,7) = global_shared_buf(ie,7)+                &
               (gs_obs(itime)%surface(i,ie)%u%val+tl_obs(itime)%surface(i,     &
               ie)%u%val-obs(itime)%surface(i,                                 &
                            ie)%u%val)**2.d0/(obs(itime)%surface(i,ie)%u%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
                ...
                ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%surface(i,             &
                                                     ie)%v%val.ne.sdmiss)) then
             global_shared_buf(ie,8) = global_shared_buf(ie,8)+                &
               (gs_obs(itime)%surface(i,ie)%v%val+tl_obs(itime)%surface(i,     &
               ie)%v%val-obs(itime)%surface(i,                                 &
                            ie)%v%val)**2.d0/(obs(itime)%surface(i,ie)%v%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
                ...
                ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%surface(i,             &
                                                     ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,9) = global_shared_buf(ie,9)+                &
               (gs_obs(itime)%surface(i,ie)%t%val+tl_obs(itime)%surface(i,     &
               ie)%t%val-obs(itime)%surface(i,                                 &
                            ie)%t%val)**2.d0/(obs(itime)%surface(i,ie)%t%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
                ...
                ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%surface(i,             &
                                                     ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,10) = global_shared_buf(ie,10)+              &
               (gs_obs(itime)%surface(i,ie)%q%val+tl_obs(itime)%surface(i,     &
               ie)%q%val-obs(itime)%surface(i,                                 &
                            ie)%q%val)**2.d0/(obs(itime)%surface(i,ie)%q%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
           if ((obs(itime)%surface(i,                                          &
             ...
             global_shared_buf(ie,11) = global_shared_buf(ie,11)+              &
               (gs_obs(itime)%surface(i,ie)%p%val+tl_obs(itime)%surface(i,     &
               ie)%p%val-obs(itime)%surface(i,                                 &
                            ie)%p%val)**2.d0/(obs(itime)%surface(i,ie)%p%error)
             global_shared_buf(ie,12) = global_shared_buf(ie,12)+1.d0
           endif
         enddo
       enddo
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%nbg_e(ie)
           if ((obs(itime)%bogus(i,                                            &
                ...
                ie)%p%val.ne.sdmiss).and.(tl_obs(itime)%bogus(i,               &
                                                     ie)%p%val.ne.sdmiss)) then
             global_shared_buf(ie,13) = global_shared_buf(ie,13)+              &
               (gs_obs(itime)%bogus(i,ie)%p%val+tl_obs(itime)%bogus(i,         &
               ie)%p%val-obs(itime)%bogus(i,                                   &
                              ie)%p%val)**2.d0/(obs(itime)%bogus(i,ie)%p%error)
             global_shared_buf(ie,14) = global_shared_buf(ie,14)+1.d0
           endif
         enddo
       enddo
         ...
         do i = 1, obsmet(itime)%nar_e(1, ie)
           if ((obs(itime)%aircraft(i,                                         &
             ...
             global_shared_buf(ie,15) = global_shared_buf(ie,15)+              &
               (gs_obs(itime)%aircraft(i,ie)%u%val+tl_obs(itime)%aircraft(i,   &
               ie)%u%val-obs(itime)%aircraft(i,                                &
                           ie)%u%val)**2.d0/(obs(itime)%aircraft(i,ie)%u%error)
             global_shared_buf(ie,18) = global_shared_buf(ie,18)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nar_e(2, ie)
           if ((obs(itime)%aircraft(i,                                         &
             ...
             global_shared_buf(ie,16) = global_shared_buf(ie,16)+              &
               (gs_obs(itime)%aircraft(i,ie)%v%val+tl_obs(itime)%aircraft(i,   &
               ie)%v%val-obs(itime)%aircraft(i,                                &
                           ie)%v%val)**2.d0/(obs(itime)%aircraft(i,ie)%v%error)
             global_shared_buf(ie,18) = global_shared_buf(ie,18)+1.d0
           endif
         enddo
         do i = 1, obsmet(itime)%nar_e(3, ie)
           if ((obs(itime)%aircraft(i,                                         &
             ...
             global_shared_buf(ie,17) = global_shared_buf(ie,17)+              &
               (gs_obs(itime)%aircraft(i,ie)%t%val+tl_obs(itime)%aircraft(i,   &
               ie)%t%val-obs(itime)%aircraft(i,                                &
                           ie)%t%val)**2.d0/(obs(itime)%aircraft(i,ie)%t%error)
             global_shared_buf(ie,18) = global_shared_buf(ie,18)+1.d0
           endif
         enddo
       enddo
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%namv_e(ie)
           if ((obs(itime)%amv(i,                                              &
                ...
                ie)%u%val.ne.sdmiss).and.(tl_obs(itime)%amv(i,                 &
                                                     ie)%u%val.ne.sdmiss)) then
           ...
           if ((obs(itime)%amv(i,                                              &
                ...
                ie)%v%val.ne.sdmiss).and.(tl_obs(itime)%amv(i,                 &
                                                     ie)%v%val.ne.sdmiss)) then
         ...
         do i = 1, obsmet(itime)%nscatwind_e(ie)
           if ((obs(itime)%scatwind(i,                                         &
             ...
             global_shared_buf(ie,22) = global_shared_buf(ie,22)+              &
               (gs_obs(itime)%scatwind(i,ie)%u%val+tl_obs(itime)%scatwind(i,   &
               ie)%u%val-obs(itime)%scatwind(i,                                &
                           ie)%u%val)**2.d0/(obs(itime)%scatwind(i,ie)%u%error)
           ...
           if ((obs(itime)%scatwind(i,                                         &
             ...
             global_shared_buf(ie,23) = global_shared_buf(ie,23)+              &
               (gs_obs(itime)%scatwind(i,ie)%v%val+tl_obs(itime)%scatwind(i,   &
               ie)%v%val-obs(itime)%scatwind(i,                                &
                           ie)%v%val)**2.d0/(obs(itime)%scatwind(i,ie)%v%error)
             global_shared_buf(ie,24) = global_shared_buf(ie,24)+1.d0
           endif
         enddo
       enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%namsua_e(ie)
             if ((obs(itime)%amsua(i, j,                                       &
               ...
               (gs_obs(itime)%amsua(i,j,ie)%t%val+tl_obs(itime)%amsua(i,j,     &
               ie)%t%val-obs(itime)%amsua(i,j,                                 &
                          ie)%tb%val)**2.d0/(obs(itime)%amsua(i,j,ie)%tb%error)
       ...
       do ie = nets, nete
         do i = 1, obsmet(itime)%ngpsro_e(ie)
           if ((obs(itime)%gpsro(i,                                            &
                ...
                ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%gpsro(i,               &
                                                     ie)%t%val.ne.sdmiss)) then
             global_shared_buf(ie,27) = global_shared_buf(ie,27)+              &
               (gs_obs(itime)%gpsro(i,ie)%t%val+tl_obs(itime)%gpsro(i,         &
               ie)%t%val-obs(itime)%gpsro(i,                                   &
                            ie)%ba%val)**2.d0/(obs(itime)%gpsro(i,ie)%ba%error)
             global_shared_buf(ie,29) = global_shared_buf(ie,29)+1.d0
           endif
           if ((obs(itime)%gpsro(i,                                            &
                ie)%ba%val.ne.sdmiss).and.(gs_obs(itime)%gpsro(i,              &
                ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%gpsro(i,               &
                                                     ie)%q%val.ne.sdmiss)) then
             global_shared_buf(ie,28) = global_shared_buf(ie,28)+              &
               (gs_obs(itime)%gpsro(i,ie)%q%val+tl_obs(itime)%gpsro(i,         &
               ie)%q%val-obs(itime)%gpsro(i,                                   &
                            ie)%ba%val)**2.d0/(obs(itime)%gpsro(i,ie)%ba%error)
             global_shared_buf(ie,29) = global_shared_buf(ie,29)+1.d0
           endif
         enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%niasi_e(ie)
             if ((obs(itime)%iasi(i, j,                                        &
               ...
               (gs_obs(itime)%iasi(i,j,ie)%t%val+tl_obs(itime)%iasi(i,j,       &
               ie)%t%val-obs(itime)%iasi(i,j,                                  &
                           ie)%tb%val)**2.d0/(obs(itime)%iasi(i,j,ie)%tb%error)
             ...
             if ((obs(itime)%iasi(i, j,                                        &
               ...
               (gs_obs(itime)%iasi(i,j,ie)%q%val+tl_obs(itime)%iasi(i,j,       &
               ie)%q%val-obs(itime)%iasi(i,j,                                  &
                           ie)%tb%val)**2.d0/(obs(itime)%iasi(i,j,ie)%tb%error)
               global_shared_buf(ie,32) = global_shared_buf(ie,32)+1.d0
             endif
           enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%ncris_e(ie)
             if ((obs(itime)%cris(i, j,                                        &
               ...
               (gs_obs(itime)%cris(i,j,ie)%t%val+tl_obs(itime)%cris(i,j,       &
               ie)%t%val-obs(itime)%cris(i,j,                                  &
                           ie)%tb%val)**2.d0/(obs(itime)%cris(i,j,ie)%tb%error)
             ...
             if ((obs(itime)%cris(i, j,                                        &
               ...
               (gs_obs(itime)%cris(i,j,ie)%q%val+tl_obs(itime)%cris(i,j,       &
               ie)%q%val-obs(itime)%cris(i,j,                                  &
                           ie)%tb%val)**2.d0/(obs(itime)%cris(i,j,ie)%tb%error)
               global_shared_buf(ie,35) = global_shared_buf(ie,35)+1.d0
             endif
           enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%natms_e(ie)
             if ((obs(itime)%atms(i, j,                                        &
               ...
               (gs_obs(itime)%atms(i,j,ie)%t%val+tl_obs(itime)%atms(i,j,       &
               ie)%t%val-obs(itime)%atms(i,j,                                  &
                           ie)%tb%val)**2.d0/(obs(itime)%atms(i,j,ie)%tb%error)
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%natmswv_e(ie)
             if ((obs(itime)%atmswv(i, j,                                      &
               ...
               (gs_obs(itime)%atmswv(i,j,ie)%t%val+tl_obs(itime)%atmswv(i,j,   &
               ie)%t%val-obs(itime)%atmswv(i,j,                                &
                         ie)%tb%val)**2.d0/(obs(itime)%atmswv(i,j,ie)%tb%error)
             ...
             if ((obs(itime)%atmswv(i, j,                                      &
               ...
               (gs_obs(itime)%atmswv(i,j,ie)%q%val+tl_obs(itime)%atmswv(i,j,   &
               ie)%q%val-obs(itime)%atmswv(i,j,                                &
                         ie)%tb%val)**2.d0/(obs(itime)%atmswv(i,j,ie)%tb%error)
               global_shared_buf(ie,40) = global_shared_buf(ie,40)+1.d0
             endif
           enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%nmhs_e(ie)
             if ((obs(itime)%mhs(i, j,                                         &
                ...
                ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%mhs(i, j,              &
                                                     ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,41) = global_shared_buf(ie,41)+            &
               (gs_obs(itime)%mhs(i,j,ie)%t%val+tl_obs(itime)%mhs(i,j,         &
               ie)%t%val-obs(itime)%mhs(i,j,                                   &
                            ie)%tb%val)**2.d0/(obs(itime)%mhs(i,j,ie)%tb%error)
               global_shared_buf(ie,43) = global_shared_buf(ie,43)+1.d0
             endif
             if ((obs(itime)%mhs(i, j,                                         &
                ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%mhs(i, j,             &
                ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%mhs(i, j,              &
                                                     ie)%q%val.ne.sdmiss)) then
               global_shared_buf(ie,42) = global_shared_buf(ie,42)+            &
               (gs_obs(itime)%mhs(i,j,ie)%q%val+tl_obs(itime)%mhs(i,j,         &
               ie)%q%val-obs(itime)%mhs(i,j,                                   &
                            ie)%tb%val)**2.d0/(obs(itime)%mhs(i,j,ie)%tb%error)
               global_shared_buf(ie,43) = global_shared_buf(ie,43)+1.d0
             endif
           enddo
         ...
         do ie = nets, nete
           do i = 1, obsmet(itime)%ncsr_e(ie)
             if ((obs(itime)%csr(i, j,                                         &
                ...
                ie)%t%val.ne.sdmiss).and.(tl_obs(itime)%csr(i, j,              &
                                                     ie)%t%val.ne.sdmiss)) then
               global_shared_buf(ie,44) = global_shared_buf(ie,44)+            &
               (gs_obs(itime)%csr(i,j,ie)%t%val+tl_obs(itime)%csr(i,j,         &
               ie)%t%val-obs(itime)%csr(i,j,                                   &
                            ie)%tb%val)**2.d0/(obs(itime)%csr(i,j,ie)%tb%error)
               global_shared_buf(ie,46) = global_shared_buf(ie,46)+1.d0
             endif
             if ((obs(itime)%csr(i, j,                                         &
                ie)%tb%val.ne.sdmiss).and.(gs_obs(itime)%csr(i, j,             &
                ie)%q%val.ne.sdmiss).and.(tl_obs(itime)%csr(i, j,              &
                                                     ie)%q%val.ne.sdmiss)) then
               global_shared_buf(ie,45) = global_shared_buf(ie,45)+            &
               (gs_obs(itime)%csr(i,j,ie)%q%val+tl_obs(itime)%csr(i,j,         &
               ie)%q%val-obs(itime)%csr(i,j,                                   &
                            ie)%tb%val)**2.d0/(obs(itime)%csr(i,j,ie)%tb%error)
               global_shared_buf(ie,46) = global_shared_buf(ie,46)+1.d0
             endif
           enddo
...
! 1-2. Jb
   ...
   do wn = 1,zwn_nproc
     tmp = tmp+sum((x_eig(:,wn)-x_eig_acc(:,wn))**2.d0)+sum((x_eig_chi(:,wn)-  &
               x_eig_acc_chi(:,wn))**2.d0)+sum((x_eig_tps(:,wn)-               &
               x_eig_acc_tps(:,wn))**2.d0)+sum((x_eig_q(:,wn)-x_eig_acc_q(:,   &
                                                                    wn))**2.d0)
   enddo
   do ismpl = 1,nsmpl
     do wn = 1, zwn_nproc_a
...
! 1-3. Communication and evaluation
   ...
   j_o = 0.5d0*(global_shared_sum(2)+global_shared_sum(3)+global_shared_sum(4)+&
               ...
               global_shared_sum(11)+global_shared_sum(13)+                    &
               global_shared_sum(15)+global_shared_sum(16)+                    &
               global_shared_sum(17)+global_shared_sum(19)+                    &
               global_shared_sum(20)+global_shared_sum(22)+                    &
               global_shared_sum(23)+global_shared_sum(25)+                    &
               global_shared_sum(27)+global_shared_sum(28)+                    &
               global_shared_sum(30)+global_shared_sum(31)+                    &
               global_shared_sum(33)+global_shared_sum(34)+                    &
               global_shared_sum(36)+global_shared_sum(38)+                    &
               global_shared_sum(39)+global_shared_sum(41)+                    &
               global_shared_sum(42)+global_shared_sum(44)+                    &
...
!
   ...
   call mpi_allgatherv(sp_tmp,nlevar*l_zwn_nproc,par_double_precision,sp,      &
               nlevar*l_zwn_blength,nlevar*l_zwn_bstart,par_double_precision,  &
                                                                 par%comm,ierr)
...
!IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     ...
     do k = 1, neig_tps
       ad_eigen_tps(k,wn) = dsqrt(eigv_tps(k,wn))*(sum(bcov_sp_tps(1:nlev,k,   &
               wn)*ad_sp(2*nlev+1:3*nlev,wn))+bcov_sp_tps(nlev+1,k,            &
                                                        wn)*ad_sp(4*nlev+1,wn))
! Check
!IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     enddo
     do k = 1, neig_q
...
!       END DO
     do k = 1,nlev+1
       ad_eigen_a(k,wn) = dsqrt(eigv_a(k,wn))*(sum(bcov_sp_a(1:nlev,           &
               k)*ad_sp(2*nlev+1:3*nlev,wn))+bcov_sp_a(nlev+1,k)*ad_sp(4*nlev+ &
                                                                         1,wn))
!          ! Check
!          !IF(par%isMasterProc) WRITE(iulog,*) 'ad_eigen = ',ad_eigen(k,wn)
     enddo
