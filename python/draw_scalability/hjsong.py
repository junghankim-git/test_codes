...
def Rfunc(r, R0, L):
  ...
  #return 0.1+R0*np.exp(-r/L)
  #return R0*(1.0+r)*np.exp(-r)+L/200
  #return R0*np.exp(L*r)

def LogRfunc(r, R0, L):
  return np.log(R0)+np.log(1.0+r/L) - r/L

def Rfunc_raw(x, R0):
  return R0*(1.0+x)*np.exp(-x)
...
popt, pcov = curve_fit(LogRfunc, x, logy)
print popt, pcov
fun_y = np.exp(LogRfunc(x, popt[0], popt[1]))
