...
int_psi_i   = array([0.0 for i in arange(n+1)])
#analytic
...
int_psi_m   = array([0.0 for i in arange(n)])
#analytic
...
#### Plot #####
ymin       = array([[0.0 for i in arange(4)] for j in arange(2)])
ymax       = array([[0.0 for i in arange(4)] for j in arange(2)])
ymin[0][0] = 0.0
...
axis.set_xlim(0,1)
#axis.set_ylim(-0.05,0.2)
axis.plot(eta_i, int_psi_i, 'ro', label='Numeriacal')
...
axis.set_ylabel(r'$\int \psi (\eta) \mathbf{d} x$')
axis.set_xlim(0,1)
axis.plot(eta_m, int_psi_m, 'ro', label='Numerical')
...
outfilename = 'LagrangeInterpolation_i'+'.png'
pltfig_i.savefig(outfilename)
outfilename = 'LagrangeInterpolation_m'+'.png'
