...
ratio_1 = [0.0 for i in range(nps)]
for i in range(nps):
    ratio_1[i] = walltime[2][i]/walltime[1][i]
...
ratio_2 = [0.0 for i in range(nps)]
for i in range(nps):
    ratio_2[i] = walltime_[2][i]/walltime_[1][i]
...
ylabel2 = 'Ratio'
ylabel3 = 'Speed-up [a.u.]'
comp_r = ['Ratio (Write/Computation)']
carts = ''
pltfig,axis = create_plot(ncols=2,nrows=3,title='KIM scalability')
title1 = '(a) Wall-clock time (NE240L100, 0.5d fcst)'
title2 = '(c) Ratio of writing time to computation time'
title3 = '(e) Speed-up (NE240L100, 0.5d fcst)'
draw_plots(axis[0][0],title1,carts,comps,nprocs,walltime,xlabel=xlabel,ylabel=ylabel1,xmax=xmax1,fmts=fmts)
#--
draw_plots(axis[0][1],title2,carts,comp_r,nprocs,ratio_1,xlabel=xlabel,ylabel=ylabel2,xmin=0.0,xmax=xmax1,ymin=0.0,ymax=ymaxr,leg_loc=2)
#--
draw_plots(axis[0][2],title3,carts,comps[0],nprocs,speedup[0],xlabel=xlabel,ylabel=ylabel3,xmin=0.0,xmax=xmax1,ymin=0.0,ymax=xmax1)
add_plots(axis[0][2],'Ideal',nprocs_id,nprocs_id,['r--'],leg_loc=2)

print ' - plot 2'
title1 = '(b) Wall-clock time (NE240L91, 0.5d fcst)'
title2 = '(d) Ratio of writing time to computation time'
title3 = '(f) Speed-up (NE240L91, 0.5d fcst)'
draw_plots(axis[1][0],title1,carts,comps_,nprocs_,walltime_,xlabel=xlabel,ylabel=ylabel1,xmin=0.0,xmax=xmax2,ymin=0.0,fmts=fmts)
add_plots(axis[1][0],'Total (w/o initialize)',nprocs_,walltime_s,['bo-'])
for i in range(nps):
    axis[0][1].text(nprocs_[i],80,str(nios[i]),color='k',size=10,va='center',ha='center')
#--
draw_plots(axis[1][1],title2,carts,comp_r,nprocs_,ratio_2,xlabel=xlabel,ylabel=ylabel2,xmin=0.0,xmax=xmax2,ymin=0.0,ymax=ymaxr,leg_loc=2)
#--
draw_plots(axis[1][2],title3,carts,comps_[0],nprocs_,speedup_[0],xlabel=xlabel,ylabel=ylabel3,xmin=0.0,xmax=xmax2,ymin=0.0,ymax=xmax2)
add_plots(axis[1][2],'Total (w/o initialize)',nprocs_,speedup_s,['bo-'])
add_plots(axis[1][2],'Ideal',nprocs_id,nprocs_id,['r--'],leg_loc=2)
print 'parallel efficiency = ', nprocs_[-1], speedup_s[-1], speedup_s[-1]/nprocs_[-1]

pltfig.savefig('figure_08.png')
