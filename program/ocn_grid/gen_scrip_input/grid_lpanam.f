...
c jhkim
      integer :: fid, idid, jdid, ivid, jvid, ivid_q, jvid_q, err
c jhkim
c
...
c jhkim
      print *, 'start jhkim'
      err = nf90_create('grid.nc',
     &      ior(nf90_64bit_offset,nf90_clobber),fid)
      if (err.ne.0) print *,'nc error (01) '
      err = nf90_def_dim(fid,'idm',idm,idid)
      if (err.ne.0) print *,'nc error (02) '
      err = nf90_def_dim(fid,'jdm',jdm,jdid)
      if (err.ne.0) print *,'nc error (03) '
      err = nf90_def_var(fid,'plon',nf90_real8,(/idid,jdid/),ivid)
      if (err.ne.0) print *,'nc error (04) '
      err = nf90_def_var(fid,'plat',nf90_real8,(/idid,jdid/),jvid)
      if (err.ne.0) print *,'nc error (05) '
      err = nf90_def_var(fid,'qlon',nf90_real8,(/idid,jdid/),ivid_q)
      if (err.ne.0) print *,'nc error (06) '
      err = nf90_def_var(fid,'qlat',nf90_real8,(/idid,jdid/),jvid_q)
      if (err.ne.0) print *,'nc error (07) '
      err = nf90_put_att(fid,nf90_global,'reflon',reflon)
      if (err.ne.0) print *,'nc error (08) '
      err = nf90_put_att(fid,nf90_global,'grdlon',grdlon)
      if (err.ne.0) print *,'nc error (09) '
      err = nf90_put_att(fid,nf90_global,'half',half)
      if (err.ne.0) print *,'nc error (11) '
      err = nf90_enddef(fid)
      if (err.ne.0) print *,'nc error (12) '
      err = nf90_put_var(fid,ivid,plon)
      if (err.ne.0) print *,'nc error (13) '
      err = nf90_put_var(fid,jvid,plat)
      if (err.ne.0) print *,'nc error (14) '
      err = nf90_put_var(fid,ivid_q,qlon)
      if (err.ne.0) print *,'nc error (15) '
      err = nf90_put_var(fid,jvid_q,qlat)
      if (err.ne.0) print *,'nc error (16) '
      err = nf90_close(fid)
      if (err.ne.0) print *,'nc error (16) '
      print *, 'end jhkim'
c jhkim
