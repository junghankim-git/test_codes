...
inputfile  = basedir+'/file/grims_'+levstr
...
for iline in file:
   line          = iline.strip()
   splitline2    = str(line).split('"')
   splitline     = str(splitline2[1]).split(',')
   for ilev in range(nlevs+1):
      if ii == 0:
         coefA_h[nlevs-ilev] = float(splitline[ilev])
      elif ii == 1:
         coefB_h[nlevs-ilev] = float(splitline[ilev])
   ii = ii + 1

file.close()
if len(splitline) != nlevs+1:
   print 'Check: nlevs'
   quit()
...
hvifile = basedir+'/grims_half_'+levstr
hvmfile = basedir+'/grims_full_'+levstr
...
for ilev in range(nlevs+1):
   ...
   if p_h[ilev] != 0.0: h_h[ilev] = Hs*log(p0/p_h[ilev])
