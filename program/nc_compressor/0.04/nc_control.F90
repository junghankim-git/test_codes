...
!
   ...
   call nc_check(err,'nc_initialize::cannot open...')
   err = nf90_inquire(nc%in_nc,nc%ndims,nc%nvars,nc%natts)
   call nc_check(err,'nc_initialize::nf90_inquire')
...
! variables
   do ii = 1,nc%nvars
     ...
     call nc_check(err1,'check get variable')
     call nc_check(err2,'check put variable')
     deallocate(dsizes)
   enddo
!
